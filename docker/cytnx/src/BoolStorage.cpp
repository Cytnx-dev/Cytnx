...
// here, cpu->gpu with gid=device
        ...
        checkCudaErrors(
          cudaMemcpy(dtmp, this->Mem, sizeof(bool) * this->len, cudaMemcpyHostToDevice));
        free(this->Mem);
        this->Mem = dtmp;
        this->device = device;
#else
        ...
        if (device == Device.cpu) {
          ...
          checkCudaErrors(
            cudaMemcpy(htmp, this->Mem, sizeof(bool) * this->len, cudaMemcpyDeviceToHost));
          cudaFree(this->Mem);
          this->Mem = htmp;
          this->device = device;
        } else {
          ...
          void *dtmp = utils_internal::cuMalloc_gpu(sizeof(bool) * this->len);
          checkCudaErrors(
            cudaMemcpyPeer(dtmp, device, this->Mem, this->device, sizeof(bool) * this->len));
          cudaFree(this->Mem);
          this->Mem = dtmp;
          this->device = device;
        }
