...
class Hxx: public LinOp{
    ...
    Tensor matvec(const Tensor &psi) override{
        ...
        psic.reshape_((this->L).shape()[1],(this->M1).shape()[2],(this->M2).shape()[2],(this->R).shape()[1]);
...
std::vector<UniTensor> DMRG(std::vector<UniTensor>& A, UniTensor& ML, UniTensor& M, UniTensor& MR, int chi, int Nsweeps, int maxit, int krydim){
    ...
    for(int k = 1; k<Nsweeps+2; k++){
        ...
        for(int p = Nsites-2; p>-1; p--){
            ...
            R_AMAH.PutUniTensors({"R","B","M","B_Conj"},{LR[p+2],A[p+1],M,A[p+1].Conj()}, false);
        ...
        for(int p = 0; p < Nsites-1; p++){
            ...
            // /home/j9263178/cytnx_test/lsq.cpp:98: 未定義參考到 cytnx::UniTensor cytnx::operator/<cytnx::Scalar::Sproxy>(cytnx::UniTensor const&, cytnx::Scalar::Sproxy const&)
            // /home/j9263178/cytnx_test/lsq.cpp:136: 未定義參考到 cytnx::UniTensor cytnx::operator/<cytnx::Scalar::Sproxy>(cytnx::UniTensor const&, cytnx::Scalar::Sproxy const&)
            // collect2: 錯誤：ld 回傳 1
            s = svdtemp[0]; s.Div_(s.get_block_().Norm().item());
            u = svdtemp[1];
            vT = svdtemp[2];
...
int main(){
    ...
    auto Sp = zeros({2,2}); Sp.at<cytnx_double>(0, 1) = 1;
    auto Sm = zeros({2,2}); Sm.at<cytnx_double>(1, 0) = 1;
