...
def dmrg_XXmodel_U1(Nsites, chi, numsweeps, maxit):
    ...
    L0 = cytnx.UniTensor([bd_inner.redirect(),VbdL.redirect(),VbdL]) \
                         .set_rowrank_(1).set_name_("L0") #Left boundary
    R0 = cytnx.UniTensor([bd_inner,VbdR,VbdR.redirect()]) \
                         .set_rowrank_(1).set_name_("R0") #Right boundary
    L0.set_elem([0,0,0],1)
    R0.set_elem([3,0,0],1)
    ...
    for k in range(1,Nsites):
        ...
        A[k] = cytnx.UniTensor([B1,B2,B3]).set_rowrank_(2).set_name_(f"A{k}")
