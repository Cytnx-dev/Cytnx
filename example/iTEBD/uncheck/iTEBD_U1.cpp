...
int main(int argc, char *argv[]) {
    ...
    UniTensor la = UniTensor({bd_mid,bd_mid.redirect()},vector<string>({"-2","-3"}),-1,Type.Double,Device.cpu,true);
    UniTensor lb = UniTensor({bdi,bdo},vector<string>({"-4","-5"}),-1,Type.Double,Device.cpu,true);
    ...
    for(int i = 0; i < 10000; i++){
        ...
        cytnx_double Norm = 0;
        for(int a = 0; a < la.get_blocks_().size(); a++){
            Norm += double((linalg::Norm(la.get_block_ (a)).item()*
                    linalg::Norm(la.get_block_(a)).item()).real());
        }
        Norm = sqrt(Norm);
        for(int a = 0; a < la.get_blocks_().size(); a++){
