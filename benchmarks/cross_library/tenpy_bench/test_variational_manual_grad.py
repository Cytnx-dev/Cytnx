...
initial state, so (as with quimb's AD benchmark, which uses the same
`LEARNING_RATE`/`_n_grad_steps`) each test asserts only against its own
`REFERENCE_ENERGIES` at a tight `rel=1e-6` self-consistency tolerance --
this run is fully deterministic given the seeded initial state, so no
cross-library energy comparison is expected to land this close.
"""
import numpy as np
import pytest
