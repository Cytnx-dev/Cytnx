...
namespace BMTest_Vstack {
  ...
  static void BM_Vstack_F64(benchmark::State& state) {
    ...
    // start test here
    for (auto _ : state) {
      Tensor vstack_tens = algo::Vstack(tens_list);
    }
  }
  ...
  static void BM_Vstack_C128(benchmark::State& state) {
    // prepare data
    auto D_row = state.range(0);
    auto D_col = state.range(1);
    auto tens_num = state.range(2);
  ...
  BENCHMARK(BM_Vstack_C128)
    //{D_row, D_col, tens_num}
    ->Args({10, 10, 10})
    ->Args({100, 100, 100})
    ->Args({1000, 1000, 10});

  static void BM_Vstack_F64_non_conti(benchmark::State& state) {
    // prepare data
    auto D_row = state.range(0);
    auto D_col = state.range(1);
    auto tens_num = state.range(2);
  ...
  BENCHMARK(BM_Vstack_F64_non_conti)
    //{D_row, D_col, tens_num}
    ->Args({10, 10, 10})
    ->Args({100, 100, 100})
    ->Args({1000, 1000, 10});
