            ...
            public:
                ...
                */            
                ...
                void accumulate(const std::vector<T> &data_x,const std::vector<T> &data_y){
                    //[not fin!]
                    cytnx_error_msg(data_x.size()!=data_y.size(), "[ERROR][Histogram2d::accumulate] data_x and data_y should have same size!%s","\n");
                    ...
                    for(unsigned long long i=0;i<data_x.size();i++){
                        ...
                        vars.at<double>(ny*binx+nx) +=1; total_count+=1; 
