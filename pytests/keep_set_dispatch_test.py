...
def test_storage_from_pylist_empty_list_defaults_to_double():
    ...
    # from_pylist([]) is at the mercy of registration order alone unless
    # handled explicitly. Handled in the first-registered overload's body
    # (a runtime check, not a reordering, so it can't disturb
    # mypy.stubtest's view of the stub signatures) to match numpy/the Python
    # array API's own empty-array, no-explicit-dtype default:
    # np.array([]).dtype == float64. This changed from ComplexDouble, this
    # overload's default before the keep-set consolidation.
