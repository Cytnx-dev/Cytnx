...
namespace BMTest_UniTensor {
  std::vector<cytnx::UniTensor> ConstructDenseUT(const long unsigned int D, 
	...
	*/
    ...
    auto T = cytnx::random::uniform(shape = {4, D, D}, low = -1.0, high = 1.0, 
                             device, seed = 0, dtype);

    cytnx::UniTensor L = cytnx::UniTensor(
        T, is_diag = false, rowrank = -1, 
        labels = {"-5", "-1", "0"} , name = "L");

    cytnx::UniTensor R = cytnx::UniTensor(
        T, is_diag = false, rowrank = -1, 
        labels = {"-7", "-4", "3"} , name = "R");

    T = cytnx::random::uniform(shape = {4, 4, 2, 2}, low = -1.0, high = 1.0, 
                        device, seed = 0, dtype);

    cytnx::UniTensor M1 = cytnx::UniTensor(
        T, is_diag = false, rowrank = -1, 
        labels = {"-5", "-6", "-2", "1"} , "M1");

    cytnx::UniTensor M2 = cytnx::UniTensor(
        T, is_diag = false, rowrank = -1, 
        labels = {"-6", "-7", "-3", "2"} , "M2");

    T = cytnx::random::uniform(shape = {D, 2, 2, D}, low = -1.0, high = 1.0, 
                        device, seed = 0, dtype);

    cytnx::UniTensor psi = cytnx::UniTensor(
        T, is_diag = false, rowrank = -1, 
        labels = {"-1", "-2", "-3", "-4"} , "psi");
  ...
  static void Cytnx_Hpsi_dense_F64_cpu(benchmark::State& state)
  {
	...
	auto& L = unitens[0], R = unitens[1], 
          M1 = unitens[2], M2 = unitens[3], psi = unitens[4];
    for (auto _: state) {
      auto out = cytnx::Contracts({L, R, M1, M2, psi}, 
                                  "(L,(M1,(M2,(psi,R))))", false);
    }
  }
  BENCHMARK(Cytnx_Hpsi_dense_F64_cpu)
      ->Args({1})->Args({10})->Args({100})->Args({1000});
  ...
  BENCHMARK(Cytnx_Hpsi_dense_C128_cpu)
      ->Args({1})->Args({10})->Args({100})->Args({1000});
  ...
  BENCHMARK(Cytnx_Hpsi_dense_F64_gpu)
      ->Args({1})->Args({10})->Args({100})->Args({1000});
  ...
  BENCHMARK(Cytnx_Hpsi_dense_C128_gpu)
      ->Args({1})->Args({10})->Args({100})->Args({1000});
#endif
