...
// Tensor
Tensor ConstructMat(const cytnx_uint64 row, 
                    const cytnx_uint64 col, const int dtype) {
...
UniTensor ConstructDenseUT(const int D, const int dtype) {
  auto bd_i = Bond(D, BD_IN);
  auto bd_o = Bond(D, BD_OUT);
  std::vector<Bond> bonds = {bd_i, bd_o};
...
// Block UniTensor
UniTensor ConstructBkUT(const int deg, const int dtype, 
                        const int symType, const int n = 0) {
  //construct bonds
