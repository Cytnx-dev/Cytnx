    ...
    namespace {
      ...
      TEST_F(BlockUniTensorTest, Init) {
        ...
        EXPECT_NO_THROW(BkUt.Init({phy, phy.redirect(), aux}, {"a", "b", "c"}, 1, Type.ComplexFloat,
                                  Device.cpu, false, false));
        EXPECT_NO_THROW(BkUt.Init({phy, phy.redirect(), aux}, {"a", "b", "c"}, 1,
                                  Type.ComplexDouble, Device.cpu, false, false));
        ...
        EXPECT_ANY_THROW(BkUt.Init({phy, phy.redirect(), aux}, {"a", "b", "c"}, 99, Type.Float,
                                   Device.cpu, false, false));
        EXPECT_NO_THROW(BkUt.Init({phy, phy.redirect(), aux}, {"a", "b", "c"}, 2, Type.Float,
                                  Device.cpu, false, false));
        EXPECT_NO_THROW(BkUt.Init({phy, phy.redirect(), aux}, {"a", "b", "c"}, 1, Type.Float,
                                  Device.cpu, false, false));
        EXPECT_NO_THROW(BkUt.Init({phy, phy.redirect(), aux}, {"a", "b", "c"}, -1, Type.Float,
        ...
        EXPECT_ANY_THROW(BkUt.Init({phy, phy.redirect(), aux}, {"a", "b", "c"}, -2, Type.Float,
                                   Device.cpu, false, false));
        EXPECT_NO_THROW(BkUt.Init({phy, phy.redirect(), aux}, {"a", "b", "c"}, 0, Type.Float,
                                  Device.cpu, false, false));
      ...
      TEST_F(BlockUniTensorTest, SetRowrank) {
      ...
      TEST_F(BlockUniTensorTest, DtypeStr) {
      ...
      TEST_F(BlockUniTensorTest, DeviceStr) { EXPECT_EQ(Spf.device_str(), "cytnx device: CPU"); }
      ...
      TEST_F(BlockUniTensorTest, IsContiguous) {
      ...
      TEST_F(BlockUniTensorTest, IsBlockform) {
      ...
      TEST_F(BlockUniTensorTest, Relabels2) {
      ...
      TEST_F(BlockUniTensorTest, Relabel2) {
        auto tmp = BUT1.clone();
        ...
        for (cytnx_uint64 j = 0; j < 10; j++)
          for (cytnx_uint64 k = 0; k < 10; k++) {
            ...
            if (permuted.at({j, k}).exists())
              EXPECT_EQ(double(permuted.at({j, k}).real()),
                        double(UT_permute_ans2.at({j, k}).real()));
          }
      }

      TEST_F(BlockUniTensorTest, Permute12) {
        // rank-3 tensor
        std::vector<cytnx_int64> a = {1, 2, 0};
      ...
      TEST_F(BlockUniTensorTest, Permute22) {
        std::vector<cytnx_int64> a = {1, 0};
        ...
        for (cytnx_uint64 j = 0; j < 10; j++)
          for (cytnx_uint64 k = 0; k < 10; k++) {
            ...
            if (uT_indep.at({j, k}).exists())
              EXPECT_EQ(double(uT_indep.at({j, k}).real()),
                        double(UT_permute_ans2.at({j, k}).real()));
          }
      ...
      TEST_F(BlockUniTensorTest, Contiguous2) {
      ...
      TEST_F(BlockUniTensorTest, GroupBasis) {
      ...
      TEST_F(BlockUniTensorTest, AtForSparse) {
      ...
      TEST_F(BlockUniTensorTest, GetBlockByidx) {
      ...
      TEST_F(BlockUniTensorTest, GetBlockByqnum) {
      ...
      TEST_F(BlockUniTensorTest, GetBlocks) {
      ...
      TEST_F(BlockUniTensorTest, GetBlocks2) {
      ...
      TEST_F(BlockUniTensorTest, PutBlockByidx) {
      ...
      TEST_F(BlockUniTensorTest, PutBlockByidx2) {
      ...
      TEST_F(BlockUniTensorTest, PutBlockByqnum) {
      ...
      TEST_F(BlockUniTensorTest, PutBlockByqnum2) {
      ...
      TEST_F(BlockUniTensorTest, Reshape2) { EXPECT_ANY_THROW(Spf.reshape_({4, 1}, 1)); }
      ...
      TEST_F(BlockUniTensorTest, ContractMixedDtypeOrderIndependent) {
      ...
      TEST_F(BlockUniTensorTest, FullContractPreservesSymmetryMetadata) {
        ...
        EXPECT_THROW(left.item(), error);
      }

      TEST_F(BlockUniTensorTest, DiagonalPermutePreservesBlocks) {
        Bond bi = Bond(BD_IN, {Qs(0) >> 1, Qs(1) >> 1});
        UniTensor diag =
          UniTensor({bi, bi.redirect()}, {"a", "b"}, 1, Type.Double, Device.cpu, true);
      ...
      TEST_F(BlockUniTensorTest, SameData) {
              ...
              for (cytnx_uint64 k = 0; k < 3; k++)
                for (cytnx_uint64 l = 0; l < 5; l++) {
                  ...
                  if (proxy.exists()) {
                    Scalar val = proxy;
                    proxy = std::pow(
                      cytnx_complex128((cytnx_double)val.real(), (cytnx_double)val.imag()), p);
                  }
                }
        ...
        for (cytnx_uint64 i = 0; i < 5; i++)
          for (cytnx_uint64 j = 0; j < 9; j++)
            for (cytnx_uint64 k = 0; k < 30; k++) {
              if (UT_pB.at({i, j, k}).exists()) {
                EXPECT_DOUBLE_EQ(double(tmp.at({k, j, i}).real()),
                                 double(UT_pB.at({i, j, k}).real()));
              } else {
      ...
      TEST_F(BlockUniTensorTest, ElemExist) {
        for (cytnx_uint64 i = 0; i < 5; i++)
          for (cytnx_uint64 j = 0; j < 11; j++)
            for (cytnx_uint64 k = 0; k < 3; k++)
              for (cytnx_uint64 l = 0; l < 5; l++)
                if (BUT4.elem_exists({i, j, k, l})) {
                  ...
                  EXPECT_EQ(
                    BUT4.bonds()[0].qnums()[qind[0]][0] - BUT4.bonds()[1].qnums()[qind[1]][0] +
        ...
        for (cytnx_uint64 i = 0; i < UT_diag_cplx.bonds()[0].qnums().size(); i++) {
          ...
          for (cytnx_uint64 j = 0; j < deg; j++) {
            ...
            EXPECT_DOUBLE_EQ(double(UT_diag_cplx.at({offset + j, offset + j}).real()),
                             double(i + 1));
            EXPECT_DOUBLE_EQ(double(UT_diag_cplx.at({offset + j, offset + j}).imag()),
                             double(i + 1));
          }
          offset += deg;
        }
      ...
      TEST_F(BlockUniTensorTest, GetQindices) {
      ...
      TEST_F(BlockUniTensorTest, GetItoi) {
      ...
      TEST_F(BlockUniTensorTest, GetBondRef) {
        /*
      ...
      // Dense <-> Block round-trip: full block content (incl. within-block off-diagonal) is
      // preserved, symmetry-forbidden entries stay zero, and convert_from returns *this for
      // chaining.
      TEST_F(BlockUniTensorTest, ConvertFromDenseBlockRoundtrip) {
      ...
      TEST_F(BlockUniTensorTest, ConvertFromDenseBlockRoundtrip2) {
        Bond bi = Bond(BD_IN, {Qs(0) >> 2, Qs(1) >> 2});
        UniTensor B = UniTensor({bi, bi.redirect()});
        B.at({0, 0}) = 1.0;
        B.at({0, 1}) = 5.0;
      ...
      // Dense -> Block honors tol in all cases: a nonzero symmetry-forbidden entry is rejected at
      // the default tol=0, but a large tol or force=true tolerates it (the forbidden entry is
      // dropped and the allowed entries reproduce the original block exactly).
      TEST_F(BlockUniTensorTest, ConvertFromTolForbiddenNonzero) {
        Bond bi = Bond(BD_IN, {Qs(0) >> 2, Qs(1) >> 2});
        UniTensor B = UniTensor({bi, bi.redirect()});
        B.at({0, 0}) = 1.0;
        B.at({0, 1}) = 5.0;
      ...
      TEST_F(BlockUniTensorTest, ConvertFromDiagonalDenseTargetThrows) {
        Bond bi = Bond(BD_IN, {Qs(0) >> 1, Qs(1) >> 1});
        UniTensor B = UniTensor({bi, bi.redirect()});
        B.at({0, 0}) = 1.0;
        B.at({1, 1}) = 2.0;
      ...
      TEST_F(BlockUniTensorTest, ToDenseDiag) {
        ...
        for (auto dtype : {Type.ComplexDouble, Type.Double}) {
          ...
          for (cytnx_uint64 b = 0; b < B.Nblocks(); b++)
            EXPECT_TRUE(
              AreNearlyEqTensor(dense.get_block_(b), linalg::Diag(B.get_block_(b)), 1e-14));
      ...
      TEST_F(BlockUniTensorTest, ToDenseNonDiag) {
      ...
      TEST_F(BlockUniTensorTest, BondObtainedFromUniTensorCannotDesyncBlockStructure) {
        ...
        UniTensor ut =
          UniTensor({bi, bi.redirect()}, {"a", "b"}, 1, Type.Double, Device.cpu, false);
      ...
      /*=====test info=====
      describe:#1052 regression -- BlockUniTensor::combineBonds must correctly regroup a
      *three*-bond merge, not just two. Two internal bookkeeping steps behave differently once 3+
      bonds are combined at once: the cb_stride left-shift (previously done with an overlapping
      memcpy that is well-defined UB only up to 2 combined bonds) and the _inner_to_outer_idx tail
      shift. Check both a structural invariant (the combined bond's dimension is the product of the
      three input dimensions, computed independently of combineBonds -- it comes straight from
        ...
        (idx_a * (N_b*N_c) + idx_b*N_c + idx_c) -- independent of the buggy bookkeeping this PR
      fixes.
      ====================*/
