  ...
  for(size_t j=1;j<=11;j++)
      for(size_t k=1;k<=3;k++)
        if(BUtrT4.at({j-1,k-1}).exists()){
          ...
          EXPECT_FLOAT_EQ(double(tmp.at({j-1,k-1}).real()),double(BUtrT4.at({j-1,k-1}).real()));
        }
...
TEST_F(BlockUniTensorTest, Norm){
  ...
  EXPECT_FLOAT_EQ(double(BUT4.Norm().at({0}).real()),10.02330912178208);
}
  ...
  for(size_t i=1;i<=2;i++)for(size_t j=1;j<=1;j++)
      for(size_t k=1;k<=2;k++)for(size_t l=1;l<=4;l++)
          if(BUT4.elem_exists({i-1,j-1,k-1,l-1})){
            EXPECT_EQ(BUT4.bonds()[0].qnums()[i-1][0]-
            BUT4.bonds()[1].qnums()[j-1][0]+
            BUT4.bonds()[2].qnums()[k-1][0]-
            BUT4.bonds()[3].qnums()[l-1][0],0);
          }
