...
static std::map<int, std::string> device_str_map = {
      {Device.cpu , "cytnx device: CPU" }, 
      {Device.cuda, "cytnx device: CUDA"}
};

// declaration of methods
void CheckTensStruct(const Tensor& T, unsigned int expect_type, int expect_device, std::vector<unsigned int> expect_shape);
...
TEST(Tensor, plus_constant){
  ...
  ValuesList V1_2{std::complex<double>(1,7),std::complex<double>(1,7),std::complex<double>(1,7),std::complex<double>(1,7)};
...
TEST(Tensor, SVD){
  ...
  auto SVDs = T1.Svd();
  std::cout << SVDs[0] << std::endl;
  std::cout << SVDs[1] << std::endl;
  std::cout << SVDs[2] << std::endl;
}
