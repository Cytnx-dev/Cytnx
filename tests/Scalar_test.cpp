...
namespace cytnx {
  namespace test {
    namespace {
      ...
      static void ExpectDoubleScalarEq(const Scalar &value, double expected) {
        EXPECT_EQ(value.dtype(), Type.Double);
        EXPECT_DOUBLE_EQ(static_cast<cytnx_double>(value), expected);
      }

      static void ExpectInt64ScalarEq(const Scalar &value, cytnx_int64 expected) {
        EXPECT_EQ(value.dtype(), Type.Int64);
        EXPECT_EQ(static_cast<cytnx_int64>(value), expected);
      }

      static void ExpectUint64ScalarEq(const Scalar &value, cytnx_uint64 expected) {
        EXPECT_EQ(value.dtype(), Type.Uint64);
        EXPECT_EQ(static_cast<cytnx_uint64>(value), expected);
      }

      static void ExpectComplexDoubleScalarEq(const Scalar &value,
                                              const cytnx_complex128 &expected) {
        EXPECT_EQ(value.dtype(), Type.ComplexDouble);
        const auto got = complex128(value);
      ...
      static Scalar RepresentativeValue(unsigned int dtype) {
      ...
      static Scalar RepresentativeSecondValue(unsigned int dtype) {
      ...
      static cytnx_complex128 AsComplex(const Scalar &s) {
        return complex128(s.astype(Type.ComplexDouble));
      }
      ...
      TEST(ScalarTest, MinvalForFloatingDtypesUsesLowest) {
        ...
        EXPECT_DOUBLE_EQ(static_cast<cytnx_double>(min_d),
                         std::numeric_limits<cytnx_double>::lowest());
        EXPECT_LT(static_cast<cytnx_double>(min_d), 0.0);

        Scalar min_f = Scalar::minval(Type.Float);
        EXPECT_FLOAT_EQ(static_cast<cytnx_float>(min_f),
                        std::numeric_limits<cytnx_float>::lowest());
      ...
      TEST(ScalarTest, MaxvalForFloatingDtypesUsesMax) {
        ...
        EXPECT_DOUBLE_EQ(static_cast<cytnx_double>(max_d),
                         std::numeric_limits<cytnx_double>::max());
      }
      ...
      TEST(ScalarTest, IabsOnComplexKeepsDtypeAndMatchesAbsMagnitude) {
        ...
        cytnx_complex128 v = complex128(z);
