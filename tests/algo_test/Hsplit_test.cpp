  ...
  namespace {
    ...
    namespace HsplitTest {
      ...
      input:
        ...
        dims:[2, 3, 2]
      ====================*/
      TEST(Hsplit, VoidAPIOutputNotEmpty) {
        for (const auto& dtype : dtype_list) {
          if (dtype == Type.Bool) continue;
          Tensor T = Tensor({10, 7}, dtype);
          InitTensorUniform(T);
      ...
      input:
        ...
        dims:[3, 4, 0]
      ====================*/
      TEST(Hsplit, ErrDimsContainsZero) {
        Tensor T = Tensor({10, 7}, Type.Double);
        InitTensorUniform(T);
