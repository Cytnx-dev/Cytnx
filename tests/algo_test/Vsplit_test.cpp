...
namespace cytnx {
  namespace test {
    namespace {
      ...
      static void CheckResult(const Tensor& T_in, const std::vector<Tensor>& vsplit_tens,
      ...
      input:
        ...
        dims:[2, 3, 2]
      ====================*/
      TEST(Vsplit, VoidAPIOutputNotEmpty) {
        for (const auto& dtype : dtype_list) {
          if (dtype == Type.Bool) continue;
          Tensor T = Tensor({7, 10}, dtype);
          InitTensorUniform(T);
      ...
      input:
        ...
        dims:[3, 4, 0]
      ====================*/
      TEST(Vsplit, ErrDimsContainsZero) {
        Tensor T = Tensor({7, 10}, Type.Double);
        InitTensorUniform(T);
