...
TEST_F(BlockUniTensorTest, gpu_syms){
...
TEST_F(BlockUniTensorTest, gpu_Transpose){
...
TEST_F(BlockUniTensorTest, gpu_Dagger){
...
TEST_F(BlockUniTensorTest, gpu_truncate){
...
TEST_F(BlockUniTensorTest, gpu_Init){
    ...
    EXPECT_NO_THROW(BkUt.Init({phy,phy.redirect(),aux},{"a", "b", "c"},1,Type.Double,Device.cpu,false,false));
    EXPECT_NO_THROW(BkUt.Init({phy,phy.redirect(),aux},{"a", "b", "c"},1,Type.ComplexFloat,Device.cpu,false,false));
    EXPECT_NO_THROW(BkUt.Init({phy,phy.redirect(),aux},{"a", "b", "c"},1,Type.ComplexDouble,Device.cpu,false,false));
    ...
    //valid rowranks
    EXPECT_ANY_THROW(BkUt.Init({phy,phy.redirect(),aux},{"a", "b", "c"},99,Type.Float,Device.cpu,false,false));
    ...
    EXPECT_ANY_THROW(BkUt.Init({phy,phy.redirect(),aux},{"a", "b", "c"},1,Type.Float,Device.cpu,true,false));

    // is_diag = true, but rowrank!=1
    EXPECT_ANY_THROW(BkUt.Init({phy,phy.redirect()},{"a", "b"},2,Type.Float,Device.cpu,true,false));
...
TEST_F(BlockUniTensorTest, gpu_shape){
    EXPECT_EQ(std::vector<cytnx::cytnx_uint64>({2,2,1}), Spf.shape());
}


TEST_F(BlockUniTensorTest, gpu_dtype) {
...
TEST_F(BlockUniTensorTest, gpu_dtype_str) {
...
TEST_F(BlockUniTensorTest, gpu_device) {
    EXPECT_EQ(Spf.device(), Device.cuda);
}
...
TEST_F(BlockUniTensorTest, gpu_set_rowrank) {
...
TEST_F(BlockUniTensorTest, gpu_astype) {
...
TEST_F(BlockUniTensorTest, gpu_Nblocks) {
    EXPECT_EQ(UT_pB.Nblocks(), 4);
}

TEST_F(BlockUniTensorTest, gpu_reshape) {
    EXPECT_ANY_THROW(Spf.reshape({4,1},1));
}

TEST_F(BlockUniTensorTest, gpu_reshape_) {
    EXPECT_ANY_THROW(Spf.reshape_({4,1},1));
}

TEST_F(BlockUniTensorTest, gpu_contiguous) {
...
TEST_F(BlockUniTensorTest, gpu_contiguous_) {
...
TEST_F(BlockUniTensorTest, gpu_same_data) {
...
TEST_F(BlockUniTensorTest, gpu_put_block__byidx) {
...
TEST_F(BlockUniTensorTest, gpu_put_block_byidx) {
...
TEST_F(BlockUniTensorTest, gpu_put_block__byqnum) {
...
TEST_F(BlockUniTensorTest, gpu_put_block_byqnum) {
...
TEST_F(BlockUniTensorTest, gpu_permute1) {
    // rank-3 tensor
...
TEST_F(BlockUniTensorTest, gpu_permute2) {
...
TEST_F(BlockUniTensorTest, gpu_permute_1) {
    // rank-3 tensor
    std::vector<cytnx_int64> a = {1,2,0};
...
TEST_F(BlockUniTensorTest, gpu_permute_2) {
    std::vector<cytnx_int64> a = {1,0};
    ...
    for(size_t i=1;i<=5;i++)for(size_t j=1;j<=11;j++)
      for(size_t k=1;k<=3;k++)for(size_t l=1;l<=5;l++)
        if(out2.at({i-1,j-1,k-1,l-1}).exists()){
          EXPECT_DOUBLE_EQ(double(out2.at({i-1,j-1,k-1,l-1}).real()), double(BUTpT2.at({i-1,j-1,k-1,l-1}).real()));
          EXPECT_DOUBLE_EQ(double(out2.at({i-1,j-1,k-1,l-1}).imag()), double(BUTpT2.at({i-1,j-1,k-1,l-1}).imag()));
        }
    BUT4.Add_(BUT4_2);
    for(size_t i=1;i<=5;i++)for(size_t j=1;j<=11;j++)
      for(size_t k=1;k<=3;k++)for(size_t l=1;l<=5;l++)
        if(BUT4.at({i-1,j-1,k-1,l-1}).exists()){
          EXPECT_DOUBLE_EQ(double(BUT4.at({i-1,j-1,k-1,l-1}).real()), double(BUTpT2.at({i-1,j-1,k-1,l-1}).real()));
          EXPECT_DOUBLE_EQ(double(BUT4.at({i-1,j-1,k-1,l-1}).imag()), double(BUTpT2.at({i-1,j-1,k-1,l-1}).imag()));
        }
}

TEST_F(BlockUniTensorTest, gpu_Sub){
    using namespace std::complex_literals;
    ...
    for(size_t i=1;i<=5;i++)for(size_t j=1;j<=11;j++)
      for(size_t k=1;k<=3;k++)for(size_t l=1;l<=5;l++)
        if(out2.at({i-1,j-1,k-1,l-1}).exists()){
          EXPECT_DOUBLE_EQ(double(out2.at({i-1,j-1,k-1,l-1}).real()), double(BUTsT2.at({i-1,j-1,k-1,l-1}).real()));
          EXPECT_DOUBLE_EQ(double(out2.at({i-1,j-1,k-1,l-1}).imag()), double(BUTsT2.at({i-1,j-1,k-1,l-1}).imag()));
        }
    BUT4.Sub_(BUT4_2);
    for(size_t i=1;i<=5;i++)for(size_t j=1;j<=11;j++)
      for(size_t k=1;k<=3;k++)for(size_t l=1;l<=5;l++)
        if(BUT4.at({i-1,j-1,k-1,l-1}).exists()){
          EXPECT_DOUBLE_EQ(double(BUT4.at({i-1,j-1,k-1,l-1}).real()), double(BUTsT2.at({i-1,j-1,k-1,l-1}).real()));
          EXPECT_DOUBLE_EQ(double(BUT4.at({i-1,j-1,k-1,l-1}).imag()), double(BUTsT2.at({i-1,j-1,k-1,l-1}).imag()));
        }
}
...
TEST_F(BlockUniTensorTest, gpu_Mul){
    ...
    for(size_t i=1;i<=5;i++)for(size_t j=1;j<=11;j++)
      for(size_t k=1;k<=3;k++)for(size_t l=1;l<=5;l++)
        if(out.at({i-1,j-1,k-1,l-1}).exists()){
          EXPECT_DOUBLE_EQ(double(out.at({i-1,j-1,k-1,l-1}).real()), double(BUTm9.at({i-1,j-1,k-1,l-1}).real()));
          EXPECT_DOUBLE_EQ(double(out.at({i-1,j-1,k-1,l-1}).imag()), double(BUTm9.at({i-1,j-1,k-1,l-1}).imag()));
        }
    BUT4.Mul_(9);
    for(size_t i=1;i<=5;i++)for(size_t j=1;j<=11;j++)
      for(size_t k=1;k<=3;k++)for(size_t l=1;l<=5;l++)
        if(BUT4.at({i-1,j-1,k-1,l-1}).exists()){
          EXPECT_DOUBLE_EQ(double(BUT4.at({i-1,j-1,k-1,l-1}).real()), double(BUTm9.at({i-1,j-1,k-1,l-1}).real()));
          EXPECT_DOUBLE_EQ(double(BUT4.at({i-1,j-1,k-1,l-1}).imag()), double(BUTm9.at({i-1,j-1,k-1,l-1}).imag()));
        }
}
...
TEST_F(BlockUniTensorTest, gpu_Div){
    ...
    for(size_t i=1;i<=5;i++)for(size_t j=1;j<=11;j++)
      for(size_t k=1;k<=3;k++)for(size_t l=1;l<=5;l++)
        if(out.at({i-1,j-1,k-1,l-1}).exists()){
          EXPECT_DOUBLE_EQ(double(out.at({i-1,j-1,k-1,l-1}).real()), double(BUTd9.at({i-1,j-1,k-1,l-1}).real()));
          EXPECT_DOUBLE_EQ(double(out.at({i-1,j-1,k-1,l-1}).imag()), double(BUTd9.at({i-1,j-1,k-1,l-1}).imag()));
        }
    BUT4.Div_(9);
    for(size_t i=1;i<=5;i++)for(size_t j=1;j<=11;j++)
      for(size_t k=1;k<=3;k++)for(size_t l=1;l<=5;l++)
        if(BUT4.at({i-1,j-1,k-1,l-1}).exists()){
          EXPECT_DOUBLE_EQ(double(BUT4.at({i-1,j-1,k-1,l-1}).real()), double(BUTd9.at({i-1,j-1,k-1,l-1}).real()));
...
TEST_F(BlockUniTensorTest, gpu_group_basis){
...
TEST_F(BlockUniTensorTest, gpu_get_qindices){
...
TEST_F(BlockUniTensorTest, gpu_get_itoi){
...
TEST_F(BlockUniTensorTest, gpu_at_for_sparse){
