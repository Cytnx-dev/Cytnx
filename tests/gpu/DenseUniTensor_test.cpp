...
#pragma GCC diagnostic ignored "-Wdeprecated-declarations"
      ...
      TEST_F(DenseUniTensorTest, GpuRelabels) {
      ...
      TEST_F(DenseUniTensorTest, GpuRelabels2) {
...
#pragma GCC diagnostic pop
      ...
      TEST_F(DenseUniTensorTest, GpuRelabel) {
      ...
      TEST_F(DenseUniTensorTest, GpuRelabel2) {
        auto tmp = utzero3456.clone();
      ...
      TEST_F(DenseUniTensorTest, GpuNorm) {
      ...
      TEST_F(DenseUniTensorTest, GpuInv) {
        ...
        auto tmp = utarcomplex3456.to(Device.cpu);
        tmp.Inv_(clip);
        tmp.to_(Device.cuda);
        EXPECT_TRUE(AreNearlyEqUniTensor(utarcomplex3456.Inv(clip), tmp, tol));
      }

      TEST_F(DenseUniTensorTest, GpuConj) {
      ...
      TEST_F(DenseUniTensorTest, GpuTranspose) {
      ...
      TEST_F(DenseUniTensorTest, GpuDagger) {
      ...
      //   {0,1}}));
      //   // EXPECT_EQ(tmp.bonds()[1].qnums(),std::vector<std::vector<cytnx_int64>>({{0,2}, {1,5},
      //   {1,6}, {0,1}})); tmp = utarcomplex3456.truncate(1,0);
      //   // EXPECT_EQ(tmp.bonds()[0].qnums(),std::vector<std::vector<cytnx_int64>>({{0,2}, {1,5},
      //   {1,6}, {0,1}}));
      ...
      TEST_F(DenseUniTensorTest, GpuInitTagged) {
        // different types
      ...
      TEST_F(DenseUniTensorTest, GpuDtypeStr) {
      ...
      TEST_F(DenseUniTensorTest, GpuDevice) { EXPECT_EQ(Spf.device(), Device.cuda); }

      TEST_F(DenseUniTensorTest, GpuDeviceStr) {
        EXPECT_EQ(Spf.device_str().substr(0, 18), "cytnx device: CUDA");
      }

      TEST_F(DenseUniTensorTest, GpuIsBlockform) {
        EXPECT_EQ(Spf.is_blockform(), false);
        EXPECT_EQ(utzero345.is_blockform(), false);
      }

      TEST_F(DenseUniTensorTest, GpuIsContiguous) {
        EXPECT_EQ(Spf.is_contiguous(), true);
        auto Spf_new = Spf.permute({2, 1, 0}, 1);
        EXPECT_EQ(Spf_new.is_contiguous(), false);
      }

      TEST_F(DenseUniTensorTest, GpuSetRowrank) {
      ...
      TEST_F(DenseUniTensorTest, GpuAstype) {
        UniTensor Spf2d = Spf.astype(Type.Double).to(Device.cuda);
        UniTensor Spf2cf = Spf.astype(Type.ComplexFloat).to(Device.cuda);
        UniTensor Spf2cd = Spf.astype(Type.ComplexDouble).to(Device.cuda);
        EXPECT_EQ(Spf.dtype(), Type.Float);
      ...
      TEST_F(DenseUniTensorTest, GpuReshape) { EXPECT_ANY_THROW(Spf.reshape({6, 1}, 1)); }

      TEST_F(DenseUniTensorTest, GpuReshape2) { EXPECT_ANY_THROW(Spf.reshape_({6, 1}, 1)); }

      TEST_F(DenseUniTensorTest, GpuContiguous) {
      ...
      TEST_F(DenseUniTensorTest, GpuContiguous2) {
      ...
      TEST_F(DenseUniTensorTest, GpuSameData) {
      ...
      TEST_F(DenseUniTensorTest, GpuGetBlocks) {
        EXPECT_THROW(utzero345.get_blocks(), std::logic_error);
      }

      TEST_F(DenseUniTensorTest, GpuGetBlocks2) {
        EXPECT_THROW(utzero345.get_blocks_(), std::logic_error);
      }

      TEST_F(DenseUniTensorTest, GpuClone) {
      ...
      TEST_F(DenseUniTensorTest, GpuPermute1) {
        // rank-4 tensor
      ...
      TEST_F(DenseUniTensorTest, GpuPermute2) {
      ...
      TEST_F(DenseUniTensorTest, GpuPermute12) {
        // rank-4 tensor
        std::vector<cytnx_int64> a = {1, 0, 3, 2};
        auto permuted = ut4;
      ...
      TEST_F(DenseUniTensorTest, GpuPermute22) {
        std::vector<cytnx_int64> a = {1, 0};
        auto permuted = ut3;
        permuted.permute_(a, -1);
        for (size_t i = 0; i < 6; i++)
      ...
      TEST_F(DenseUniTensorTest, GpuContract1) {
      ...
      TEST_F(DenseUniTensorTest, GpuContract2) {
        ut1.relabel_({"a", "b", "c", "d"});
      ...
      TEST_F(DenseUniTensorTest, GpuContract3) {
        ut1.relabel_({"a", "b", "c", "d"});
      ...
      TEST_F(DenseUniTensorTest, GpuAdd) {
            ...
            for (size_t k = 1; k <= 5; k++)
              for (size_t l = 1; l <= 6; l++)
                if (out.at({i - 1, j - 1, k - 1, l - 1}).exists()) {
                  EXPECT_DOUBLE_EQ(
                    double(out.at({i - 1, j - 1, k - 1, l - 1}).real()),
                    double(utarcomplex3456.at({i - 1, j - 1, k - 1, l - 1}).real()) + 9);
                  EXPECT_DOUBLE_EQ(
                    double(out.at({i - 1, j - 1, k - 1, l - 1}).imag()),
      ...
      TEST_F(DenseUniTensorTest, GpuSub) {
        auto cnst = Scalar(std::complex<double>(9, 9));
      ...
      TEST_F(DenseUniTensorTest, GpuMul) {
      ...
      TEST_F(DenseUniTensorTest, GpuDiv) {
