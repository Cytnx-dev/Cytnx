...
TEST(Vstack, gpu_multi_tensor) {
  for (auto dtype : dtype_list) {
    ...
    std::vector<Tensor> Ts = {
        Tensor({3, 4}, dtype).to(cytnx::Device.cuda), 
...
TEST(Vstack, gpu_two_type_tensor) {
  for (auto dtype1 : dtype_list) {
	...
	continue;
      std::vector<Tensor> Ts = {
          Tensor({3, 4}, dtype1).to(cytnx::Device.cuda), 
...
TEST(Vstack, gpu_err_contains_void) {
  std::vector<Tensor> Ts = {
      Tensor({3, 4}, Type.Double).to(cytnx::Device.cuda), 
...
TEST(Vstack, gpu_err_contains_multi_bool_type) {
  std::vector<Tensor> Ts = {
      Tensor({3, 4}, Type.Bool).to(cytnx::Device.cuda), 
      Tensor({2, 4}, Type.Bool).to(cytnx::Device.cuda)
  };
  InitTensorUniform(Ts);
  EXPECT_THROW({
