...
namespace VstackTest {
  ...
  ====================*/
  TEST(Vstack, gpu_only_one_tensor) {
  ...
  ====================*/
  TEST(Vstack, gpu_two_type_tensor) {
    for (auto dtype1 : dtype_list) {
      for (auto dtype2 : dtype_list) {
        if (dtype1 == Type.Bool &&
            dtype2 == Type.Bool)  // if bothe are bool type, it will throw error.
          continue;
        std::vector<Tensor> Ts = {Tensor({3, 4}, dtype1).to(cytnx::Device.cuda),
                                  Tensor({2, 4}, dtype2).to(cytnx::Device.cuda)};
        InitTensorUniform(Ts);
        Tensor vstack_tens = algo::Vstack(Ts);
        CheckResult(vstack_tens, Ts);
      }
  ...
  ====================*/
  TEST(Vstack, gpu_diff_type_tensor) {
    std::vector<Tensor> Ts = {Tensor({2, 4}, Type.Bool).to(cytnx::Device.cuda),
                              Tensor({3, 4}, Type.ComplexDouble).to(cytnx::Device.cuda),
                              Tensor({2, 4}, Type.Double).to(cytnx::Device.cuda),
                              Tensor({1, 4}, Type.Double).to(cytnx::Device.cuda),
                              Tensor({5, 4}, Type.Uint64).to(cytnx::Device.cuda)};
    InitTensorUniform(Ts);
    Tensor vstack_tens = algo::Vstack(Ts);
    CheckResult(vstack_tens, Ts);
  }
  ...
  ====================*/
  TEST(Vstack, gpu_non_contiguous_tensor) {
  ...
  ====================*/
  TEST(Vstack, gpu_err_col_not_eq) {
    std::vector<Tensor> Ts = {Tensor({3, 4}, Type.Double).to(cytnx::Device.cuda),
                              Tensor({3, 2}, Type.Double).to(cytnx::Device.cuda)};
    InitTensorUniform(Ts);
    EXPECT_THROW({ Tensor tens = algo::Vstack(Ts); }, std::logic_error);
  }
  ...
  ====================*/
  TEST(Vstack, gpu_err_a_bool_type) {
  ...
  ====================*/
  TEST(Vstack, gpu_err_contains_multi_bool_type) {
    std::vector<Tensor> Ts = {Tensor({3, 4}, Type.Bool).to(cytnx::Device.cuda),
