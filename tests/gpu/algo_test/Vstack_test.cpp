...
namespace cytnx {
  namespace {
    ...
    ====================*/
    TEST(Vstack, GpuOnlyOneTensor) {
      for (auto dtype : dtype_list) {
        if (dtype == Type.Bool)  // if both bool type, it will throw error
          continue;
        std::vector<Tensor> Ts = {Tensor({3, 4}, dtype).to(Device.cuda)};
    ...
    input:Three tensor with the shapes [(3, 4), (2, 4), (5, 4)] and same data type. Test all
    possible type on cpu device.
    ====================*/
    TEST(Vstack, GpuMultiTensor) {
      for (auto dtype : dtype_list) {
        if (dtype == Type.Bool)  // if both bool type, it will throw error
          continue;
        std::vector<Tensor> Ts = {Tensor({3, 4}, dtype).to(Device.cuda),
                                  Tensor({2, 4}, dtype).to(Device.cuda),
                                  Tensor({5, 4}, dtype).to(Device.cuda)};
        InitTensorUniform(Ts);
        Tensor vstack_tens = algo::Vstack(Ts);
        CheckResult(vstack_tens, Ts);
      }
    ...
    ====================*/
    TEST(Vstack, GpuTwoTypeTensor) {
      for (auto dtype1 : dtype_list) {
        for (auto dtype2 : dtype_list) {
          if (dtype1 == Type.Bool &&
              dtype2 == Type.Bool)  // if bothe are bool type, it will throw error.
            continue;
          std::vector<Tensor> Ts = {Tensor({3, 4}, dtype1).to(Device.cuda),
                                    Tensor({2, 4}, dtype2).to(Device.cuda)};
          InitTensorUniform(Ts);
          Tensor vstack_tens = algo::Vstack(Ts);
          CheckResult(vstack_tens, Ts);
        }
    ...
    ====================*/
    TEST(Vstack, GpuDiffTypeTensor) {
      std::vector<Tensor> Ts = {Tensor({2, 4}, Type.Bool).to(Device.cuda),
                                Tensor({3, 4}, Type.ComplexDouble).to(Device.cuda),
                                Tensor({2, 4}, Type.Double).to(Device.cuda),
                                Tensor({1, 4}, Type.Double).to(Device.cuda),
                                Tensor({5, 4}, Type.Uint64).to(Device.cuda)};
      InitTensorUniform(Ts);
      Tensor vstack_tens = algo::Vstack(Ts);
      CheckResult(vstack_tens, Ts);
    }
    ...
    ====================*/
    TEST(Vstack, GpuNonContiguousTensor) {
    ...
    input:
      ...
      T2:void tensor
    ====================*/
    TEST(Vstack, GpuErrContainsVoid) {
      std::vector<Tensor> Ts = {Tensor({3, 4}, Type.Double).to(Device.cuda), Tensor()};
      InitTensorUniform(Ts);
      EXPECT_THROW({ Tensor tens = algo::Vstack(Ts); }, std::logic_error);
    }
    ...
    ====================*/
    TEST(Vstack, GpuErrColNotEq) {
      std::vector<Tensor> Ts = {Tensor({3, 4}, Type.Double).to(Device.cuda),
                                Tensor({3, 2}, Type.Double).to(Device.cuda)};
      InitTensorUniform(Ts);
      EXPECT_THROW({ Tensor tens = algo::Vstack(Ts); }, std::logic_error);
    }
    ...
    ====================*/
    TEST(Vstack, GpuErrContainsMultiBoolType) {
      std::vector<Tensor> Ts = {Tensor({3, 4}, Type.Bool).to(Device.cuda),
