...
TEST(Hstack, gpu_multi_tensor) {
  for (auto dtype : dtype_list) {
    ...
    std::vector<Tensor> Ts = {
        Tensor({4, 3}, dtype).to(cytnx::Device.cuda), 
  ...
  for (auto dtype1 : dtype_list) {
    for (auto dtype2 : dtype_list) {
      if(dtype1 == Type.Bool && dtype2 ==Type.Bool) //if both are bool type, it will throw error.
        continue;
      std::vector<Tensor> Ts = {
          Tensor({4, 3}, dtype1).to(cytnx::Device.cuda), 
...
TEST(Hstack, gpu_err_contains_void) {
  std::vector<Tensor> Ts = {
      Tensor({4, 3}, Type.Double).to(cytnx::Device.cuda), 
...
TEST(Hstack, gpu_err_multi_bool_type) {
  std::vector<Tensor> Ts = {
      Tensor({4, 3}, Type.Bool).to(cytnx::Device.cuda), 
      Tensor({4, 2}, Type.Bool).to(cytnx::Device.cuda)
  };
  InitTensorUniform(Ts);
  EXPECT_THROW({
