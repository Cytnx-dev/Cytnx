...
namespace cytnx {
  namespace gpu_test {
    namespace {
      ...
      static void CheckResult(const Tensor& hstack_tens, const std::vector<Tensor>& input_tens);
      ...
      ====================*/
      TEST(Hstack, GpuOnlyOneTensor) {
        for (auto dtype : dtype_list) {
          if (dtype == Type.Bool)  // if both bool type, it will throw error
            continue;
          std::vector<Tensor> Ts = {Tensor({4, 3}, dtype).to(Device.cuda)};
      ...
      input:Three tensor with the shapes [(4, 3), (4, 2), (4, 5)] and same data type. Test all
      possible type on cpu device.
      ====================*/
      TEST(Hstack, GpuMultiTensor) {
        for (auto dtype : dtype_list) {
          if (dtype == Type.Bool)  // if both bool type, it will throw error
            continue;
          std::vector<Tensor> Ts = {Tensor({4, 3}, dtype).to(Device.cuda),
                                    Tensor({4, 2}, dtype).to(Device.cuda),
                                    Tensor({4, 5}, dtype).to(Device.cuda)};
          InitTensorUniform(Ts);
          Tensor hstack_tens = algo::Hstack(Ts);
          CheckResult(hstack_tens, Ts);
        }
      ...
      ====================*/
      TEST(Hstack, GpuTwoTypeTensor) {
        for (auto dtype1 : dtype_list) {
          for (auto dtype2 : dtype_list) {
            if (dtype1 == Type.Bool &&
                dtype2 == Type.Bool)  // if both are bool type, it will throw error.
              continue;
            std::vector<Tensor> Ts = {Tensor({4, 3}, dtype1).to(Device.cuda),
                                      Tensor({4, 2}, dtype2).to(Device.cuda)};
            InitTensorUniform(Ts);
            Tensor hstack_tens = algo::Hstack(Ts);
            CheckResult(hstack_tens, Ts);
          }
      ...
      ====================*/
      TEST(Hstack, GpuDiffTypeTensor) {
        std::vector<Tensor> Ts = {Tensor({4, 2}, Type.Bool).to(Device.cuda),
                                  Tensor({4, 3}, Type.ComplexDouble).to(Device.cuda),
                                  Tensor({4, 2}, Type.Double).to(Device.cuda),
                                  Tensor({4, 1}, Type.Double).to(Device.cuda),
                                  Tensor({4, 5}, Type.Uint64).to(Device.cuda)};
        InitTensorUniform(Ts);
        Tensor hstack_tens = algo::Hstack(Ts);
        CheckResult(hstack_tens, Ts);
      }
      ...
      ====================*/
      TEST(Hstack, GpuNonContiguousTensor) {
        std::vector<Tensor> Ts = {
          Tensor({2, 4}, Type.Bool).to(Device.cuda),
          Tensor({3, 4}, Type.ComplexDouble).to(Device.cuda),
          Tensor({1, 4}, Type.Double).to(Device.cuda),
        };
        InitTensorUniform(Ts);
      ...
      input:
        ...
        T2:void tensor
      ====================*/
      TEST(Hstack, GpuErrContainsVoid) {
        std::vector<Tensor> Ts = {Tensor({4, 3}, Type.Double).to(Device.cuda), Tensor()};
        InitTensorUniform(Ts);
        EXPECT_THROW({ Tensor tens = algo::Hstack(Ts); }, std::logic_error);
      }
      ...
      ====================*/
      TEST(Hstack, GpuErrRowNotEq) {
        std::vector<Tensor> Ts = {Tensor({4, 3}, Type.Double).to(Device.cuda),
                                  Tensor({2, 3}, Type.Double).to(Device.cuda)};
        InitTensorUniform(Ts);
        EXPECT_THROW({ Tensor tens = algo::Hstack(Ts); }, std::logic_error);
      }
      ...
      ====================*/
      TEST(Hstack, GpuErrABoolType) {
        std::vector<Tensor> Ts = {Tensor({2, 3}, Type.Bool).to(Device.cuda)};
        InitTensorUniform(Ts);
        EXPECT_THROW({ Tensor tens = algo::Hstack(Ts); }, std::logic_error);
      }
      ...
      ====================*/
      TEST(Hstack, GpuErrMultiBoolType) {
        std::vector<Tensor> Ts = {Tensor({4, 3}, Type.Bool).to(Device.cuda),
                                  Tensor({4, 2}, Type.Bool).to(Device.cuda)};
        InitTensorUniform(Ts);
        EXPECT_THROW({ Tensor tens = algo::Hstack(Ts); }, std::logic_error);
      }
