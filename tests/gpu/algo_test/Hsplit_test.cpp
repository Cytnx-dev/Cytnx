...
input:
  ...
  dims:[7]
====================*/
TEST(Hsplit, gpu_dim_one_elem) {
  for(const auto& dtype : dtype_list) {
    if (dtype == Type.Bool) continue;
    Tensor T = Tensor({10, 7}, dtype).to(cytnx::Device.cuda);
    InitTensorUniform(T);
...
input:
  ...
  dims:[2, 3, 2]
====================*/
TEST(Hsplit, gpu_not_contiguous) {
  for(const auto& dtype : dtype_list) {
    if (dtype == Type.Bool) continue;
    Tensor T = Tensor({7, 10}, dtype).to(cytnx::Device.cuda);
    T.permute_(1, 0);
    InitTensorUniform(T);
    std::vector<cytnx_uint64> dims = {2, 3, 2};
    std::vector<Tensor> Ts = algo::Hsplit(T, dims);
...
input:
  ...
  dims:[2, 3, 2]
====================*/
TEST(Hsplit, gpu_voidAPI_output_not_empty) {
  for(const auto& dtype : dtype_list) {
    if (dtype == Type.Bool) continue;
    Tensor T = Tensor({10, 7}, dtype).to(cytnx::Device.cuda);
    InitTensorUniform(T);
...
input:
  ...
  dims:[2, 3, 2]
====================*/
TEST(Hsplit, gpu_err_input_bool_type) {
  Tensor T = Tensor({10, 7}, Type.Bool).to(cytnx::Device.cuda);
  InitTensorUniform(T);
  std::vector<cytnx_uint64> dims = {2, 3, 2};
  EXPECT_THROW({
    auto Ts = algo::Hsplit(T, dims);
  }, std::logic_error);
}
...
input:
  ...
  dims:[]
====================*/
TEST(Hsplit, gpu_err_void_tensor) {
...
input:
  ...
  dims:[2, 3, 2]
====================*/
TEST(Hsplit, gpu_err_not_mat) {
  Tensor T = Tensor({7}, Type.Bool).to(cytnx::Device.cuda);
  std::vector<cytnx_uint64> dims = {2, 3, 2};
  EXPECT_THROW({
    auto Ts = algo::Hsplit(T, dims);
  }, std::logic_error);
...
input:
  ...
  dims:[]
====================*/
TEST(Hsplit, gpu_err_dims_empty) {
  Tensor T = Tensor({10, 7}, Type.Double).to(cytnx::Device.cuda);
  InitTensorUniform(T);
  std::vector<cytnx_uint64> dims = {};
  EXPECT_THROW({
    auto Ts = algo::Hsplit(T, dims);
...
input:
  ...
  dims:[3, 4, 0]
====================*/
TEST(Hsplit, gpu_err_dims_contains_zero) {
  Tensor T = Tensor({10, 7}, Type.Double).to(cytnx::Device.cuda);
  InitTensorUniform(T);
...
input:
  ...
  dims:[3, 4, 1]
====================*/
TEST(Hsplit, gpu_err_dims_not_match) {
  Tensor T = Tensor({10, 7}, Type.Double).to(cytnx::Device.cuda);
  InitTensorUniform(T);
  std::vector<cytnx_uint64> dims = {3, 4, 0};
  EXPECT_THROW({
