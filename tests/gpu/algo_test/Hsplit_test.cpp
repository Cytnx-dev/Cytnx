  ...
  namespace {
    ...
    namespace HsplitTest {
      ...
      input:
        ...
        dims:[2, 3, 2]
      ====================*/
      TEST(Hsplit, GpuVoidAPIOutputNotEmpty) {
        for (const auto& dtype : dtype_list) {
          if (dtype == Type.Bool) continue;
          Tensor T = Tensor({10, 7}, dtype).to(Device.cuda);
          InitTensorUniform(T);
      ...
      input:
        ...
        dims:[2, 3, 2]
      ====================*/
      TEST(Hsplit, GpuErrInputBoolType) {
        Tensor T = Tensor({10, 7}, Type.Bool).to(Device.cuda);
        InitTensorUniform(T);
        std::vector<cytnx_uint64> dims = {2, 3, 2};
      ...
      input:
        ...
        dims:[]
      ====================*/
      TEST(Hsplit, GpuErrDimsEmpty) {
        Tensor T = Tensor({10, 7}, Type.Double).to(Device.cuda);
        InitTensorUniform(T);
        std::vector<cytnx_uint64> dims = {};
        EXPECT_THROW({ auto Ts = algo::Hsplit(T, dims); }, std::logic_error);
      }
      ...
      input:
        ...
        dims:[3, 4, 0]
      ====================*/
      TEST(Hsplit, GpuErrDimsContainsZero) {
        Tensor T = Tensor({10, 7}, Type.Double).to(Device.cuda);
        InitTensorUniform(T);
      ...
      input:
        ...
        dims:[3, 4, 1]
      ====================*/
      TEST(Hsplit, GpuErrDimsNotMatch) {
        Tensor T = Tensor({10, 7}, Type.Double).to(Device.cuda);
        InitTensorUniform(T);
        std::vector<cytnx_uint64> dims = {3, 4, 0};
        EXPECT_THROW({ auto Ts = algo::Hsplit(T, dims); }, std::logic_error);
