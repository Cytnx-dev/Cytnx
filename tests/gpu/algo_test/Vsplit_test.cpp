...
input:
  ...
  dims:[2, 3, 2]
====================*/
TEST(Vsplit, gpu_not_contiguous) {
  for(const auto& dtype : dtype_list) {
    if (dtype == Type.Bool) continue;
...
input:
  ...
  dims:[2, 3, 2]
====================*/
TEST(Vsplit, gpu_voidAPI_output_not_empty) {
  for(const auto& dtype : dtype_list) {
    if (dtype == Type.Bool) continue;
    Tensor T = Tensor({7, 10}, dtype).to(cytnx::Device.cuda);
    InitTensorUniform(T);
...
input:
  ...
  dims:[2, 3, 2]
====================*/
TEST(Vsplit, gpu_err_input_bool_type) {
  Tensor T = Tensor({7, 10}, Type.Bool).to(cytnx::Device.cuda);
  InitTensorUniform(T);
  std::vector<cytnx_uint64> dims = {2, 3, 2};
  EXPECT_THROW({
    auto Ts = algo::Vsplit(T, dims);
  }, std::logic_error);
}
...
input:
  ...
  dims:[]
====================*/
TEST(Vsplit, gpu_err_dims_empty) {
  Tensor T = Tensor({7, 10}, Type.Double).to(cytnx::Device.cuda);
  InitTensorUniform(T);
  std::vector<cytnx_uint64> dims = {};
  EXPECT_THROW({
    auto Ts = algo::Vsplit(T, dims);
...
input:
  ...
  dims:[3, 4, 0]
====================*/
TEST(Vsplit, gpu_err_dims_contains_zero) {
  Tensor T = Tensor({7, 10}, Type.Double).to(cytnx::Device.cuda);
  InitTensorUniform(T);
...
input:
  ...
  dims:[3, 4, 1]
====================*/
TEST(Vsplit, gpu_err_dims_not_match) {
  Tensor T = Tensor({7, 10}, Type.Double).to(cytnx::Device.cuda);
  InitTensorUniform(T);
  std::vector<cytnx_uint64> dims = {3, 4, 0};
  EXPECT_THROW({
