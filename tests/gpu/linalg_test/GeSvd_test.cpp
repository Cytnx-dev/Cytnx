...
namespace cytnx {
  namespace {
    ...
    TEST(Gesvd, GpuDenseOneElem) {
      ...
      auto T = UniTensor(bonds, labels, rowrank, Type.Double, Device.cuda, is_diag).to(Device.cuda);
    ...
    input:
      ...
      is_VT:true
    ====================*/
    TEST(Gesvd, GpuDenseNondiagTest) {
    ...
    TEST(Gesvd, GpuNoneIsUIsVT) {
      ...
      EXPECT_EQ(GesvdsUV.size(), 3)
        << "The number of the output of Gesvd not correct. Line:" << __LINE__ << std::endl;
    ...
    input:
      ...
      is_VT:true
    ====================*/
    TEST(Gesvd, GpuErrDenseDiagTest) {
      int size = 5;
    ...
    TEST(Gesvd, GpuErrSymDiagTest) {
      ...
      auto UT =
        UniTensor(bonds, labels, row_rank, Type.Double, Device.cuda, is_diag).to(Device.cuda);
    ...
    TEST(Gesvd, GpuErrBoolTypeUT) {
      ...
      std::vector<std::string> labels = {};
      auto src_T =
        UniTensor(bonds, labels, row_rank, Type.Bool, Device.cuda, false).to(Device.cuda);
    ...
    input:
      T:Void UniTensor.
    ====================*/
    TEST(Gesvd, GpuErrVoidUTenTypeUT) {
      auto Ut = UniTensor();
      EXPECT_THROW({ std::vector<UniTensor> Gesvds = linalg::Gesvd(Ut); }, std::logic_error)
    ...
    bool SingularValsCorrect(const UniTensor& res, const UniTensor& ans) {
      ...
      /*
       *  I changed relative_err tolerance of singular value in Gesvd_test from 1e-14 to 0.05,
       * since singular value may varies when using different algorithms
       */
    ...
    bool CheckResult(const std::string& case_name) {
      ...
      UniTensor ans_T = UniTensor::Load(ans_file_name).to(Device.cuda);  // sigular values UniTensor
