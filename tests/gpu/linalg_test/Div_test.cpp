...
namespace cytnx {
  namespace gpu_test {
    ...
    ::testing::AssertionResult CheckDivScalarResult(const Tensor& gpu_result, const Tensor& tensor,
    ...
    TEST(DivMixedDtypeTest, GpuScalarDivTensorMixedUnsignedSignedTypePromote) {
      ...
      EXPECT_TRUE(
        test::AreNearlyEqTensor(gpu_result_l_cpu, expected_l, GetTolerance(gpu_result_l.dtype())));
      EXPECT_TRUE(
        test::AreNearlyEqTensor(gpu_result_r_cpu, expected_r, GetTolerance(gpu_result_r.dtype())));
    }
