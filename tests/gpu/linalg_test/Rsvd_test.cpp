  ...
  namespace gpu_test {
    namespace {
      ...
      TEST(Rsvd, GpuDenseOneElem) {
        ...
        auto T = UniTensor(bonds, labels, rowrank, Type.Double, Device.cuda, is_diag);
      ...
      TEST(Rsvd, GpuDenseLowRankRectangularAndTransposedExactReconstruction) {
      ...
      input:
        ...
        is_VT:true
      ====================*/
      TEST(Rsvd, GpuDenseExpSvalsTest) {
#ifndef UNI_CUQUANTUM
        GTEST_SKIP() << "GPU randomized SVD dense tests require cuQuantum-enabled GPU QR support.";
#endif
      ...
      ====================*/
      TEST(Rsvd, GpuDenseExpSvalsNoPowerIterationTest) {
#ifndef UNI_CUQUANTUM
        GTEST_SKIP() << "GPU randomized SVD dense tests require cuQuantum-enabled GPU QR support.";
#endif
        std::vector<std::string> case_list = {"dense_nondiag_exp_Svals_C128",
      ...
      input:
        ...
        is_VT:true
      ====================*/
      TEST(Rsvd, GpuU1ExpSvalsTest) {
        std::vector<std::string> case_list = {"sym_UT_U1_exp_Svals_C128",
                                              "sym_UT_U1_exp_Svals_F64"};
        for (const auto& case_name : case_list) {
          std::string test_case_name =
            ::testing::UnitTest::GetInstance()->current_test_info()->name();
          fail_msg.Init(test_case_name + ", " + case_name);
      ...
      ====================*/
      TEST(Rsvd, GpuU1ExpSvalsMinblockdimTest) {
        std::vector<std::string> case_list = {"sym_UT_U1_exp_Svals_C128",
                                              "sym_UT_U1_exp_Svals_F64"};
        for (const auto& case_name : case_list) {
          std::string test_case_name =
      ...
      input:
        ...
        is_VT:true
      ====================*/
      TEST(Rsvd, GpuBlockFermionicTest) {
        std::string test_case_name =
          ::testing::UnitTest::GetInstance()->current_test_info()->name();
        fail_msg.Init(test_case_name);
      ...
      static bool CheckResult(const std::string& case_name, const cytnx_uint64& keepdim,
        ...
        std::vector<UniTensor> rsvds = linalg::Rsvd(src_T, keepdim, min_blockdim, 0., true, true, 0,
                                                    0, 2, 1, power_iteration, 0);
      ...
      // (Originally a separate Rsvd_notruncate test suite.)
      // ============================================================================
      ...
      input:
        ...
        is_VT:true
      ====================*/
      TEST(RsvdNoTruncation, GpuDenseOneElem) {
        std::string test_case_name =
          ::testing::UnitTest::GetInstance()->current_test_info()->name();
      ...
      input:
        ...
        is_VT:true
      ====================*/
      TEST(RsvdNoTruncation, GpuDenseNondiagCompareGesvd) {
#ifndef UNI_CUQUANTUM
        GTEST_SKIP() << "GPU randomized SVD dense tests require cuQuantum-enabled GPU QR support.";
#endif
        ...
        for (const auto& case_name : case_list) {
          ...
          UniTensor src_T =
            UniTensor::Load(no_trunc_src_data_root + case_name + ".cytnx").to(Device.cuda);
          std::vector<UniTensor> rsvd = linalg::Rsvd(src_T, 2, 0., true, true, 0, 1, 0, 0., 3, 0);
          std::vector<UniTensor> gesvd = linalg::Gesvd(src_T, true, true);

          ASSERT_TRUE(NoTruncCheckLabels(src_T, rsvd)) << no_trunc_fail_msg.TraceFailMsgs();
          ...
          EXPECT_TRUE(matches_truncated_gesvd) << no_trunc_fail_msg.TraceFailMsgs();
        }
      }
      ...
      input:
        ...
        is_VT:true
      ====================*/
      TEST(RsvdNoTruncation, GpuErrDenseDiagTest) {
        int size = 5;
        std::vector<Bond> bonds = {Bond(size), Bond(size)};
        int rowrank = 1;
        bool is_diag = true;
      ...
      ====================*/
      TEST(RsvdNoTruncation, GpuErrSymDiagTest) {
      ...
      ====================*/
      TEST(RsvdNoTruncation, GpuErrRank1Unitensor) {
      ...
      input:
        ...
        is_VT:true
      ====================*/
      TEST(RsvdNoTruncation, GpuDenseExpSvalsTest) {
#ifndef UNI_CUQUANTUM
        GTEST_SKIP() << "GPU randomized SVD dense tests require cuQuantum-enabled GPU QR support.";
#endif
        std::vector<std::string> case_list = {"dense_nondiag_exp_Svals_C128",
      ...
      is_VT:true
      ====================*/
      TEST(RsvdNoTruncation, GpuDenseExpSvalsNoPowerIterationTest) {
#ifndef UNI_CUQUANTUM
        GTEST_SKIP() << "GPU randomized SVD dense tests require cuQuantum-enabled GPU QR support.";
#endif
        std::vector<std::string> case_list = {"dense_nondiag_exp_Svals_C128",
      ...
      input:
        ...
        is_VT:true
      ====================*/
      TEST(RsvdNoTruncation, GpuDenseTaggedOutputBondTypes) {
      ...
      ====================*/
      TEST(RsvdNoTruncation, GpuBlockU1CompareGesvd) {
        ...
        for (const auto& case_name : case_list) {
          ...
          UniTensor src_T =
            UniTensor::Load(no_trunc_src_data_root + case_name + ".cytnx").to(Device.cuda);
          EXPECT_TRUE(NoTruncCheckAgainstGesvd(src_T, 1000, 2))
            << no_trunc_fail_msg.TraceFailMsgs();
        }
      }
      ...
      ====================*/
      TEST(RsvdNoTruncation, GpuBlockU1xz2CompareGesvd) {
      ...
      ====================*/
      TEST(RsvdNoTruncation, GpuBlockFermionicCompareGesvd) {
        std::string test_case_name =
          ::testing::UnitTest::GetInstance()->current_test_info()->name();
      ...
      ====================*/
      TEST(RsvdNoTruncation, GpuBlockAndFermionicOutputSize) {
      ...
      // Note: the original Rsvd_notruncate suite had block_combined_sector_* tests covering per-
      // sector truncation patterns produced by the un-truncated randomized SVD. Those tests are
      // not portable to Rsvd because Rsvds outer global truncation overrides the per-sector dims
      // the original tests asserted on. The per-sector sketch correctness is still covered by the
      // block_*_compare_gesvd tests above.
      ...
      static bool NoTruncCheckAgainstGesvd(const UniTensor& src_T, const cytnx_uint64& keepdim,
                                           const cytnx_uint64& power_iteration, bool is_U,
                                           bool is_vT, cytnx_uint64 mindim,
        ...
        if (!NoTruncSingularValsCorrect(rsvd[0], gesvd[0])) {
          ...
          no_trunc_fail_msg.AppendMsg("The singular values differ from Gesvd. ", __func__,
                                      __LINE__);
          return false;
        }
