...
namespace cytnx {
  namespace gpu_test {
    namespace {
      ...
      void ApplyInplace(int op, Tensor& lhs, const Tensor& rhs) {
        switch (op) {
          case 0:
            linalg::iAdd(lhs, rhs);
            break;
      ...
      TEST(InplacePromoteTest, GpuInplacePromotesLhsDtype) {
        for (int op = 0; op < 4; ++op) {
          for (auto ldtype : dtype_list) {
            if (ldtype == Type.Bool) continue;
            for (auto rdtype : dtype_list) {
              ...
              EXPECT_TRUE(AreNearlyEqTensor(gpu_l.to(Device.cpu), cpu_l, 1e-5));
            }
          }
        }
      }
      ...
      TEST(InplacePromoteTest, GpuInplaceNoncontiguousTensorTensor) {
        for (int op = 0; op < 4; ++op) {
      ...
      TEST(InplacePromoteTest, GpuInplaceLiteralExpected) {
      ...
      TEST(InplacePromoteTest, GpuInplaceRealOpComplexWeakScalarRejected) {
        for (int op = 0; op < 4; ++op) {
