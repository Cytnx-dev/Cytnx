#ifndef CYTNX_TESTS_GPU_LINALG_TEST_LINALG_TEST_H_
#define CYTNX_TESTS_GPU_LINALG_TEST_LINALG_TEST_H_
...
namespace cytnx {
  namespace test {
    ...
    class linalg_Test : public ::testing::Test {
     public:
      // ==================== general ===================
      Tensor arange3x3d = arange(0, 9, 1, Type.Double, Device.cuda).reshape(3, 3);
      Tensor ones3x3d = ones({3, 3}, Type.Double, Device.cuda);
      Tensor eye3x3d = eye(3, Type.Double, Device.cuda);
      Tensor zeros3x3d = zeros({3, 3}, Type.Double, Device.cuda);

      Tensor arange3x3cd = arange(0, 9, 1, Type.ComplexDouble, Device.cuda).reshape(3, 3) +
                           1.0i * arange(0, 9, 1, Type.ComplexDouble, Device.cuda).reshape(3, 3);
      Tensor ones3x3cd = ones({3, 3}, Type.ComplexDouble, Device.cuda);
      Tensor eye3x3cd = eye(3, Type.ComplexDouble, Device.cuda);
      Tensor zeros3x3cd = zeros({3, 3}, Type.ComplexDouble, Device.cuda);
      ...
      Tensor svd_Sans;
      //==================== Lanczos_Gnd_Ut ===================
      Tensor A = Tensor::Load(data_dir + "Lanczos_Gnd/lan_block_A.cytn").to(Device.cuda);
      Tensor B = Tensor::Load(data_dir + "Lanczos_Gnd/lan_block_B.cytn").to(Device.cuda);
      Tensor C = Tensor::Load(data_dir + "Lanczos_Gnd/lan_block_C.cytn").to(Device.cuda);
      Bond lan_I = Bond(BD_IN, {Qs(-1), Qs(0), Qs(1)}, {9, 9, 9});
      Bond lan_J = Bond(BD_OUT, {Qs(-1), Qs(0), Qs(1)}, {9, 9, 9});
      UniTensor H = UniTensor({lan_I, lan_J}).to(Device.cuda);

      //==================== QR ===================
      Tensor Qr_Qans = Tensor::Load(data_dir + "Qr/qr_Qans.cytn").to(Device.cuda);
      Tensor Qr_Rans = Tensor::Load(data_dir + "Qr/qr_Rans.cytn").to(Device.cuda);

      //==================== ExpH ===================
      Tensor expH_ans = Tensor::Load(data_dir + "expH/expH_ans.cytn").to(Device.cuda);

      //==================== Pow ===================
      Tensor Pow_ans = Tensor::Load(data_dir + "Pow/Pow_ans.cytn").to(Device.cuda);

      //==================== Mod ===================
      Tensor Mod_ans = Tensor::Load(data_dir + "Mod/Mod_ans.cytn").to(Device.cuda);
      Tensor ModUtUt_ans = Tensor::Load(data_dir + "Mod/ModUtUt_ans.cytn").to(Device.cuda);

     protected:
      void SetUp() override {
        //================ svd truncate =======================

        svd_T = svd_T.Load(data_dir + "Svd_truncate/Svd_truncate1.cytnx").to(Device.cuda);
        svd_T.permute_({1, 0, 3, 2});
        svd_T.contiguous_();
        svd_T.set_rowrank_(2);
        svd_Sans = Tensor::Load(data_dir + "Svd_truncate/S_truncate1.cytn").to(Device.cuda);
    ...
    inline void expect_same(const std::vector<double> &a, const std::vector<double> &b,
                            double tol) {
    ...
    // (aux first). The dagger operand is followed by fermion_twists() per the fermionic
    // contraction.
    ...
    // Op psi = A^dag (A psi). Returns the raw contraction result: the solver's matvec wrapper
    // resolves the signflip frame and makes it contiguous, and psi is built (below) in this
    // contraction's leg order so no permute is needed (a permute would swap fermionic legs and
    // wrongly negate the odd-odd sector).
    ...
    // sign-flip-active initial vector (built in the opposite leg order then permuted into the
    // matvec's output order, which injects the pending signflip).
    ...
    // |<v|w>| / sqrt(<v|v><w|w>): phase-invariant. |<v|w>| is the magnitude (Norm()) of the
    // fermionic overlap (valid for complex eigenvectors); Norm() is signflip-independent so no
    // apply() needed.
    ...
    // kmax lowest eigenpairs of O = A^dag A in the charge-0 ket sector by explicit dense Eigh
    // (CPU).
    ...
    inline void expect_lowest_states(const UniTensor &A, const std::vector<UniTensor> &eigs,
      ...
      for (cytnx_uint64 i = 0; i < k; i++) {
        ...
        // A is promoted to the eigenvector dtype here (only Arnoldi's complex eigenvector needs
        // it).
