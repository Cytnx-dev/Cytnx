...
namespace cytnx {
  namespace gpu_test {
    namespace {
      ...
      ::testing::AssertionResult CheckModScalarResult(const Tensor& gpu_result,
                                                      const Tensor& tensor,
      ...
      TEST_P(ModTestAllShapes, GpuTensorModTensorAllTypes) {
        ...
        for (auto dtype : supported_types) {
          SCOPED_TRACE("Testing Mod(tensor, tensor) with shape: " +
      ...
      TEST_P(ModTestAllShapes, GpuScalarModTensorAllTypes) {
        ...
        for (auto dtype : supported_types) {
          SCOPED_TRACE("Testing Mod(scalar, tensor) with shape: " +
                       ::testing::PrintToString(shape) + " and dtype: " + std::to_string(dtype));
      ...
      TEST_P(ModTestAllShapes, GpuTensorModScalarAllTypes) {
        ...
        for (auto dtype : supported_types) {
          SCOPED_TRACE("Testing Mod(tensor, scalar) with shape: " +
                       ::testing::PrintToString(shape) + " and dtype: " + std::to_string(dtype));
