...
namespace cytnx {
  namespace gpu_test {
    namespace {
      ...
      static Tensor ContiguousCpuReferenceTrace(const Tensor& gpu_t, cytnx_uint64 a,
                                                cytnx_uint64 b) {
        return linalg::Trace(gpu_t.to(Device.cpu).contiguous(), a, b);
      }

      static Tensor ZeroExtentGpuTensor(const std::vector<cytnx_uint64>& shape,
                                        unsigned int dtype) {
      ...
      TEST(LinalgGpuTraceTest, GpuPermutedTracesAcrossRanksAndDtypes) {
        ...
        for (unsigned int dtype : {Type.Double, Type.ComplexDouble, Type.Int32}) {
          for (const auto& c : cases) {
            auto t =
              random::random_tensor(c.shape, -2.0, 2.0, Device.cpu, 0, dtype).to(Device.cuda);
      ...
      TEST(LinalgGpuTraceTest, GpuRank2Path) {
        ...
        auto t =
          random::random_tensor({6, 6}, -1.0, 1.0, Device.cpu, 0, Type.Double).to(Device.cuda);
      ...
      TEST(LinalgGpuTraceTest, GpuBlockSizeBoundaries) {
        ...
        for (cytnx_uint64 diagonal_length : {1u, 31u, 32u, 33u, 64u, 255u, 256u, 257u, 1024u}) {
          auto cpu = random::random_tensor({diagonal_length, diagonal_length}, -2.0, 2.0,
