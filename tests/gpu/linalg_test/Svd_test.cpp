...
TEST(Svd, gpu_dense_one_elem) {
  ...
  auto T = UniTensor(bonds, labels, rowrank, cytnx::Type.Double,
                     cytnx::Device.cpu, is_diag).to(cytnx::Device.cuda);
...
TEST(Svd, gpu_err_sym_diag_test) {
  ...
  auto UT = UniTensor(bonds, labels, row_rank, Type.Double, Device.cpu, is_diag).to(cytnx::Device.cuda);
...
TEST(Svd, gpu_err_bool_type_UT) {
  ...
  cytnx_int64 row_rank = -1;
  std::vector<std::string> labels = {};
  auto src_T = UniTensor(bonds, labels, row_rank, Type.Bool, Device.cpu, false).to(cytnx::Device.cuda);
