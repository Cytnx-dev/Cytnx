...
namespace cytnx {
  namespace gpu_test {
    namespace {
      ...
      class GpuArnoldiMatOp : public LinOp {
       public:
        Tensor opMat;
        Tensor T_init;
        GpuArnoldiMatOp(const cytnx_uint64& nx, const int& dtype, const int& in_device);
        Tensor matvec(const Tensor& v) override { return (linalg::Dot(opMat, v)); }
        void InitVec();
        friend class CheckOp;
      };
      GpuArnoldiMatOp::GpuArnoldiMatOp(const cytnx_uint64& in_nx, const int& in_dtype,
                                       const int& in_device)
      ...
      static bool CheckResult(CheckOp& H, const std::vector<Tensor>& arnoldi_eigs_cuda,
      ...
      static void ExcuteTest(const std::string& which, const int& mat_type = Type.ComplexDouble,
        ...
        GpuArnoldiMatOp H = GpuArnoldiMatOp(dim, mat_type, Device.cuda);
      ...
      TEST(Arnoldi, GpuWhichLMTest) {
        std::string which = "LM";
        ExcuteTest(which);
      }

      // 1-2, test for 'which' = 'LR'
      TEST(Arnoldi, GpuWhichLRTest) {
        std::string which = "LR";
        ExcuteTest(which);
      }

      // 1-3, test for 'which' = 'LI'
      TEST(Arnoldi, GpuWhichLITest) {
        std::string which = "LI";
        ExcuteTest(which);
      }

      // 1-4, test for 'which' = 'SR'
      TEST(Arnoldi, GpuWhichSRTest) {
        std::string which = "SR";
        ExcuteTest(which);
      }

      // 1-5, test for 'which' = 'SI'
      TEST(Arnoldi, GpuWhichSITest) {
        std::string which = "SI";
        ExcuteTest(which);
      }

      // 1-6, test matrix is all type
      TEST(Arnoldi, GpuMatTypeAllTest) {
        std::string which = "LM";
