  ...
  namespace gpu_test {
    namespace {
      ...
      */
      ...
      //   UniTensor cyT = UniTensor({I, J, K, L}, {"a", "b", "c", "d"}, 2, Type.Double,
      //   Device.cuda, false)
      //                     .to(Device.cuda);
      ...
      // GPU outputs reconstruct the input and are isometric). Gauge-free results (ExpH) are
      // compared element-wise CPU-vs-GPU.
      // ====================================================================================
      ...
      TEST_F(linalg_Test, GpuBkFUtGesvdTruncate) {
        ...
        expect_same(sorted_diagonal(gesvd_cpu[0]), sorted_diagonal(gesvd_gpu[0].to(Device.cpu)),
                    tol);
      ...
      // NOTE: no ExpM GPU test -- ExpM uses the general (non-symmetric) Eig internally, which has
      // no GPU backend ("Eig for non-symmetric matrix is not supported"); only ExpH (via the
      // Hermitian Eigh / cuEigh) runs on the GPU. ExpM is covered by the CPU test.
