  ...
  namespace gpu_test {
    namespace {
      ...
      TEST(ZeroExtentGpuLinalgTest, GpuElementwiseAndReductionsDoNotLaunchZeroBlockKernels) {
        ...
        ExpectGpuEmpty(linalg::Abs(Tensor(shape, Type.ComplexFloat, Device.cuda)), shape,
                       Type.Float);
        ...
        ExpectGpuEmpty(linalg::Pow(Tensor(shape, Type.Int64, Device.cuda), 2.0), shape,
                       Type.Double);
      ...
      TEST(ZeroExtentGpuLinalgTest, GpuProductsUseEmptyAndZeroInnerDimensionSemantics) {
        ...
        Tensor contracted =
          linalg::Tensordot(Tensor({2, 0, 3}, Type.Double, Device.cuda),
