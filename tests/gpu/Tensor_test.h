...
class TensorTest : public ::testing::Test {
 ...
 protected:
  void SetUp() override {
    tzero345 = zeros(3 * 4 * 5).reshape({3, 4, 5}).astype(Type.ComplexDouble).to(cytnx::Device.cuda);
    ...
    tzero3456 = zeros(3 * 4 * 5 * 6).reshape({3, 4, 5, 6}).astype(Type.ComplexDouble).to(cytnx::Device.cuda);
    tone3456 = ones(3 * 4 * 5 * 6).reshape({3, 4, 5, 6}).astype(Type.ComplexDouble).to(cytnx::Device.cuda);
    tar3456 = arange(3*4*5*6).reshape({3, 4, 5, 6}).astype(Type.ComplexDouble).to(cytnx::Device.cuda);
    ...
    tarcomplex345 = tarcomplex345.reshape({3, 4, 5}).astype(Type.ComplexDouble).to(cytnx::Device.cuda);
    ...
    tarcomplex3456 = tarcomplex3456.reshape({3, 4, 5, 6}).astype(Type.ComplexDouble).to(cytnx::Device.cuda);
