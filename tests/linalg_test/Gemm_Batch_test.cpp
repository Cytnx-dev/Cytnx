...
namespace cytnx {
  namespace test {
    namespace {
      ...
      static Tensor RunSingle(const Scalar& alpha, const Scalar& beta, Tensor A, Tensor B,
                              Tensor C) {
      ...
      static void ExpectNearEl(double got, double expected, double rel, double abs_floor,
      ...
      TEST_P(GemmBatchScalarTest, DoubleMatrices) {
        ...
        for (int i = 0; i < m; i++)
          for (int j = 0; j < n; j++)
            ExpectNearEl(C.at<double>({(cytnx_uint64)i, (cytnx_uint64)j}), ref[i * n + j], 1e-10,
                         1e-12, "(" + std::to_string(i) + "," + std::to_string(j) + ")");
      }
      ...
      TEST_P(GemmBatchDimTest, Dim1x1x1ScalarDegenerate) {
        ...
        EXPECT_NEAR(res.at<cytnx_complex128>({(cytnx_uint64)0, (cytnx_uint64)0}).real(), 82.0,
                    p.tol);
        EXPECT_NEAR(res.at<cytnx_complex128>({(cytnx_uint64)0, (cytnx_uint64)0}).imag(), 0.0,
                    p.tol);
      }
      ...
      TEST_P(GemmBatchBatchTest, BatchOneGroupThreeMatrices) {
        ...
        for (int idx = 0; idx < 3; idx++) {
          auto ref =
            RefMatMul(m, n, k, alphas_v[idx], betas_v[idx], *as_ref[idx], ball, cs_ref[idx]);
      ...
      TEST_P(GemmBatchBatchTest, BatchTwoGroupsDifferentDims) {
        ...
        for (int i = 0; i < m1; i++)
          for (int j = 0; j < n1; j++) {
            ...
            EXPECT_NEAR(got.real(), (i == j ? 1.0 : 0.0), p.tol)
              << "group1 (" << i << "," << j << ")";
            EXPECT_NEAR(got.imag(), 0.0, p.tol);
          }
      }
