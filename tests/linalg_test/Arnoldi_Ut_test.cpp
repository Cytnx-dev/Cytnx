...
namespace cytnx {
  namespace test {
    namespace {
      ...
      static std::vector<Scalar> OrderEigvals(const Tensor& eigvals,
      ...
      // get resigue |Hv - ev|
      static Scalar GetResidue(TMOp& H, const Scalar& eigval, const UniTensor& eigvec) {
      ...
      static bool CheckResult(TMOp& H, const std::vector<UniTensor>& arnoldi_eigs,
      ...
      TEST(ArnoldiUt, WhichLMTest) {
        std::string which = "LM";
        ExcuteTest(which);
      }

      // 1-2, test for 'which' = 'LR'
      TEST(ArnoldiUt, WhichLRTest) {
        std::string which = "LR";
        ExcuteTest(which);
      }

      // 1-3, test for 'which' = 'LI'
      TEST(ArnoldiUt, WhichLITest) {
        std::string which = "LI";
        ExcuteTest(which);
      }

      // 1-4, test for 'which' = 'SR'
      TEST(ArnoldiUt, WhichSRTest) {
        std::string which = "SR";
        ExcuteTest(which);
      }

      // 1-5, test for 'which' = 'SI'
      TEST(ArnoldiUt, WhichSITest) {
        std::string which = "SI";
        ExcuteTest(which);
      }

      // 1-6, test matrix is all allowed data type
      TEST(ArnoldiUt, AllDtypeTest) {
        std::string which = "LM";
      ...
      void ArnoldiUtErrorTestClass::ExcuteErrorTest() {
        try {
      ...
      TEST(ArnoldiUt, ErrWhich) {
        ArnoldiUtErrorTestClass err_task1;
        err_task1.which = "ML";
        err_task1.ExcuteErrorTest();
      }

      // 2-2, test SM is not support for UniTensor
      TEST(ArnoldiUt, ErrWhichSM) {
        ArnoldiUtErrorTestClass err_task;
        err_task.which = "SM";
        err_task.ExcuteErrorTest();
      }

      // 2-3, test for wrong input LinOp dtype
      TEST(ArnoldiUt, ErrMatType) {
        ArnoldiUtErrorTestClass err_task;
      ...
      // 2-6, test cvg_crit <= 0
      TEST(ArnoldiUt, ErrCritNegative) {
        ArnoldiUtErrorTestClass err_task;
      ...
      TEST(ArnoldiUt, ErrNcvOutOfRange) {
        ArnoldiUtErrorTestClass err_task;
      ...
      /*=====test info=====
      describe:Arnoldi ('SR') for a fermionic LinOp Op=A^dag A introducing sign flips; solving for
      the TWO lowest states (k=2). Tests the fermionic ARPACK matvec and the BlockFermionicUniTensor
      returns. Checks both eigenvalues against the dense reference, that both eigenvectors are
      normalized and match the dense eigenvectors (|<v|w>| ~ 1), and that the two eigenvectors are
      mutually orthogonal. (ARPACK's real driver packs the eigenvector as complex with zero
      imaginary part for a real eigenvalue; ferm_fidelity uses the magnitude |<v|w>| so it handles
      that.)
      ====================*/
      TEST(ArnoldiUt, FermionicArnoldiFermionic) {
