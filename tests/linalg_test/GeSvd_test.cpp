...
namespace GesvdTest {
  ...
  input:
    ...
    is_VT:true
  ====================*/
  TEST(Gesvd, err_dense_diag_test) {
    int size = 5;
  ...
  input:
    T:Void UniTensor.
  ====================*/
  TEST(Gesvd, err_Void_UTenType_UT) {
    auto Ut = UniTensor();
    EXPECT_THROW({ std::vector<UniTensor> Gesvds = linalg::Gesvd(Ut); }, std::logic_error)
