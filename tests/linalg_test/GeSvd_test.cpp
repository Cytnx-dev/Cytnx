  ...
  namespace {
    ...
    namespace GesvdTest {
      ...
      TEST(Gesvd, DenseOneElem) {
        ...
        EXPECT_EQ(Gesvds[0].at<double>({0}), std::abs(T.at<double>({0, 0, 0})))
      ...
      ====================*/
      TEST(Gesvd, U1SymRegressionMultiSectorBdOutLeft) {
        fail_msg.Init(::testing::UnitTest::GetInstance()->current_test_info()->name());
        ...
        EXPECT_EQ(gesvds[1].bonds().back().getDegeneracies(),
                  gesvds[0].bonds()[0].getDegeneracies());
        EXPECT_EQ(gesvds[2].bonds().front().getDegeneracies(),
                  gesvds[0].bonds()[0].getDegeneracies());
      }
      ...
      TEST(Gesvd, ErrRowrankEqualRankDenseUT) {
