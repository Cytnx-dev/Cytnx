  ...
  namespace test {
    namespace {
      ...
      static void ExcuteTest(const std::string& which, const int& mat_type = Type.Double,
        ...
        std::vector<Tensor> lanczos_eigs =
      ...
      introducing sign flips; solving for the TWO lowest states (k=2). Checks both eigenvalues
      against the dense reference, that both eigenvectors are normalized and match the dense
      eigenvectors
