...
bool CheckResult(MatOp& H, const std::vector<Tensor>& arnoldi_eigs, 
		 const std::string& which, const cytnx_uint64 k);

void ExcuteTest(const std::string& which, 
		const int &mat_type = Type.ComplexDouble, 
		const cytnx_uint64& k = 5,
		cytnx_uint64 dim = 23) { 
  MatOp H = MatOp(dim, mat_type);
  const cytnx_uint64 maxiter = 10000;
  const cytnx_double cvg_crit = tolerance;
  std::vector<Tensor> arnoldi_eigs = 
...
bool cmpNorm(const Scalar& l, const Scalar& r) {
  return abs(l) < abs(r);
}
bool cmpReal(const Scalar& l, const Scalar& r) {
  return l.real() < r.real();
}
bool cmpImag(const Scalar& l, const Scalar& r) {
  return l.imag() < r.imag();
}
std::vector<Scalar> OrderEigvals(const Tensor &eigvals, 
  ...
  for(cytnx_uint64 i = 0; i < eigvals_len; ++i)
    ordered_eigvals[i] = eigvals.storage().at(i);
...
//get resigue |Hv - ev|
Scalar GetResidue(MatOp& H, const Scalar& eigval, 
      	           const Tensor& eigvec) {
