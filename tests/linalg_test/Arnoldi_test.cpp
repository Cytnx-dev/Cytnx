...
namespace cytnx {
  namespace test {
    namespace {
      ...
      TEST(Arnoldi, WhichLMTest) {
        std::string which = "LM";
        ExcuteTest(which);
      }

      // 1-2, test for 'which' = 'LR'
      TEST(Arnoldi, WhichLRTest) {
        std::string which = "LR";
        ExcuteTest(which);
      }

      // 1-3, test for 'which' = 'LI'
      TEST(Arnoldi, WhichLITest) {
        std::string which = "LI";
        ExcuteTest(which);
      }

      // 1-4, test for 'which' = 'SR'
      TEST(Arnoldi, WhichSRTest) {
        std::string which = "SR";
        ExcuteTest(which);
      }

      // 1-5, test for 'which' = 'SI'
      TEST(Arnoldi, WhichSITest) {
        std::string which = "SI";
        ExcuteTest(which);
      }

      // 1-6, test matrix is all type
      TEST(Arnoldi, MatTypeAllTest) {
        std::string which = "LM";
