...
namespace cytnx {
  namespace test {
    namespace {
      ...
      class LanczosGndOneSiteOp : public LinOp {
       public:
        LanczosGndOneSiteOp(const int d = 2, const int D = 5, const int dw = 3,
      ...
      static bool CheckResult(LanczosGndOneSiteOp& H, const std::vector<UniTensor>& lanczos_eigs,
      ...
      static void ExcuteTest(const std::string& which, const int& mat_type = Type.Double,
        ...
        int D = 5, d = 2, dw = 3;
        LanczosGndOneSiteOp op = LanczosGndOneSiteOp(d, D, dw, mat_type);
      ...
      // error test
      class LanczosGndErrorTestClass {
       public:
        std::string which = "LM";
        cytnx_uint64 k = 1;
        ...
        bool is_V = true;
        cytnx_int32 ncv = 0;
        LanczosGndErrorTestClass(){};
        void ExcuteErrorTest();
        // set
        void Set_mat_type(const int _mat_type) {
          mat_type = _mat_type;
          op = LanczosGndOneSiteOp(d, D, dw, mat_type);
        }
      ...
      void LanczosGndErrorTestClass::ExcuteErrorTest() {
        try {
          auto lanczos_eigs =
      ...
      static std::vector<Scalar> OrderEigvals(const UniTensor& eigvals,
      ...
      // get resigue |Hv - ev|
      static Scalar GetResidue(LanczosGndOneSiteOp& H, const Scalar& eigval,
                               const UniTensor& eigvec) {
      ...
      TEST(LanczosUt, WhichLMTest) {
        std::string which = "LM";
        ExcuteTest(which);
      }

      // 1-2, test for 'which' = 'LA'
      TEST(LanczosUt, WhichLATest) {
        std::string which = "LA";
        ExcuteTest(which);
      }

      // 1-3, test for 'which' = 'SA'
      TEST(LanczosUt, WhichSATest) {
        std::string which = "SA";
        ExcuteTest(which);
      }

      // 1-4, test matrix is all allowed data type
      TEST(LanczosUt, AllDtypeTest) {
        std::string which = "LM";
      ...
      TEST(LanczosUt, IsVFalse) {
        LanczosGndOneSiteOp op = LanczosGndOneSiteOp();
        const cytnx_uint64 maxiter = 1000;
        const cytnx_double cvg_crit = 0;
      ...
      TEST(LanczosUt, ErrWhich) {
        LanczosGndErrorTestClass err_task1;
        err_task1.which = "ML";
        err_task1.ExcuteErrorTest();
      }

      // 2-2, test SM is not support for UniTensor
      TEST(LanczosUt, ErrWhichSM) {
        LanczosGndErrorTestClass err_task;
        err_task.which = "SM";
        err_task.ExcuteErrorTest();
      }

      // 2-3, test for wrong input LinOp dtype
      TEST(LanczosUt, ErrMatType) {
        LanczosGndErrorTestClass err_task;
      ...
      TEST(LanczosUt, ErrKTooLarge) {}

      // 2-6, test cvg_crit <= 0
      TEST(LanczosUt, ErrCritNegative) {
        LanczosGndErrorTestClass err_task;
      ...
      TEST(LanczosUt, ErrNcvOutOfRange) {
        LanczosGndErrorTestClass err_task;
      ...
      TEST(LanczosGnd, LanczosGndTest) {
        // CompareWithScipy
      ...
      TEST(LanczosGnd, BkLanczosGndTest) {
        // CompareWithScipy
        ...
        std::vector<UniTensor> eigs = linalg::Lanczos(
          &H, lan_guess, "Gnd", 9.999999999999999988e-15, 10000, 1, true, true, 0, false);
        cytnx_double ev = (cytnx_double)eigs[0].get_block_().item().real();
      ...
      /*=====test info=====
      describe:Lanczos ('Gnd', Lanczos_Gnd_Ut) for the fermionic LinOp Op=A^dag A which introduces
      sign flips, and with an initial vector containing sign flips. Checks the eigenvalue against
      and independent dense ground state run (and against the squre of the singular values of A),
      the eigenvector residual ||Op v - E v|| ~ 0, and the eigenvector fidelity
      |<v|w>|/sqrt(<v|v><w|w>) ~ 1 vs the dense diagonalization.
      ====================*/
      TEST(LanczosGnd, FermionicLanczosFermionic) {
