...
namespace cytnx {
  namespace test {
    ...
    TEST(LanczosGnd, BkLanczosGndTest) {
      ...
      std::vector<UniTensor> eigs = linalg::Lanczos(&H, lan_guess, "Gnd", 9.999999999999999988e-15,
                                                    10000, 1, true, true, 0, false);
      cytnx_double ev = (cytnx_double)eigs[0].get_block_().item().real();
    ...
    sign flips, and with an initial vector containing sign flips. Checks the eigenvalue against and
    independent dense ground state run (and against the squre of the singular values of A), the
    eigenvector residual ||Op v - E v|| ~ 0, and the eigenvector fidelity |<v|w>|/sqrt(<v|v><w|w>) ~
    1 vs the dense diagonalization.
    ====================*/
