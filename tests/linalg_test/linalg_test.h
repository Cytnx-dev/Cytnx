#ifndef CYTNX_TESTS_LINALG_TEST_LINALG_TEST_H_
#define CYTNX_TESTS_LINALG_TEST_LINALG_TEST_H_
  ...
  namespace test {
    ...
    class linalg_Test : public ::testing::Test {
     public:
      ...
      Bond svd_J = Bond(BD_IN, {Qs(5), Qs(3), Qs(1), Qs(-1), Qs(-3), Qs(-5), Qs(-7)},
                        {6, 22, 57, 68, 38, 8, 1});
      Bond svd_K = Bond(BD_OUT, {Qs(5), Qs(3), Qs(1), Qs(-1), Qs(-3), Qs(-5), Qs(-7)},
                        {6, 22, 57, 68, 38, 8, 1});
      Bond svd_L = Bond(BD_OUT, {Qs(1), Qs(-1)}, {1, 1});
      UniTensor svd_T = UniTensor({svd_I, svd_J, svd_K, svd_L}, {"a", "b", "c", "d"}, 1,
                                  Type.Double, Device.cpu, false);
    ...
    // Helper: verify that X is an isometry / unitary over its auxiliary index `aux`: contracting
    // the physical legs against X^dagger yields the identity on the aux index. Handles both
    // conventions:
    //   - aux is the FIRST leg (e.g. Svd vT):        checks X X^dagger = I
    //   - aux is the LAST  leg (e.g. Svd/Eig U, V):  checks X^dagger X = I
    // Per the fermionic contraction rule, the dagger operand is followed by fermion_twists() and
    // placed on the left of the contraction.
    ...
    // Helper: verify an SVD decomposition usv = [S, U, vT]: U and vT are isometries (U^dagger U =
    // I, vT vT^dagger = I) and the reconstruction A = U S vT holds. vT is the genuine
    // right-singular-vector tensor (already living on the column space), so the reconstruction is
    // the direct product -- no Dagger construction and no fermion_twists are needed (this is what
    ...
    // Helper: rank-4 square fermionic UniTensor on row space (a,b), canonical leg order [a, b, a*,
    // b*] (rowrank 2). Real-symmetric values -> Hermitian. A *consistent* permute (same permutation
    // on row and column legs, e.g. {1,0,3,2}) then makes the pending signflip non-trivial while
    // leaving the operator (hence its spectrum / singular values) unchanged.
    ...
    // ascending. Eigenvalues / singular values are invariant under a consistent permute, so
    // comparing these between a tensor and its permuted (sign-flip-active) form verifies correct
    // sign handling.
    ...
    // Helper: given the (diagonal) eigenvalue tensors of exp(a*M) and of M, assert their sorted
    // real diagonals satisfy exp_evals[i] == exp(a * evals[i]). exp is monotonic for real a, so the
    ...
    // Helper: assert two eigenvector tensors (physical legs + aux as the LAST leg) describe the
    // same eigenvectors up to a per-vector phase. For orthonormal eigenbases Ua^dagger Ub is then a
    // diagonal unitary: each diagonal entry is a unit-modulus phase and the off-diagonals vanish.
    // Assumes a non-degenerate spectrum with matching sorted ordering in both inputs (true for Eigh
    // of M and of a monotonic function of M). Per the fermionic contraction rule the dagger operand
    // is followed by fermion_twists().
    ...
    // Forms the linear, Hermitian operator A^dag A. Its spectrum equals the squared singular values
    // of A. The initial Krylov vector is likewise created permuted, and carries a non-trivial
    // signflip;
    ...
    // distinct ground state for the Krylov solvers), then permuted {1,0,3,2} so the pending
    // signflip is non-trivial (several blocks with, several without). The charge-0 column sector
    // has dimension 8 (large enough for ARPACK's ncv constraint 2+k <= ncv <= nx).
    ...
    inline UniTensor make_clean_ket(const UniTensor &A) {
      UniTensor v = UniTensor({A.bonds()[3].redirect(), A.bonds()[2].redirect()},
                              {A.labels()[3], A.labels()[2]});
      v.set_rowrank_(2);
      return v;
    }

    // the discriminating initial vector: the same signflip-free ket but *created in reversed leg
    // order and permuted into A's column order*, so it creates a non-trivial signflip in the
    // contraction
    inline UniTensor make_ferm_ada_ket(const UniTensor &A) {
      ...
      UniTensor vr = UniTensor({A.bonds()[2].redirect(), A.bonds()[3].redirect()},
                               {A.labels()[2], A.labels()[3]});
      vr.set_rowrank_(2);
      auto sh = vr.shape();
      double val = 1.0;
    ...
    // signed fermionic inner product <P|Q> (fully contracted -> a Dense scalar, so .item() is
    // valid).
    ...
    // eigenvector fidelity |<v|w>| / sqrt(<v|v><w|w>): phase-invariant, ~1 for the same
    // eigenvector.
    ...
    // diagonalization (independent of the Krylov solvers and of the library's sign-frame pinning --
    // it acts on clean, signflip-resolved basis kets). Returns {(eigenvalue, eigenvector ket)}
    ...
    // promoted to v's dtype here (not inside ferm_ada_apply): the real solve never needs it, only
    // this post-hoc residual on Arnoldi's complex eigenvector does.
    ...
    // Validate a Krylov result for the k lowest states against the dense reference `low`
    // (ascending). eigs[0] holds the k eigenvalues; eigs[1..k] are the eigenvectors. For each
    // state: eigenvalue matches (and is genuine), eigenvector is normalized and matches the dense
    // eigenvector
    ...
    inline void expect_lowest_states(const UniTensor &A, const std::vector<UniTensor> &eigs,
                                     const std::vector<std::pair<double, UniTensor>> &low,
                                     double tol) {
...
#endif  // CYTNX_TESTS_LINALG_TEST_LINALG_TEST_H_
