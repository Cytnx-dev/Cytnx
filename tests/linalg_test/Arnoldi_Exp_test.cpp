...
namespace Arnoldi_Exp_Ut_Test {
  ...
  TEST(Arnoldi_Exp_Ut, RealTypeTest) {
    int d = 2, D = 5;
    auto op = OneSiteOp(d, D);
    auto Tin = CreateA(d, D);
    const double crit = 1.0e-10;
    double tau = 0.1;
    auto x = linalg::Arnoldi_Exp(&op, Tin, tau, crit);
  ...
  TEST(Arnoldi_Exp_Ut, ComplexTypeTest) {
    int d = 2, D = 5;
  ...
  TEST(Arnoldi_Exp_Ut, IncorrectDType) {
    int d = 2, D = 10;
    auto op = OneSiteOp(d, D, Type.Int64);
    auto Tin = CreateA(d, D, Type.Int64);
    const double crit = 1.0e-3;
    double tau = 0.1;
    EXPECT_THROW({ linalg::Arnoldi_Exp(&op, Tin, crit, tau); }, std::logic_error);
  }
  ...
  UniTensor CreateOneSiteEffHam(const int d, const int D, const unsigned int dtype,
    ...
    //HEff_mat = HEff_mat + HEff_mat.permute({1, 0}).Conj();  // Hermitian
	HEff_mat *= 0.01; // Let H can be converge in ExpM
