  ...
  namespace {
    ...
    namespace GesvdTruncateTest {
      ...
      input:
        ...
        is_VT:true
      ====================*/
      TEST(GesvdTruncate, DenseOneElem) {
      ...
      //   is_VT:true
      // ====================*/
      // TEST(GesvdTruncate, DenseNondiagTest) {
      ...
      input:
        ...
        is_VT:true
      ====================*/
      TEST(GesvdTruncate, ErrDenseDiagTest) {
        int size = 5;
        ...
        EXPECT_THROW({ std::vector<UniTensor> Svds = linalg::Gesvd_truncate(T, 2); },
                     std::logic_error);
      }
      ...
      input:
        ...
        is_VT:true
      ====================*/
      TEST(GesvdTruncate, DenseExpSvalsTest) {
      ...
      input:
        ...
        is_VT:true
      ====================*/
      TEST(GesvdTruncate, U1ExpSvalsTest) {
      ...
      ====================*/
      TEST(GesvdTruncate, U1ExpSvalsMinblockdimTest) {
        std::vector<std::string> case_list = {"sym_UT_U1_exp_Svals_C128",
                                              "sym_UT_U1_exp_Svals_F64"};
        for (const auto& case_name : case_list) {
          std::string test_case_name =
      ...
      reference via CheckTruncatedSvdResult; U / vT shapes are checked inline (their gauge depends
      on LAPACK jobu/jobvt mode so we don't compare values).
      ====================*/
      TEST(GesvdTruncate, FlagCombinationsDense) {
          ...
          for (bool is_U : {false, true}) {
            for (bool is_vT : {false, true}) {
              for (int return_err : {0, 1, 2}) {
                ...
                CheckTruncatedSvdResult(out, full_ref[0], keep, is_U, is_vT, return_err, 1e-12,
                                        label);
      ...
      ====================*/
      TEST(GesvdTruncate, NoTruncationReturnsZeroError) {
      ...
      ====================*/
      TEST(GesvdTruncate, ReturnErrWhenMinBlockdimExhaustsKeepdim) {
      ...
      ====================*/
      TEST(GesvdTruncate, MinBlockdimKeepsEachSector) {
        auto syms = std::vector<Symmetry>{Symmetry(SymmetryType::U)};
        Bond bk = Bond(BD_KET, {{0}, {1}, {2}}, {3, 3, 3}, syms);
        UniTensor src_T({bk, bk.redirect()}, {"l", "r"}, 1, Type.Double, Device.cpu, false);
      ...
      ====================*/
      TEST(GesvdTruncate, MindimFloor) {
        Tensor T = Tensor({6, 5}, Type.Double);
