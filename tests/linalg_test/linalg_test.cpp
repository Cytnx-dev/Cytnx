...
namespace cytnx {
  namespace test {
    ...
    TEST_F(linalg_Test, BkUtEighReversedQnums) {
      ...
      // "_aux_L"; rename V.Dagger()'s row labels to M's col labels and its aux to "_aux_R" to chain
      // correctly.
    ...
    TEST_F(linalg_Test, BkFUtEig) {
      ...
      // construction would have to swap an odd-parity column-leg pair, which picks up a sign that
      // no fermion_twists() placement cancels -- so the eigen equation is only verified here, and
      // the rank-4 sign-flip case is covered by spectrum invariance instead (full reconstruction:
      // issue #782).
    ...
    describe:Eig for a fermionic operator with genuinely COMPLEX eigenvalues (non-Hermitian blocks):
      the eigenvalue UniTensor must keep ComplexDouble dtype and report the correct complex
    ...
    TEST_F(linalg_Test, BkFUtQr) {
      ...
      // rank-4 case whose fermionic reordering produces non-trivial sign flips, run for a real and
      // a genuinely complex operator -- exercises the real (dgeqrf) and complex (zgeqrf) Qr code
      // paths plus the signflip negation in Qr_BlockFermionic_UT_internal. (Qr needs no
      // Hermiticity.)
      for (bool cplx : {false, true}) {
        SCOPED_TRACE(cplx ? "ComplexDouble" : "Double");
    ...
    TEST_F(linalg_Test, BkFUtGesvdTruncateUnitaryAndReconstruction) {
      ...
      // the negation path in the BlockFermionic truncation while leaving U, S, vT correct. Run for
      // both a real and a genuinely complex-Hermitian operator so the real (?gesdd/?gesvd) and
      // complex (z-variant) code paths are both covered.
      for (bool cplx : {false, true}) {
        SCOPED_TRACE(cplx ? "ComplexDouble" : "Double");
    ...
    TEST_F(linalg_Test, BkFUtExpM) {
      ...
      // compare is well defined, while the consistent permute still exercises the signflip negation
      // path.
