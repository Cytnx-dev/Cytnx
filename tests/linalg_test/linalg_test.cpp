...
namespace cytnx {
  namespace test {
    namespace {
      ...
      TEST_F(linalg_Test, BkUtSvdTruncate1) {
      ...
      TEST_F(linalg_Test, BkUtSvdTruncate2) {
      ...
      TEST_F(linalg_Test, BkUtSvdTruncateReturnErrReturnsDiscardedValues) {
      ...
      TEST_F(linalg_Test, BkUtGesvdTruncateReturnErrReturnsDiscardedValues) {
      ...
      TEST_F(linalg_Test, BkUtSvdTruncateReturnErrOneReturnsFirstDiscardedValue) {
        std::vector<UniTensor> full = linalg::Svd_truncate(svd_T, 999, 0, true, 0);
      ...
      TEST_F(linalg_Test, BkUtGesvdTruncateReturnErrOneReturnsFirstDiscardedValue) {
        std::vector<UniTensor> full = linalg::Gesvd_truncate(svd_T, 999, 0, true, true, 0);
      ...
      ====================*/
      TEST_F(linalg_Test, BkUtSvdTruncateReturnErrNoTruncation) {
        // keepdim=999 keeps everything; return_err=1 is a scalar zero, while return_err=2 is an
        // empty vector.
      ...
      TEST_F(linalg_Test, BkUtGesvdTruncateReturnErrNoTruncation) {
        for (unsigned int re : {1u, 2u}) {
      ...
      ====================*/
      TEST_F(linalg_Test, BkFermionicUtSvdTruncateReturnErrReturnsDiscardedValues) {
      ...
      TEST_F(linalg_Test, BkFermionicUtGesvdTruncateReturnErrReturnsDiscardedValues) {
        UniTensor fermi_T = make_square_fermionic({"a", "b"});
        random::uniform_(fermi_T, -1.0, 1.0, 42);
        ASSERT_EQ(fermi_T.uten_type(), UTenType.BlockFermionic);
      ...
      // TEST_F(linalg_Test, BkUtSvdTruncate3) {
      ...
      //   Bond L = Bond(BD_OUT, {Qs(-5), Qs(-3), Qs(-1), Qs(1), Qs(3), Qs(5)}, {1, 4, 10, 9, 5,
      //   1}); UniTensor cyT = UniTensor({I, J, K, L}, {"a", "b", "c", "d"}, 2, Type.Double,
      //   Device.cpu, false); auto cyT2 = UniTensor::Load(data_dir +
      //   "Svd_truncate/Svd_truncate2.cytnx"); std::vector<UniTensor> res =
      ...
      // TEST_F(linalg_Test, BkUtSvdTruncate4) {
      ...
      TEST_F(linalg_Test, BkUtQrReversedQnums) {
      ...
      // 4-leg square Hermitian Block UniTensor where the two row legs use the directions given by
      // input arguments. Real-valued (row,col)-symmetric values -> Hermitian.
      ...
      // Eigh and ExpM use BD_OUT/BD_OUT as left bonds; Eig and ExpH use BD_OUT/BD_IN; Covers
      // different combine_bonds patternes in the linalg functions.

      TEST_F(linalg_Test, BkUtEighReversedQnums) {
        ...
        // M = V * diag(e) * V^dagger. V's row legs are M's row labels ("a","b") and its col is
        // "_aux_L"; rename V.Dagger()'s row labels to M's col labels and its aux to "_aux_R" to
        // chain correctly.
      ...
      TEST_F(linalg_Test, BkUtEigReversedQnums) {
        const double tol = 1e-10;
      ...
      TEST_F(linalg_Test, BkUtExpHReversedQnums) {
        const double tol = 1e-10;
        ...
        UniTensor RM =
          Contract(R.relabel({"a", "b", "_m", "_n"}), M.relabel({"_m", "_n", "c", "d"}));
        UniTensor MR =
          Contract(M.relabel({"a", "b", "_m", "_n"}), R.relabel({"_m", "_n", "c", "d"}));
      ...
      TEST_F(linalg_Test, BkUtExpMReversedQnums) {
        const double tol = 1e-10;
        const double alpha = 0.1;
        UniTensor M = make_rank4_hermitian_left_dirs(BD_OUT, BD_OUT);
      ...
      TEST_F(linalg_Test, DenseUtGesvdTruncate) {
        ...
        std::vector<UniTensor> truc2 =
          linalg::Gesvd_truncate(svd_T_dense, 5, 1e-12, true, true, 999);
      ...
      TEST_F(linalg_Test, DenseUtSvdTruncate) {
      ...
      TEST_F(linalg_Test, DenseUtPow2) {
        UniTensor Ht = UniTensor(A);
        linalg::Pow_(Ht, 3);
        for (size_t i = 0; i < 9; i++)
          for (size_t j = 0; j < 9; j++) {
      ...
      TEST_F(linalg_Test, TensorGemm2) {
      ...
      TEST_F(linalg_Test, TensorNorm2) {
        cytnx_double ans = 0;
        for (cytnx_uint64 i = 0; i < 9; i++) {
          ans += i * i * 2;
        }
      ...
      TEST_F(linalg_Test, TensorAddMixedDtypeTypePromoteCpu) {
      ...
      TEST_F(linalg_Test, TensorAddScalarMixedDtypeTypePromoteCpu) {
        Tensor rhs = arange(0, 4, 1, Type.Int16).reshape(2, 2);
      ...
      TEST_F(linalg_Test, DenseUtNorm) {
        cytnx_double ans = 0;
        for (cytnx_uint64 i = 0; i < 9; i++) {
          ans += i * i * 2;
        }
      ...
      TEST_F(linalg_Test, DenseUtNorm2) {
        cytnx_double ans = 0;
        for (cytnx_uint64 i = 0; i < 9; i++) {
          ans += i * i * 2;
        }
      ...
      TEST_F(linalg_Test, TensorEigRowV) {
      ...
      TEST_F(linalg_Test, TensorEigValuesOnly) {
      ...
      TEST_F(linalg_Test, TensorEighRowV) {
        const double tol = 1e-13;
        auto her = arange3x3cd + arange3x3cd.Conj().permute({1, 0});
      ...
      TEST_F(linalg_Test, DenseUtEigRowV) {
        const double tol = 1e-13;
      ...
      TEST_F(linalg_Test, DenseUtEigh) {
        const double tol = 1e-13;
        auto her = arange3x3cd + arange3x3cd.Conj().permute({1, 0});
      ...
      TEST_F(linalg_Test, DenseUtEighRowV) {
        const double tol = 1e-13;
        auto her = arange3x3cd + arange3x3cd.Conj().permute({1, 0});
      ...
      TEST_F(linalg_Test, DenseUtEigRowrankMustBeLessThanRank) {
      ...
      TEST_F(linalg_Test, DenseUtEighRowrankMustBeLessThanRank) {
        auto her = arange3x3cd + arange3x3cd.Conj().permute({1, 0});
        UniTensor invalid = UniTensor(her, false, 2);
        EXPECT_THROW({ linalg::Eigh(invalid); }, std::logic_error);
      }

      // TEST_F(linalg_Test, TensorInv) { EXPECT_TRUE(false); }

      // TEST_F(linalg_Test, TensorInv2) { EXPECT_TRUE(false); }

      // TEST_F(linalg_Test, DenseUtInv) { EXPECT_TRUE(false); }

      // TEST_F(linalg_Test, DenseUtInv2) { EXPECT_TRUE(false); }

      TEST_F(linalg_Test, TensorInvM) {
        auto inv = linalg::InvM(invertable3x3cd);
        EXPECT_TRUE((linalg::Tensordot(invertable3x3cd, inv, {1}, {0}) - eye3x3cd).Norm().item() <
                    1e-13);
      }

      TEST_F(linalg_Test, TensorInvM2) {
      ...
      TEST_F(linalg_Test, DenseUtInvM) {
      ...
      TEST_F(linalg_Test, DenseUtInvM2) {
      ...
      ====================*/
      TEST_F(linalg_Test, BkFUtSvdUnitaryAndReconstruction) {
        const double tol = 1e-10;
      ...
      TEST_F(linalg_Test, BkFUtEigh) {
        ...
        // cross-check against the general solver: for a Hermitian operator, Eig must return the
        // same (real) spectrum as Eigh (Eig eigenvalues are complex; sorted_diagonal compares real
        // parts).
        expect_same_diagonal(e, linalg::Eig(H)[0], tol);

        // sign-flip coverage: a consistent permute makes the pending signflip non-trivial
        // (exercising the negation path in Eigh_BlockFermionic_UT_internal). Verify the
        // eigenvectors are still orthonormal (V^dagger V = I) and the spectrum is unchanged. Run
        // real and genuinely complex-Hermitian so both the real (dsyev) and complex (zheev) Eigh
        ...
        for (bool cplx : {false, true}) {
          ...
          EXPECT_EQ(ep.dtype(),
                    Type.Double);  // eigenvalues are real even for a complex-Hermitian input
      ...
      TEST_F(linalg_Test, BkFUtEig) {
        ...
        EXPECT_TRUE(e.is_diag());
        EXPECT_EQ(e.dtype(),
                  Type.ComplexDouble);  // general (non-Hermitian) eigenvalues are complex
        ...
        // fermionic reorder sign. With two or more column legs (e.g. the rank-4 case below) the
        // same construction would have to swap an odd-parity column-leg pair, which picks up a sign
        // that no fermion_twists() placement cancels -- so the eigen equation is only verified
        // here, and the rank-4 sign-flip case is covered by spectrum invariance instead (full
        // reconstruction: issue #782).
        ...
        // leaves the eigenvalues invariant but makes the pending signflip non-trivial, exercising
        // the negation path in Eig_BlockFermionic_UT_internal.
      ...
      /*=====test info=====
      describe:Eig for a fermionic operator with genuinely COMPLEX eigenvalues (non-Hermitian
      blocks): the eigenvalue UniTensor must keep ComplexDouble dtype and report the correct complex
      eigenvalues with non-zero imaginary parts.
      ====================*/
      TEST_F(linalg_Test, BkFUtEigComplexEigenvalues) {
        const double tol = 1e-10;
        ...
        for (auto &bk : e.get_blocks_()) {
          ...
          for (cytnx_uint64 i = 0; i < st.size(); i++)
            eigvals.push_back(st.at<cytnx_complex128>(i));
        }
      ...
      TEST_F(linalg_Test, BkFUtQr) {
        ...
        // rank-4 case whose fermionic reordering produces non-trivial sign flips, run for a real
        // and a genuinely complex operator -- exercises the real (dgeqrf) and complex (zgeqrf) Qr
        // code paths plus the signflip negation in Qr_BlockFermionic_UT_internal. (Qr needs no
        // Hermiticity.)
        for (bool cplx : {false, true}) {
          SCOPED_TRACE(cplx ? "ComplexDouble" : "Double");
      ...
      ====================*/
      TEST_F(linalg_Test, BkFUtEigEighRowVGuards) {
        UniTensor H = make_square_fermionic({"i", "o"});
        H.at({0, 0}) = 2.0;
        H.at({0, 1}) = 0.7;
        H.at({1, 0}) = 0.7;
      ...
      ====================*/
      TEST_F(linalg_Test, BkFUtGesvdTruncateUnitaryAndReconstruction) {
        const double tol = 1e-10;
        UniTensor M = make_mixed_inout_fermionic();
        ...
        // sign-flip-active: a consistent permute makes the pending signflip non-trivial, exercising
        // the negation path in the BlockFermionic truncation while leaving U, S, vT correct. Run
        // for both a real and a genuinely complex-Hermitian operator so the real (?gesdd/?gesvd)
        // and complex (z-variant) code paths are both covered.
        for (bool cplx : {false, true}) {
          SCOPED_TRACE(cplx ? "ComplexDouble" : "Double");
      ...
      describe:#841 regression -- after Svd_truncate/Gesvd_truncate genuinely drops blocks (small
        ...
        post-truncation.
      ====================*/
      TEST_F(linalg_Test, BkFUtTruncateKeepsSignflipInLockstepWithNblocks) {
      ...
      /*=====test info=====
      describe:ExpH for fermionic tensors: exp of a (sign-flip-active) Hermitian operator has
      spectrum exp(a * eigenvalues), verifying the per-sector signflip negation in
      ExpH_BlockUT_internal.
      ====================*/
      TEST_F(linalg_Test, BkFUtExpH) {
        const double tol = 1e-10;
        const double a = 0.5;
        // sign-flip-active Hermitian operator (consistent permute keeps it Hermitian); run real and
        // genuinely complex-Hermitian so both the real and complex Eigh/ExpH code paths are
        // covered.
        for (bool cplx : {false, true}) {
          SCOPED_TRACE(cplx ? "ComplexDouble" : "Double");
      ...
      ====================*/
      TEST_F(linalg_Test, BkFUtExpM) {
        const double tol = 1e-10;
        const double a = 0.3;
        // ExpM accepts a general operator; a Hermitian one gives a real spectrum so the sorted
        // compare is well defined, while the consistent permute still exercises the signflip
        // negation path.
        UniTensor Mp = permute_with_signflips(make_rank4_hermitian());
        UniTensor eM = linalg::ExpM(Mp, a);
        EXPECT_EQ(eM.uten_type(), UTenType.BlockFermionic);
        // both operands are Hermitian, so use Eigh (sorted, orthonormal) for a well-defined
        // comparison even though ExpM itself uses the general Eig internally.
      ...
      /*=====test info=====
      describe:ExpM bias term for fermionic tensors: ExpM(M, a=0, b) must equal exp(0*M + b*I) =
      exp(b)*I, i.e. the bias b must NOT be dropped by the a==0 fast path.
      ====================*/
      TEST_F(linalg_Test, BkFUtExpMBias) {
        const double tol = 1e-10;
        const double b = 0.5;
        ...
        // the bias is preserved: exp(0,b) = exp(b) * exp(0,0). (With the bug eMb == eM0 !=
        // exp(b)*eM0.)
      ...
      // Helper: rank-5 fermionic UniTensor (legs a,b,c,d,e; rowrank 3) populated with sequential
      // values 1..8 (or their reciprocals when `inverse` is set) over its eight existing charge-0
      // components.
