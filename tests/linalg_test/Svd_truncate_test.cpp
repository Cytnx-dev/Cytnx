...
namespace cytnx {
  namespace test {
    namespace {
      ...
      //   is_VT:true
      // ====================*/
      // TEST(SvdTruncate, DenseNondiagTest) {
      ...
      input:
        ...
        is_VT:true
      ====================*/
      TEST(SvdTruncate, ErrDenseDiagTest) {
        int size = 5;
        ...
        EXPECT_THROW({ std::vector<UniTensor> Svds = linalg::Svd_truncate(T, 2); },
                     std::logic_error);
      }
      ...
      input:
        ...
        is_VT:true
      ====================*/
      TEST(SvdTruncate, DenseExpSvalsTest) {
      ...
      ====================*/
      TEST(SvdTruncate, U1ExpSvalsMinblockdimTest) {
      ...
      static bool CheckResult(const std::string& case_name, const cytnx_uint64& keepdim,
        ...
        std::vector<UniTensor> Svds =
          linalg::Svd_truncate(src_T, keepdim, min_blockdim, 0., true, 0);
      ...
      previously read out-of-range vector slots) and every return_err mode. Singular-value content
      is validated against a full Svd reference via CheckTruncatedSvdResult; U/vT shapes are checked
      inline (their gauge depends on LAPACK gesdd mode so we don't compare values).
      ====================*/
      TEST(SvdTruncate, FlagCombinationsDense) {
        for (auto dtype : {Type.Double, Type.ComplexDouble}) {
          ...
          for (bool is_UvT : {false, true}) {
            for (int return_err : {0, 1, 2}) {
              ...
              CheckTruncatedSvdResult(out, full_ref[0], keep, is_UvT, is_UvT, return_err, 1e-12,
                                      label);
      ...
      ====================*/
      TEST(SvdTruncate, NoTruncationReturnsZeroError) {
      ...
      report the actually dropped singular values, not a 1-element zero tensor.
      ====================*/
      TEST(SvdTruncate, ReturnErrWhenMinBlockdimExhaustsKeepdim) {
      ...
      ====================*/
      TEST(SvdTruncate, MinBlockdimKeepsEachSector) {
        auto syms = std::vector<Symmetry>{Symmetry(SymmetryType::U)};
        Bond bk = Bond(BD_KET, {{0}, {1}, {2}}, {3, 3, 3}, syms);
        UniTensor src_T({bk, bk.redirect()}, {"l", "r"}, 1, Type.Double, Device.cpu, false);
