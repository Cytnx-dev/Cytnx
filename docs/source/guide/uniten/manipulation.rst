...
The bond order can be changed with *permute* for all kinds of UniTensors. The order can either be either defined by the index order as for the permute function for a Tensor, or by specifying the label order after the permutation.
