UniTensor
==========
UniTensor is an extension object base on **cytnx.Tensor**. 
A UniTensor consist of three imporant parts: 1. Block, which is a **Cytnx.Tensor**, 2. Bonds, which defines the bond information associate to each rank. 3. labels, which gives each rank/bond unique ID. In the most simple example where we don't consider any symmetry in our system, UniTensor can be think of adding these bonds and labels meta data around **cytnx.Tensor**. 

A UniTensor can be constructed using **Bond** object, or in the simple case directly converted from a **Cytnx.Tensor**. 

Let's take a look at this:

.. toctree::
    :maxdepth: 1

    UniTensor_1_create.rst
    UniTensor_2_manip.rst
