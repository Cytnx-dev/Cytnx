...
namespace DocTest {
  ...
  TEST(Doc, guide_behavior_assign){OUT_REDIRECT
#include "guide_behavior_assign.cpp"
                                     OUT_RESET}

  TEST(Doc, guide_behavior_clone){OUT_REDIRECT
#include "guide_behavior_clone.cpp"
  ...
  TEST(Doc, guide_Device_Ncpus){OUT_REDIRECT
#include "guide_Device_Ncpus.cpp"
                                  OUT_RESET}

  // 2.2 Number of GPUs
  TEST(Doc, guide_Device_Ngpus){OUT_REDIRECT
...
// 3.1.3 Tensor with different dtype and device
  ...
  TEST(Doc, guide_basic_obj_Tensor_1_create_astype){OUT_REDIRECT
...
#include "guide_basic_obj_Tensor_1_create_from_storage.cpp"
  ...
  // 3.2.1 reshape
  TEST(Doc, guide_basic_obj_Tensor_2_manip_reshape){OUT_REDIRECT
#include "guide_basic_obj_Tensor_2_manip_reshape.cpp"
                                                      OUT_RESET}

  TEST(Doc, guide_basic_obj_Tensor_2_manip_reshape_){OUT_REDIRECT
#include "guide_basic_obj_Tensor_2_manip_reshape_.cpp"
                                                       OUT_RESET}

  // 3.2.2 permute
  TEST(Doc, guide_basic_obj_Tensor_2_manip_permute){OUT_REDIRECT
#include "guide_basic_obj_Tensor_2_manip_permute.cpp"
                                                      OUT_RESET}

  TEST(Doc, guide_basic_obj_Tensor_2_manip_contiguous){OUT_REDIRECT
#include "guide_basic_obj_Tensor_2_manip_contiguous.cpp"
                                                         OUT_RESET}

  // 3.3 Accessing elements
  // 3.3.1 Get elements
  TEST(Doc, guide_basic_obj_Tensor_3_access_slice_get){OUT_REDIRECT
#include "guide_basic_obj_Tensor_3_access_slice_get.cpp"
                                                         OUT_RESET}

  TEST(Doc, guide_basic_obj_Tensor_3_access_item){OUT_REDIRECT
#include "guide_basic_obj_Tensor_3_access_item.cpp"
                                                    OUT_RESET}

  // 3.3.2 Get elements
  TEST(Doc, guide_basic_obj_Tensor_3_access_slice_set){OUT_REDIRECT
...
#include "guide_basic_obj_Tensor_3_access_c_get_set.cpp"
  ...
  TEST(Doc, guide_basic_obj_Tensor_4_arithmetic_tensor_scalar){OUT_REDIRECT
#include "guide_basic_obj_Tensor_4_arithmetic_tensor_scalar.cpp"
                                                                 OUT_RESET}

  // 3.4.3 Tensor-Tensor arithmetic
  TEST(Doc, guide_basic_obj_Tensor_4_arithmetic_tensor_tensor){OUT_REDIRECT
...
#include "guide_basic_obj_Tensor_4_arithmetic_Add.cpp"
  ...
  TEST(Doc, guide_basic_obj_Tensor_6_app_scalar){OUT_REDIRECT
#include "guide_basic_obj_Tensor_6_app_scalar.cpp"
                                                   OUT_RESET}

  TEST(Doc, guide_basic_obj_Tensor_6_app_tensor){OUT_REDIRECT
#include "guide_basic_obj_Tensor_6_app_tensor.cpp"
                                                   OUT_RESET}

  // 3.7 Save/Load
  // 3.7.1 Save a Tensor
  TEST(Doc, guide_basic_obj_Tensor_7_io_Save){OUT_REDIRECT
#include "guide_basic_obj_Tensor_7_io_Save.cpp"
                                                OUT_RESET}

  // 3.7.2 Load a Tensor
  TEST(Doc, guide_basic_obj_Tensor_7_io_Load){OUT_REDIRECT
#include "guide_basic_obj_Tensor_7_io_Load.cpp"
                                                OUT_RESET}

  // 3.8 When will data be copied?
  // 3.8.1 Reference to & Copy of objects
  TEST(Doc, guide_basic_obj_Tensor_8_cp_assign){OUT_REDIRECT
#include "guide_basic_obj_Tensor_8_cp_assign.cpp"
                                                  OUT_RESET}

  TEST(Doc, guide_basic_obj_Tensor_8_cp_clone){OUT_REDIRECT
...
#include "guide_basic_obj_Tensor_8_cp_contiguous-2.cpp"
  ...
  TEST(Doc, guide_basic_obj_Storage_1_create_create){OUT_REDIRECT
#include "guide_basic_obj_Storage_1_create_create.cpp"
                                                       OUT_RESET}

  // 4.1.1 Type conversion
  TEST(Doc, guide_basic_obj_Storage_1_create_astype){OUT_REDIRECT
...
// 4.1.2 Transfer between devices
  ...
  TEST(Doc, guide_basic_obj_Storage_1_create_get_storage){OUT_REDIRECT
#include "guide_basic_obj_Storage_1_create_get_storage.cpp"
                                                            OUT_RESET}

  // 4.2 Accessing elements
  // 4.2.1 Get/Set elements
  TEST(Doc, guide_basic_obj_Storage_2_access_access){OUT_REDIRECT
#include "guide_basic_obj_Storage_2_access_access.cpp"
                                                       OUT_RESET}

  TEST(Doc, guide_basic_obj_Storage_2_access_at){OUT_REDIRECT
...
#include "guide_basic_obj_Storage_2_access_ptr_void.cpp"
  ...
  // 4.3 Increase size
  // 4.3.1 append
  TEST(Doc, guide_basic_obj_Storage_3_expand_append){OUT_REDIRECT
#include "guide_basic_obj_Storage_3_expand_append.cpp"
                                                       OUT_RESET}

  // 4.3.2 resize
  TEST(Doc, guide_basic_obj_Storage_3_expand_resize){OUT_REDIRECT
#include "guide_basic_obj_Storage_3_expand_resize.cpp"
                                                       OUT_RESET}

// 4.4 From/To C++ .vector
#ifdef UNI_GPU
  TEST(Doc, guide_basic_obj_Storage_4_vec_from_vec){OUT_REDIRECT
  #include "guide_basic_obj_Storage_4_vec_from_vec.cpp"
                                                      OUT_RESET}
#endif

  TEST(Doc, guide_basic_obj_Storage_4_vec_to_vec){OUT_REDIRECT
...
#include "guide_basic_obj_Storage_5_io_Save.cpp"
  ...
  TEST(Doc, guide_basic_obj_Storage_5_io_Load){OUT_REDIRECT
#include "guide_basic_obj_Storage_5_io_Load.cpp"
                                                 OUT_RESET}

  // 4.5.3 Save & load from/to binary
  TEST(Doc, guide_basic_obj_Storage_5_io_from_to_file){OUT_REDIRECT
#include "guide_basic_obj_Storage_5_io_from_to_file.cpp"
                                                         OUT_RESET}

  // 5. Scalar
  // 5.1 Define/Declare a Scalar
  TEST(Doc, guide_basic_obj_Scalar_from_c_var){OUT_REDIRECT
#include "guide_basic_obj_Scalar_from_c_var.cpp"
                                                 OUT_RESET}

  TEST(Doc, guide_basic_obj_Scalar_create){OUT_REDIRECT
#include "guide_basic_obj_Scalar_create.cpp"
                                             OUT_RESET}

  TEST(Doc, guide_basic_obj_Scalar_to_c_var){OUT_REDIRECT
#include "guide_basic_obj_Scalar_to_c_var.cpp"
                                               OUT_RESET}

  // 5.2 Change date type
  TEST(Doc, guide_basic_obj_Scalar_astype){OUT_REDIRECT
#include "guide_basic_obj_Scalar_astype.cpp"
                                             OUT_RESET}

  // 5.3 Application scenarios
  TEST(Doc, guide_basic_obj_Scalar_application){OUT_REDIRECT
...
#include "guide_uniten_elements_at_set.cpp"
  ...
  TEST(Doc,
       guide_contraction_network_launch){OUT_REDIRECT_FILE("guide_contraction_network_PutUniTensor")
...
#include "guide_contraction_network_launch.cpp"
  ...
  // 10. Iterative solver
  // 10.1 LinOp class
  TEST(Doc, guide_itersol_LinOp_Dot){OUT_REDIRECT
