...
# The wheels' library and NVVM layouts need the same repair here as in a
# Pixi CUDA environment, so both call the one script.
bash "$(dirname "$0")/prepare_cuda_layout.sh" "${toolchain_prefix}"
