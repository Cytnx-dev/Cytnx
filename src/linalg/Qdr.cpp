...
namespace cytnx{
    namespace linalg{
        ...
        std::vector<Tensor> Qdr(const Tensor &Tin, const bool &is_tau){
            ...
            tau.Init({n_tau},in.dtype(),in.device()); tau.storage().set_zeros();
            ...
            if(Tin.device()==Device.cpu){
                ...
                cytnx::linalg_internal::lii.QR_ii[in.dtype()](in._impl->storage()._impl, 
                                                        ...
                                                        tau._impl->storage()._impl,in.shape()[0],in.shape()[1],true);
                ...
                #else
                    cytnx_error_msg(true,"[QdR] fatal error,%s","try to call the gpu section without CUDA support.\n");
                    return std::vector<Tensor>();
                #endif
            }    
    ...
    namespace linalg{
        std::vector<UniTensor> Qdr(const UniTensor &Tin, const bool &is_tau){
            if(Tin.is_blockform()){
                ...
                cytnx_error_msg(Tin.rowrank() < 1 || Tin.rank()==1,"[Qdr][ERROR] Qdr for DenseUniTensor should have rank>1 and rowrank>0%s","\n");
               ...
               //D
               outCyT[1] = UniTensor(outT[1],1,true);
               ...
               outCyT[2] = UniTensor(outT[2],1);
               outCyT[2].set_labels(Qlbl);


               // tau
               if(is_tau){
                   outCyT[3] = UniTensor(outT[3],0);
               }
