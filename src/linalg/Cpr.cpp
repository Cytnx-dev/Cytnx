        ...
        Tensor Cpr(const Tensor &Lt, const Tensor &Rt){
            ...
            if(Lt.is_contiguous() && Rt.is_contiguous()){
                ...
                if(Lt.device() == Device.cpu){
                    cytnx::linalg_internal::lii.Ari_ii[Lt.dtype()][Rt.dtype()](out._impl->storage()._impl,Lt._impl->storage()._impl,Rt._impl->storage()._impl,Lt._impl->storage()._impl->size(),{},{},{},4);
                }else{
                    #ifdef UNI_GPU
                        checkCudaErrors(cudaSetDevice(Rt.device()));
                        cytnx::linalg_internal::lii.cuAri_ii[Lt.dtype()][Rt.dtype()](out._impl->storage()._impl,Lt._impl->storage()._impl,Rt._impl->storage()._impl,Rt._impl->storage()._impl->size(),{},{},{},4);
                    #else
                ...
                if(Lt.device() == Device.cpu){
                    ...
                    #ifdef UNI_GPU
                        cytnx_error_msg(true, "[Cpr][on GPU/CUDA] error two tensors must be contiguous. Call Contiguous_() or Contiguous() first%s","\n");
                    #else
                        cytnx_error_msg(true,"[Cpr] fatal error, the tensor is on GPU without CUDA support.%s","\n"); 
                    #endif
                }
