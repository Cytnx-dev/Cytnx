...
namespace cytnx {
  ...
  namespace linalg {
    ...
    Tensor Axpy(const Scalar &a, const Tensor &x, const Tensor &y) {
        ...
        if(y.shape().size()==0) no_y = true;
        else{
            //   a*x + y -> return 
            cytnx_error_msg(x.shape() != y.shape(),"[ERROR][Axpy] x.shape() and y.shape() must be the same!%s","\n");
        ...
        if(no_y){
            ...
            if(y.dtype() > fin_dtype) out = y.astype(fin_dtype); 
            else out = y.clone();
        }
