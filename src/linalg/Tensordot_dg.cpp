    ...
    namespace linalg{
        Tensor Tensordot_dg(const Tensor &Tl, const Tensor &Tr, const std::vector<cytnx_uint64> &idxl, const std::vector<cytnx_uint64> &idxr, const bool &diag_L){
            ...
            if((Tl.shape().size()!=1) && (Tr.shape().size()!=1))
                cytnx_error_msg(true,"[ERROR] dg version of Tensordot requires one of them to be rank-1 input as a diagonal rank-2 tensor.%s","\n");
            ...
            if(diag_L){
                cytnx_error_msg(Tl.shape().size()!=1,"[ERROR] diag_L=true requires Tl to be rank-1 tensor.%s","\n");
                ...
                cytnx_error_msg(Tr.shape().size()!=1,"[ERROR] diag_L=false requires Tr to be rank-1 tensor.%s","\n");
            ...
            for(cytnx_uint64 i=0;i<idxl.size();i++){
                cytnx_error_msg(Tlshape[idxl[i]] != Trshape[idxr[i]],"the index L=%d and R=%d have different dimension!\n",idxl[i],idxr[i]);
