...
namespace cytnx {
  namespace linalg {
    ...
    // actual impls:
    static void Svd_Dense_UT_internal(std::vector<cytnx::UniTensor> &outCyT,
      ...
      std::vector<Tensor> outT = cytnx::linalg::Svd(tmp, compute_uv);
    ...
    }  // Svd_Dense_UT_internal

    // Block-wise Svd for a symmetric UniTensor. Handles both BlockUniTensor (bosonic) and
    ...
    // per-sector dense Svd, and the resulting U/S/vT carry an all-false signflip (they are already
    // physical). For the bosonic case there are no sign flips and those steps do nothing.
    template <class BUT>
    static void Svd_BlockUT_internal(std::vector<cytnx::UniTensor> &outCyT,
                                     const cytnx::UniTensor &Tin, const bool &compute_uv) {
    ...
    }  // Svd_BlockUT_internal

    std::vector<cytnx::UniTensor> Svd(const cytnx::UniTensor &Tin, const bool &is_UvT) {
      // using rowrank to split the bond to form a matrix.
      cytnx_error_msg(Tin.rank() <= 1,
                      "[ERROR][Svd] Input UniTensor should have rank>1, but rank is %d\n",
                      Tin.rank());
      cytnx_error_msg(Tin.rowrank() < 1,
                      "[ERROR][Svd] Input UniTensor should have rowrank>0, but rowrank is %d\n",
                      Tin.rowrank());
      cytnx_error_msg(
        Tin.rowrank() >= Tin.rank(),
        "[ERROR][Svd] Input UniTensor should have rowrank<rank, but rowrank is %d and rank is %d\n",
        Tin.rowrank(), Tin.rank());
      cytnx_error_msg(Tin.is_diag(),
                      "[ERROR][Svd] Input UniTensor is diagonal, so Svd is trivial and not "
                      "supported. Use other manipulation.%s",
                      "\n");

      std::vector<UniTensor> outCyT;
      if (Tin.uten_type() == UTenType.Dense) {
        Svd_Dense_UT_internal(outCyT, Tin, is_UvT);

      } else if (Tin.uten_type() == UTenType.Block) {
        Svd_BlockUT_internal<BlockUniTensor>(outCyT, Tin, is_UvT);

      } else if (Tin.uten_type() == UTenType.BlockFermionic) {
        Svd_BlockUT_internal<BlockFermionicUniTensor>(outCyT, Tin, is_UvT);

      } else {
        cytnx_error_msg(true, "[ERROR][Svd] UniTensor type '%s' not supported\n",
