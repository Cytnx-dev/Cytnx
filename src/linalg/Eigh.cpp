...
namespace cytnx {
  namespace linalg {
    ...
    static void Eigh_Block_UT_internal(std::vector<cytnx::UniTensor> &outCyT, const UniTensor &Tin,
                                       const bool &is_V, const bool &row_v) {
      // Exactly the same as Eig_Block_UT_internal with Eig() -> Eigh()
      ...
      //
      cytnx_error_msg(
        row_v, "[ERROR] Currently Eigh with row_v = true is not supported for BlockUniTensor.%s",
        "\n");
    ...
    }  // Eigh_Block_UT_internal

    static void Eigh_BlockFermionic_UT_internal(std::vector<cytnx::UniTensor> &outCyT,
                                                const UniTensor &Tin, const bool &is_V,
                                                const bool &row_v) {
      // Exactly the same as Eig_BlockFermionic_UT_internal with Eig() -> Eigh()
      // outCyT must be empty and Tin must be checked with proper rowrank!
      ...
      cytnx_error_msg(
        row_v,
        "[ERROR] Currently Eigh with row_v = true is not supported for BlockFermionicUniTensor.%s",
        "\n");
      ...
      if (is_V) {
        BlockFermionicUniTensor *v_ptr = new BlockFermionicUniTensor();
        for (int i = 0; i < Tin.rowrank(); i++) {
          v_ptr->_bonds.push_back(Tin.bonds()[i].clone());
          v_ptr->_labels.push_back(Tin.labels()[i]);
        }
        ...
        v_ptr->_inner_to_outer_idx = v_itoi;
        v_ptr->_blocks = v_blocks;
        v_ptr->_signflip = std::vector(v_blocks.size(), false);
        UniTensor V;
        V._impl = boost::intrusive_ptr<UniTensor_base>(v_ptr);
        outCyT.push_back(V);
      }
    }  // Eigh_BlockFermionic_UT_internal
    ...
    std::vector<cytnx::UniTensor> Eigh(const UniTensor &Tin, const bool &is_V, const bool &row_v) {
      ...
      if (Tin.uten_type() == UTenType.Dense) {
        ...
        Eigh_Block_UT_internal(outCyT, Tin, is_V, row_v);

      } else if (Tin.uten_type() == UTenType.BlockFermionic) {
        Eigh_BlockFermionic_UT_internal(outCyT, Tin, is_V, row_v);

      } else {
        cytnx_error_msg(true, "[ERROR] Eigh only supports Dense/Block/BlockFermionic UniTensors.%s",
                        "\n");
      }  // ut type
