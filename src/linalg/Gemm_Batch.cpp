...
namespace cytnx {
  namespace linalg {
    ...
    void __Gemm_Batch(const vector<char> &transa_array, const vector<char> &transb_array,
                 ...
                 const vector<Scalar> &alpha_array, const void **a_array,
                 const void **b_array,
    ...
    void Gemm_Batch(const vector<cytnx_int64> &m_array, const vector<cytnx_int64> &n_array, const vector<cytnx_int64> &k_array,
                 const vector<Scalar> &alpha_array, const vector<Tensor> &a_tensors,
                 const vector<Tensor> &b_tensors,
      ...
      vector<blas_int> ms(vec_cast<cytnx_int64,blas_int>(m_array)), ns(vec_cast<cytnx_int64,blas_int>(n_array)), ks(vec_cast<cytnx_int64,blas_int>(k_array));

      if(a_tensors[0].device()==Device.cpu){
        linalg_internal::lii.Gemm_Batch_ii[fin_dtype](transs.data(), transs.data(), ns.data(), ms.data(), ks.data(),
                tmp_alpha_array, (const void**)b_array, ns.data(), (const void**)a_array, ks.data(),
                tmp_beta_array, (void**)c_array, ns.data(), (blas_int)group_count, vec_cast<cytnx_int64,blas_int>(group_size).data());
      }else{
#ifdef UNI_GPU
        checkCudaErrors(cudaSetDevice(a_tensors[0].device()));
        linalg_internal::lii.cuGemm_Batch_ii[fin_dtype](transs.data(), transs.data(), ns.data(), ms.data(), ks.data(),
                tmp_alpha_array, (const void**)b_array, ns.data(), (const void**)a_array, ks.data(),
                tmp_beta_array, (void**)c_array, ns.data(), group_count, vec_cast<cytnx_int64,blas_int>(group_size).data());
#else
        cytnx_error_msg(true,"[Gemm_Batch] fatal error,%s","try to use GPU but not compiled with GPU support.\n");
#endif
      }
