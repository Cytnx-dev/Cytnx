...
namespace cytnx{
    namespace linalg{
        ...
        void _Lanczos_Gnd_general_Ut(std::vector<UniTensor> &out, LinOp *Hop, const UniTensor &Tin, const bool &is_V, const double &CvgCrit, const unsigned int &Maxiter, const bool &verbose){
            ...
            */
            cytnx_error_msg(new_psi.labels().size() != psi_1.labels().size(), "[ERROR] LinOp.matvec(UniTensor) -> UniTensor the output should have same labels and shape as input!%s","\n");
            cytnx_error_msg(new_psi.labels()!=psi_1.labels(),"[ERROR] LinOp.matvec(UniTensor) -> UniTensor the output should have same labels and shape as input!%s","\n");

            As(0) = Contract(new_psi.Dagger(),psi_1).item().real();
            
            //As(0) = linalg::Vectordot(new_psi.get_block_().flatten(),psi_1.get_block_().flatten(),true);
        ...
        // Lanczos       
        std::vector<UniTensor> Lanczos_Gnd_Ut(LinOp *Hop, const UniTensor &Tin, const double &CvgCrit, const bool &is_V, const bool &verbose, const unsigned int &Maxiter){
