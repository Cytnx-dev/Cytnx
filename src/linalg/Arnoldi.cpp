...
namespace cytnx {
  namespace linalg {
	...
	*itr = init_scalar;
    ...
    bool IsEigvalCvg(const std::vector<Scalar> &eigvals, 
    ...
    std::vector<Tensor> GetEigTens(const std::vector<Tensor> qs, 
    ...
    void _Arnoldi(std::vector<Tensor>& out, LinOp *Hop, 
		 const Tensor& T_init, 
      ...
      Tensor kry_mat_buffer = cytnx::zeros({imp_maxiter + 1, imp_maxiter + 1}, Hop->dtype(), Hop->device());
      bool is_cvg = false;
	...
	//check converged
        ...
        if(is_eigval_cvg || i == imp_maxiter - 1) {
          ...
          bool is_res_small_enough = 
              IsResiduleSmallEnough(Hop, eigTens_s, eigvals, CvgCrit);
    ...
    std::vector<Tensor> Arnoldi(LinOp *Hop, const Tensor& T_init, 
		                const std::string which, 
				const cytnx_uint64 &maxiter, 
