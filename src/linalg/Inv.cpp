    ...
    namespace linalg{
        Tensor Inv(const Tensor &Tin, const double &clip){
            ...
            if(Tin.device() == Device.cpu){
                cytnx::linalg_internal::lii.Inv_inplace_ii[out.dtype()](out._impl->storage()._impl,out._impl->storage()._impl->size(),clip);
            }else{
                #ifdef UNI_GPU
                checkCudaErrors(cudaSetDevice(Tin.device()));
                cytnx::linalg_internal::lii.cuInv_inplace_ii[out.dtype()](out._impl->storage()._impl,out._impl->storage()._impl->size(),clip);
                #else
