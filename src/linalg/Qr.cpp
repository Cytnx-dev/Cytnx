...
namespace cytnx {
  namespace linalg {
    ...
    static void Qr_Block_UT_internal(std::vector<UniTensor> &outCyT, const UniTensor &Tin,
                                     const bool &is_tau) {
    ...
    }  // Qr_Block_UT_internal

    static void Qr_BlockFermionic_UT_internal(std::vector<UniTensor> &outCyT, const UniTensor &Tin,
                                              const bool &is_tau) {
      // outCyT must be empty and Tin must be checked with proper rowrank!
      ...
      // process Q
      BlockFermionicUniTensor *Q_ptr = new BlockFermionicUniTensor();
      for (int i = 0; i < Tin.rowrank(); i++) {
        Q_ptr->_bonds.push_back(Tin.bonds()[i].clone());
        Q_ptr->_labels.push_back(Tin.labels()[i]);
      }
      ...
      Q_ptr->_inner_to_outer_idx = Q_itoi;
      Q_ptr->_blocks = Q_blocks;
      Q_ptr->_signflip = std::vector(Q_blocks.size(), false);
      UniTensor Q;
      Q._impl = boost::intrusive_ptr<UniTensor_base>(Q_ptr);
      outCyT.push_back(Q);

      // process R:
      BlockFermionicUniTensor *R_ptr = new BlockFermionicUniTensor();
      R_ptr->_bonds.push_back(Bd_aux);
      R_ptr->_labels.push_back("_aux_");
      for (int i = Tin.rowrank(); i < Tin.rank(); i++) {
        R_ptr->_bonds.push_back(Tin.bonds()[i].clone());
      ...
      R_ptr->_inner_to_outer_idx = R_itoi;
      R_ptr->_blocks = R_blocks;
      R_ptr->_signflip = std::vector(R_blocks.size(), false);
      UniTensor R;
      R._impl = boost::intrusive_ptr<UniTensor_base>(R_ptr);
      outCyT.push_back(R);
    ...
    std::vector<UniTensor> Qr(const UniTensor &Tin, const bool &is_tau) {
      ...
      if (Tin.uten_type() == UTenType.Dense) {
        ...
        Qr_Block_UT_internal(outCyT, Tin, is_tau);
