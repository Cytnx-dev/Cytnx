...
namespace cytnx {
  namespace linalg_internal {
    ...
    __global__ void cuAbs_kernel(cytnx_double *out, const cuDoubleComplex *ptr, const cytnx_uint64 Nelem){
          if(blockIdx.x*blockDim.x + threadIdx.x < Nelem){
              out[blockIdx.x*blockDim.x + threadIdx.x] = cuCabs(ptr[blockIdx.x*blockDim.x + threadIdx.x]);
          }
          __syncthreads();
     }
    __global__ void cuAbs_kernel(cytnx_float *out, const cuFloatComplex *ptr, const cytnx_uint64 Nelem){
          if(blockIdx.x*blockDim.x + threadIdx.x < Nelem){
              out[blockIdx.x*blockDim.x + threadIdx.x] = cuCabsf(ptr[blockIdx.x*blockDim.x + threadIdx.x]);
          }
          __syncthreads();
     }
    __global__ void cuAbs_kernel(cytnx_double *out, const double *ptr, const cytnx_uint64 Nelem){
          if(blockIdx.x*blockDim.x + threadIdx.x < Nelem){
              out[blockIdx.x*blockDim.x + threadIdx.x] = fabs(ptr[blockIdx.x*blockDim.x + threadIdx.x]);
          }
          __syncthreads();
     }
    __global__ void cuAbs_kernel(cytnx_float *out, const float *ptr, const cytnx_uint64 Nelem){
          if(blockIdx.x*blockDim.x + threadIdx.x < Nelem){
              out[blockIdx.x*blockDim.x + threadIdx.x] = fabsf(ptr[blockIdx.x*blockDim.x + threadIdx.x]);
          }
          __syncthreads();
     }
    // [NOTE] no unsigned version!
     __global__ void cuAbs_kernel(cytnx_int64 *out, const cytnx_int64 *ptr, const cytnx_uint64 Nelem){
          if(blockIdx.x*blockDim.x + threadIdx.x < Nelem){
              out[blockIdx.x*blockDim.x + threadIdx.x] = llabs(ptr[blockIdx.x*blockDim.x + threadIdx.x]);
          }
          __syncthreads();
     }
     __global__ void cuAbs_kernel(cytnx_int32 *out, const cytnx_int32 *ptr, const cytnx_uint64 Nelem){
          if(blockIdx.x*blockDim.x + threadIdx.x < Nelem){
              out[blockIdx.x*blockDim.x + threadIdx.x] = labs(ptr[blockIdx.x*blockDim.x + threadIdx.x]);
          }
          __syncthreads();
     }
     __global__ void cuAbs_kernel(cytnx_int16 *out, const cytnx_int16 *ptr, const cytnx_uint64 Nelem){
          if(blockIdx.x*blockDim.x + threadIdx.x < Nelem){
              out[blockIdx.x*blockDim.x + threadIdx.x] = labs(ptr[blockIdx.x*blockDim.x + threadIdx.x]);
          }
          __syncthreads();
