...
namespace tor10{
    ...
    namespace linalg_internal{
        ...
        __global__ void cuDiv_rconst_kernel(T1 *out, const T2 *ptr, const tor10_uint64 Nelem, const T3 val){
        ...
        template<class T1,class T2,class T3>
        __global__ void cuDiv_lconst_kernel(T1 *out, const T2 val, const tor10_uint64 Nelem, const T3 *ptr){
              if(blockIdx.x*blockDim.x + threadIdx.x < Nelem){
        ...
        template<class T1,class T2,class T3>
        __global__ void cuDiv_tn_kernel(T1 *out, const T2 *val, const tor10_uint64 Nelem, const T3 *ptr){
              if(blockIdx.x*blockDim.x + threadIdx.x < Nelem){
        ...
        /// cuDiv
        __global__ void cuDiv_rconst_kernel(cuDoubleComplex *out, const cuDoubleComplex *ptr, const tor10_uint64 Nelem, const cuDoubleComplex val){
            if(blockIdx.x*blockDim.x + threadIdx.x < Nelem){                                                
                out[blockIdx.x*blockDim.x + threadIdx.x] = cuCdiv(ptr[blockIdx.x*blockDim.x + threadIdx.x],val);
            }
            __syncthreads();
        }
        __global__ void cuDiv_lconst_kernel(cuDoubleComplex *out, const cuDoubleComplex val, const tor10_uint64 Nelem, const cuDoubleComplex *ptr){
            if(blockIdx.x*blockDim.x + threadIdx.x < Nelem){                                                
                out[blockIdx.x*blockDim.x + threadIdx.x] = cuCdiv(ptr[blockIdx.x*blockDim.x + threadIdx.x],val);
            }
            __syncthreads();
        }
        __global__ void cuDiv_tn_kernel(cuDoubleComplex *out, const cuDoubleComplex *ptr, const tor10_uint64 Nelem, const cuDoubleComplex *val){
            if(blockIdx.x*blockDim.x + threadIdx.x < Nelem){                                                
                out[blockIdx.x*blockDim.x + threadIdx.x] = cuCdiv(ptr[blockIdx.x*blockDim.x + threadIdx.x],val[blockIdx.x*blockDim.x + threadIdx.x]);
            }
            __syncthreads();
        }
        ...
        __global__ void cuDiv_rconst_kernel(cuDoubleComplex *out, const cuDoubleComplex *ptr, const tor10_uint64 Nelem, const cuFloatComplex val){
            if(blockIdx.x*blockDim.x + threadIdx.x < Nelem){                                                
                out[blockIdx.x*blockDim.x + threadIdx.x] = cuCdiv(ptr[blockIdx.x*blockDim.x + threadIdx.x],cuComplexFloatToDouble(val));
            }
            __syncthreads();
        }
        __global__ void cuDiv_lconst_kernel(cuDoubleComplex *out, const cuDoubleComplex val, const tor10_uint64 Nelem, const cuFloatComplex *ptr){
            if(blockIdx.x*blockDim.x + threadIdx.x < Nelem){                                                
                out[blockIdx.x*blockDim.x + threadIdx.x] = cuCdiv(val,cuComplexFloatToDouble(ptr[blockIdx.x*blockDim.x + threadIdx.x]));
            }
            __syncthreads();
        }
        __global__ void cuDiv_tn_kernel(cuDoubleComplex *out, const cuDoubleComplex *ptr, const tor10_uint64 Nelem, const cuFloatComplex *val){
            if(blockIdx.x*blockDim.x + threadIdx.x < Nelem){                                                
                out[blockIdx.x*blockDim.x + threadIdx.x] = cuCdiv(ptr[blockIdx.x*blockDim.x + threadIdx.x],cuComplexFloatToDouble(val[blockIdx.x*blockDim.x + threadIdx.x]));
            }
            __syncthreads();
        }
        ...
        __global__ void cuDiv_rconst_kernel(cuDoubleComplex *out, const cuDoubleComplex *ptr, const tor10_uint64 Nelem, const tor10_double val){
            if(blockIdx.x*blockDim.x + threadIdx.x < Nelem){                                                
                out[blockIdx.x*blockDim.x + threadIdx.x] = cuCdiv(ptr[blockIdx.x*blockDim.x + threadIdx.x],make_cuDoubleComplex(val,0));
            }
            __syncthreads();
        }
        __global__ void cuDiv_lconst_kernel(cuDoubleComplex *out, const cuDoubleComplex val, const tor10_uint64 Nelem, const tor10_double *ptr){
            if(blockIdx.x*blockDim.x + threadIdx.x < Nelem){                                                
                out[blockIdx.x*blockDim.x + threadIdx.x] = cuCdiv(val,make_cuDoubleComplex(ptr[blockIdx.x*blockDim.x + threadIdx.x],0));
            }
            __syncthreads();
        }
        __global__ void cuDiv_tn_kernel(cuDoubleComplex *out, const cuDoubleComplex *ptr, const tor10_uint64 Nelem, const tor10_double *val){
            if(blockIdx.x*blockDim.x + threadIdx.x < Nelem){                                                
        ...
        __global__ void cuDiv_rconst_kernel(cuDoubleComplex *out, const cuDoubleComplex *ptr, const tor10_uint64 Nelem, const tor10_float val){
            if(blockIdx.x*blockDim.x + threadIdx.x < Nelem){                                                
                out[blockIdx.x*blockDim.x + threadIdx.x] = cuCdiv(ptr[blockIdx.x*blockDim.x + threadIdx.x],make_cuDoubleComplex(val,0));
            }
            __syncthreads();
        }
        __global__ void cuDiv_lconst_kernel(cuDoubleComplex *out, const cuDoubleComplex val, const tor10_uint64 Nelem, const tor10_float *ptr){
            if(blockIdx.x*blockDim.x + threadIdx.x < Nelem){                                                
                out[blockIdx.x*blockDim.x + threadIdx.x] = cuCdiv(val,make_cuDoubleComplex(ptr[blockIdx.x*blockDim.x + threadIdx.x],0));
            }
            __syncthreads();
        }
        __global__ void cuDiv_tn_kernel(cuDoubleComplex *out, const cuDoubleComplex *ptr, const tor10_uint64 Nelem, const tor10_float *val){
            if(blockIdx.x*blockDim.x + threadIdx.x < Nelem){                                                
                out[blockIdx.x*blockDim.x + threadIdx.x] = cuCdiv(ptr[blockIdx.x*blockDim.x + threadIdx.x],make_cuDoubleComplex(val[blockIdx.x*blockDim.x + threadIdx.x],0));
            }
            __syncthreads();
        ...
        void cuDiv_internal_cdtf(boost::intrusive_ptr<Storage_base> & out, boost::intrusive_ptr<Storage_base> & Lin, boost::intrusive_ptr<Storage_base> & Rin, const unsigned long long &len){
          ...
          __global__ void cuDiv_rconst_kernel(cuDoubleComplex *out, const cuDoubleComplex *ptr, const tor10_uint64 Nelem, const tor10_uint64 val){
              if(blockIdx.x*blockDim.x + threadIdx.x < Nelem){
                  out[blockIdx.x*blockDim.x + threadIdx.x] = cuCdiv(ptr[blockIdx.x*blockDim.x + threadIdx.x],make_cuDoubleComplex(val,0));
              }
              __syncthreads();
          }
          __global__ void cuDiv_lconst_kernel(cuDoubleComplex *out, const cuDoubleComplex val, const tor10_uint64 Nelem, const tor10_uint64 *ptr){
              if(blockIdx.x*blockDim.x + threadIdx.x < Nelem){
                  out[blockIdx.x*blockDim.x + threadIdx.x] = cuCdiv(val,make_cuDoubleComplex(ptr[blockIdx.x*blockDim.x + threadIdx.x],0));
              }
              __syncthreads();
          }
          __global__ void cuDiv_tn_kernel(cuDoubleComplex *out, const cuDoubleComplex *ptr, const tor10_uint64 Nelem, const tor10_uint64 *val){
              if(blockIdx.x*blockDim.x + threadIdx.x < Nelem){
                  out[blockIdx.x*blockDim.x + threadIdx.x] = cuCdiv(ptr[blockIdx.x*blockDim.x + threadIdx.x],make_cuDoubleComplex(val[blockIdx.x*blockDim.x + threadIdx.x],0));
              }
              __syncthreads();
        ...
        __global__ void cuDiv_rconst_kernel(cuDoubleComplex *out, const cuDoubleComplex *ptr, const tor10_uint64 Nelem, const tor10_uint32 val){
            if(blockIdx.x*blockDim.x + threadIdx.x < Nelem){
                out[blockIdx.x*blockDim.x + threadIdx.x] = cuCdiv(ptr[blockIdx.x*blockDim.x + threadIdx.x],make_cuDoubleComplex(val,0));
            }
            __syncthreads();
        }
        __global__ void cuDiv_lconst_kernel(cuDoubleComplex *out, const cuDoubleComplex val, const tor10_uint64 Nelem, const tor10_uint32 *ptr){
            if(blockIdx.x*blockDim.x + threadIdx.x < Nelem){
                out[blockIdx.x*blockDim.x + threadIdx.x] = cuCdiv(val,make_cuDoubleComplex(ptr[blockIdx.x*blockDim.x + threadIdx.x],0));
            }
            __syncthreads();
        }
        __global__ void cuDiv_tn_kernel(cuDoubleComplex *out, const cuDoubleComplex *ptr, const tor10_uint64 Nelem, const tor10_uint32 *val){
            if(blockIdx.x*blockDim.x + threadIdx.x < Nelem){
                out[blockIdx.x*blockDim.x + threadIdx.x] = cuCdiv(ptr[blockIdx.x*blockDim.x + threadIdx.x],make_cuDoubleComplex(val[blockIdx.x*blockDim.x + threadIdx.x],0));
            }
            __syncthreads();
        ...
        __global__ void cuDiv_rconst_kernel(cuDoubleComplex *out, const cuDoubleComplex *ptr, const tor10_uint64 Nelem, const tor10_int64 val){
            if(blockIdx.x*blockDim.x + threadIdx.x < Nelem){
                out[blockIdx.x*blockDim.x + threadIdx.x] = cuCdiv(ptr[blockIdx.x*blockDim.x + threadIdx.x],make_cuDoubleComplex(val,0));
            }
            __syncthreads();
        }
        __global__ void cuDiv_lconst_kernel(cuDoubleComplex *out, const cuDoubleComplex val, const tor10_uint64 Nelem, const tor10_int64 *ptr){
            if(blockIdx.x*blockDim.x + threadIdx.x < Nelem){
                out[blockIdx.x*blockDim.x + threadIdx.x] = cuCdiv(val,make_cuDoubleComplex(ptr[blockIdx.x*blockDim.x + threadIdx.x],0));
            }
            __syncthreads();
        }
        __global__ void cuDiv_tn_kernel(cuDoubleComplex *out, const cuDoubleComplex *ptr, const tor10_uint64 Nelem, const tor10_int64 *val){
            if(blockIdx.x*blockDim.x + threadIdx.x < Nelem){
                out[blockIdx.x*blockDim.x + threadIdx.x] = cuCdiv(ptr[blockIdx.x*blockDim.x + threadIdx.x],make_cuDoubleComplex(val[blockIdx.x*blockDim.x + threadIdx.x],0));
            }
            __syncthreads();
        ...
        __global__ void cuDiv_rconst_kernel(cuDoubleComplex *out, const cuDoubleComplex *ptr, const tor10_uint64 Nelem, const tor10_int32 val){
            if(blockIdx.x*blockDim.x + threadIdx.x < Nelem){
                out[blockIdx.x*blockDim.x + threadIdx.x] = cuCdiv(ptr[blockIdx.x*blockDim.x + threadIdx.x],make_cuDoubleComplex(val,0));
            }
            __syncthreads();
        }
        __global__ void cuDiv_lconst_kernel(cuDoubleComplex *out, const cuDoubleComplex val, const tor10_uint64 Nelem, const tor10_int32 *ptr){
            if(blockIdx.x*blockDim.x + threadIdx.x < Nelem){
                out[blockIdx.x*blockDim.x + threadIdx.x] = cuCdiv(val,make_cuDoubleComplex(ptr[blockIdx.x*blockDim.x + threadIdx.x],0));
            }
            __syncthreads();
        }
        __global__ void cuDiv_tn_kernel(cuDoubleComplex *out, const cuDoubleComplex *ptr, const tor10_uint64 Nelem, const tor10_int32 *val){
            if(blockIdx.x*blockDim.x + threadIdx.x < Nelem){
                out[blockIdx.x*blockDim.x + threadIdx.x] = cuCdiv(ptr[blockIdx.x*blockDim.x + threadIdx.x],make_cuDoubleComplex(val[blockIdx.x*blockDim.x + threadIdx.x],0));
            }
            __syncthreads();
        ...
        __global__ void cuDiv_rconst_kernel(cuDoubleComplex *out, const cuFloatComplex *ptr, const tor10_uint64 Nelem, const cuDoubleComplex val){
            if(blockIdx.x*blockDim.x + threadIdx.x < Nelem){
                out[blockIdx.x*blockDim.x + threadIdx.x] = cuCdiv(cuComplexFloatToDouble(ptr[blockIdx.x*blockDim.x + threadIdx.x]),val);
            }
            __syncthreads();
        }
        __global__ void cuDiv_lconst_kernel(cuDoubleComplex *out, const cuFloatComplex val, const tor10_uint64 Nelem, const cuDoubleComplex *ptr){
            if(blockIdx.x*blockDim.x + threadIdx.x < Nelem){
                out[blockIdx.x*blockDim.x + threadIdx.x] = cuCdiv(cuComplexFloatToDouble(val),ptr[blockIdx.x*blockDim.x + threadIdx.x]);
            }
            __syncthreads();
        }
        __global__ void cuDiv_tn_kernel(cuDoubleComplex *out, const cuFloatComplex *ptr, const tor10_uint64 Nelem, const cuDoubleComplex *val){
            if(blockIdx.x*blockDim.x + threadIdx.x < Nelem){
                out[blockIdx.x*blockDim.x + threadIdx.x] = cuCdiv(cuComplexFloatToDouble(ptr[blockIdx.x*blockDim.x + threadIdx.x]),val[blockIdx.x*blockDim.x + threadIdx.x]);
            }
            __syncthreads();
        }
        ...
        __global__ void cuDiv_rconst_kernel(cuFloatComplex *out, const cuFloatComplex *ptr, const tor10_uint64 Nelem, const cuFloatComplex val){
            if(blockIdx.x*blockDim.x + threadIdx.x < Nelem){
                out[blockIdx.x*blockDim.x + threadIdx.x] = cuCdivf(ptr[blockIdx.x*blockDim.x + threadIdx.x],val);
            }
            __syncthreads();
        }
        __global__ void cuDiv_lconst_kernel(cuFloatComplex *out, const cuFloatComplex val, const tor10_uint64 Nelem, const cuFloatComplex *ptr){
            if(blockIdx.x*blockDim.x + threadIdx.x < Nelem){
                out[blockIdx.x*blockDim.x + threadIdx.x] = cuCdivf(val,ptr[blockIdx.x*blockDim.x + threadIdx.x]);
            }
            __syncthreads();
        }
        __global__ void cuDiv_tn_kernel(cuFloatComplex *out, const cuFloatComplex *ptr, const tor10_uint64 Nelem, const cuFloatComplex *val){
            if(blockIdx.x*blockDim.x + threadIdx.x < Nelem){
                out[blockIdx.x*blockDim.x + threadIdx.x] = cuCdivf(ptr[blockIdx.x*blockDim.x + threadIdx.x],val[blockIdx.x*blockDim.x + threadIdx.x]);
            }
            __syncthreads();
        }
        ...
        __global__ void cuDiv_rconst_kernel(cuFloatComplex *out, const cuFloatComplex *ptr, const tor10_uint64 Nelem, const tor10_double val){
            if(blockIdx.x*blockDim.x + threadIdx.x < Nelem){
                out[blockIdx.x*blockDim.x + threadIdx.x] = cuCdivf(ptr[blockIdx.x*blockDim.x + threadIdx.x],make_cuFloatComplex(val,0));
            }
            __syncthreads();
        }
        __global__ void cuDiv_lconst_kernel(cuFloatComplex *out, const cuFloatComplex val, const tor10_uint64 Nelem, const tor10_double *ptr){
            if(blockIdx.x*blockDim.x + threadIdx.x < Nelem){
                out[blockIdx.x*blockDim.x + threadIdx.x] = cuCdivf(val,make_cuFloatComplex(ptr[blockIdx.x*blockDim.x + threadIdx.x],0));
            }
            __syncthreads();
        }
        __global__ void cuDiv_tn_kernel(cuFloatComplex *out, const cuFloatComplex *ptr, const tor10_uint64 Nelem, const tor10_double *val){
            if(blockIdx.x*blockDim.x + threadIdx.x < Nelem){
        ...
        __global__ void cuDiv_rconst_kernel(cuFloatComplex *out, const cuFloatComplex *ptr, const tor10_uint64 Nelem, const tor10_float val){
            if(blockIdx.x*blockDim.x + threadIdx.x < Nelem){
                out[blockIdx.x*blockDim.x + threadIdx.x] = cuCdivf(ptr[blockIdx.x*blockDim.x + threadIdx.x],make_cuFloatComplex(val,0));
            }
            __syncthreads();
        }
        __global__ void cuDiv_lconst_kernel(cuFloatComplex *out, const cuFloatComplex val, const tor10_uint64 Nelem, const tor10_float *ptr){
            if(blockIdx.x*blockDim.x + threadIdx.x < Nelem){
                out[blockIdx.x*blockDim.x + threadIdx.x] = cuCdivf(val,make_cuFloatComplex(ptr[blockIdx.x*blockDim.x + threadIdx.x],0));
            }
            __syncthreads();
        }
        __global__ void cuDiv_tn_kernel(cuFloatComplex *out, const cuFloatComplex *ptr, const tor10_uint64 Nelem, const tor10_float *val){
            if(blockIdx.x*blockDim.x + threadIdx.x < Nelem){
                out[blockIdx.x*blockDim.x + threadIdx.x] = cuCdivf(ptr[blockIdx.x*blockDim.x + threadIdx.x],make_cuFloatComplex(val[blockIdx.x*blockDim.x + threadIdx.x],0));
            }
            __syncthreads();
        ...
        __global__ void cuDiv_rconst_kernel(cuFloatComplex *out, const cuFloatComplex *ptr, const tor10_uint64 Nelem, const tor10_uint64 val){
            if(blockIdx.x*blockDim.x + threadIdx.x < Nelem){
                out[blockIdx.x*blockDim.x + threadIdx.x] = cuCdivf(ptr[blockIdx.x*blockDim.x + threadIdx.x],make_cuFloatComplex(val,0));
            }
            __syncthreads();
        }
        __global__ void cuDiv_lconst_kernel(cuFloatComplex *out, const cuFloatComplex val, const tor10_uint64 Nelem, const tor10_uint64 *ptr){
            if(blockIdx.x*blockDim.x + threadIdx.x < Nelem){
                out[blockIdx.x*blockDim.x + threadIdx.x] = cuCdivf(val,make_cuFloatComplex(ptr[blockIdx.x*blockDim.x + threadIdx.x],0));
            }
            __syncthreads();
        }
        __global__ void cuDiv_tn_kernel(cuFloatComplex *out, const cuFloatComplex *ptr, const tor10_uint64 Nelem, const tor10_uint64 *val){
            if(blockIdx.x*blockDim.x + threadIdx.x < Nelem){
                out[blockIdx.x*blockDim.x + threadIdx.x] = cuCdivf(ptr[blockIdx.x*blockDim.x + threadIdx.x],make_cuFloatComplex(val[blockIdx.x*blockDim.x + threadIdx.x],0));
            }
            __syncthreads();
        ...
        __global__ void cuDiv_rconst_kernel(cuFloatComplex *out, const cuFloatComplex *ptr, const tor10_uint32 Nelem, const tor10_uint32 val){
            if(blockIdx.x*blockDim.x + threadIdx.x < Nelem){
                out[blockIdx.x*blockDim.x + threadIdx.x] = cuCdivf(ptr[blockIdx.x*blockDim.x + threadIdx.x],make_cuFloatComplex(val,0));
            }
            __syncthreads();
        }
        __global__ void cuDiv_lconst_kernel(cuFloatComplex *out, const cuFloatComplex val, const tor10_uint32 Nelem, const tor10_uint32 *ptr){
            if(blockIdx.x*blockDim.x + threadIdx.x < Nelem){
                out[blockIdx.x*blockDim.x + threadIdx.x] = cuCdivf(val,make_cuFloatComplex(ptr[blockIdx.x*blockDim.x + threadIdx.x],0));
            }
            __syncthreads();
        }
        __global__ void cuDiv_tn_kernel(cuFloatComplex *out, const cuFloatComplex *ptr, const tor10_uint32 Nelem, const tor10_uint32 *val){
            if(blockIdx.x*blockDim.x + threadIdx.x < Nelem){
                out[blockIdx.x*blockDim.x + threadIdx.x] = cuCdivf(ptr[blockIdx.x*blockDim.x + threadIdx.x],make_cuFloatComplex(val[blockIdx.x*blockDim.x + threadIdx.x],0));
            }
            __syncthreads();
        ...
        __global__ void cuDiv_rconst_kernel(cuFloatComplex *out, const cuFloatComplex *ptr, const tor10_int64 Nelem, const tor10_int64 val){
            if(blockIdx.x*blockDim.x + threadIdx.x < Nelem){
                out[blockIdx.x*blockDim.x + threadIdx.x] = cuCdivf(ptr[blockIdx.x*blockDim.x + threadIdx.x],make_cuFloatComplex(val,0));
            }
            __syncthreads();
        }
        __global__ void cuDiv_lconst_kernel(cuFloatComplex *out, const cuFloatComplex val, const tor10_int64 Nelem, const tor10_int64 *ptr){
            if(blockIdx.x*blockDim.x + threadIdx.x < Nelem){
                out[blockIdx.x*blockDim.x + threadIdx.x] = cuCdivf(val,make_cuFloatComplex(ptr[blockIdx.x*blockDim.x + threadIdx.x],0));
            }
            __syncthreads();
        }
        __global__ void cuDiv_tn_kernel(cuFloatComplex *out, const cuFloatComplex *ptr, const tor10_int64 Nelem, const tor10_int64 *val){
            if(blockIdx.x*blockDim.x + threadIdx.x < Nelem){
                out[blockIdx.x*blockDim.x + threadIdx.x] = cuCdivf(ptr[blockIdx.x*blockDim.x + threadIdx.x],make_cuFloatComplex(val[blockIdx.x*blockDim.x + threadIdx.x],0));
            }
            __syncthreads();
        ...
        __global__ void cuDiv_rconst_kernel(cuFloatComplex *out, const cuFloatComplex *ptr, const tor10_int32 Nelem, const tor10_int32 val){
            if(blockIdx.x*blockDim.x + threadIdx.x < Nelem){
                out[blockIdx.x*blockDim.x + threadIdx.x] = cuCdivf(ptr[blockIdx.x*blockDim.x + threadIdx.x],make_cuFloatComplex(val,0));
            }
            __syncthreads();
        }
        __global__ void cuDiv_lconst_kernel(cuFloatComplex *out, const cuFloatComplex val, const tor10_int32 Nelem, const tor10_int32 *ptr){
            if(blockIdx.x*blockDim.x + threadIdx.x < Nelem){
                out[blockIdx.x*blockDim.x + threadIdx.x] = cuCdivf(val,make_cuFloatComplex(ptr[blockIdx.x*blockDim.x + threadIdx.x],0));
            }
            __syncthreads();
        }
        __global__ void cuDiv_tn_kernel(cuFloatComplex *out, const cuFloatComplex *ptr, const tor10_int32 Nelem, const tor10_int32 *val){
            if(blockIdx.x*blockDim.x + threadIdx.x < Nelem){
                out[blockIdx.x*blockDim.x + threadIdx.x] = cuCdivf(ptr[blockIdx.x*blockDim.x + threadIdx.x],make_cuFloatComplex(val[blockIdx.x*blockDim.x + threadIdx.x],0));
            }
            __syncthreads();
        ...
        __global__ void cuDiv_rconst_kernel(cuDoubleComplex *out, const tor10_double *ptr, const tor10_uint64 Nelem, const cuDoubleComplex val){
            if(blockIdx.x*blockDim.x + threadIdx.x < Nelem){
                out[blockIdx.x*blockDim.x + threadIdx.x] = cuCdiv(make_cuDoubleComplex(ptr[blockIdx.x*blockDim.x + threadIdx.x],0),val);
            }
            __syncthreads();
        }
        __global__ void cuDiv_lconst_kernel(cuDoubleComplex *out, const tor10_double val, const tor10_uint64 Nelem, const cuDoubleComplex *ptr){
            if(blockIdx.x*blockDim.x + threadIdx.x < Nelem){
                out[blockIdx.x*blockDim.x + threadIdx.x] = cuCdiv(make_cuDoubleComplex(val,0),ptr[blockIdx.x*blockDim.x + threadIdx.x]);
            }
            __syncthreads();
        }
        __global__ void cuDiv_tn_kernel(cuDoubleComplex *out, const tor10_double *ptr, const tor10_uint64 Nelem, const cuDoubleComplex *val){
            if(blockIdx.x*blockDim.x + threadIdx.x < Nelem){
        ...
        __global__ void cuDiv_rconst_kernel(cuFloatComplex *out, const tor10_double *ptr, const tor10_uint64 Nelem, const cuFloatComplex val){
            if(blockIdx.x*blockDim.x + threadIdx.x < Nelem){
                out[blockIdx.x*blockDim.x + threadIdx.x] = cuCdivf(make_cuFloatComplex(ptr[blockIdx.x*blockDim.x + threadIdx.x],0),val);
            }
            __syncthreads();
        }
        __global__ void cuDiv_lconst_kernel(cuFloatComplex *out, const tor10_double val, const tor10_uint64 Nelem, const cuFloatComplex *ptr){
            if(blockIdx.x*blockDim.x + threadIdx.x < Nelem){
                out[blockIdx.x*blockDim.x + threadIdx.x] = cuCdivf(make_cuFloatComplex(val,0),ptr[blockIdx.x*blockDim.x + threadIdx.x]);
            }
            __syncthreads();
        }
        __global__ void cuDiv_tn_kernel(cuFloatComplex *out, const tor10_double *ptr, const tor10_uint64 Nelem, const cuFloatComplex *val){
            if(blockIdx.x*blockDim.x + threadIdx.x < Nelem){
        ...
        __global__ void cuDiv_rconst_kernel(cuDoubleComplex *out, const tor10_float *ptr, const tor10_uint64 Nelem, const cuDoubleComplex val){
            if(blockIdx.x*blockDim.x + threadIdx.x < Nelem){
                out[blockIdx.x*blockDim.x + threadIdx.x] = cuCdiv(make_cuDoubleComplex(ptr[blockIdx.x*blockDim.x + threadIdx.x],0),val);
            }
            __syncthreads();
        }
        __global__ void cuDiv_lconst_kernel(cuDoubleComplex *out, const tor10_float val, const tor10_uint64 Nelem, const cuDoubleComplex *ptr){
            if(blockIdx.x*blockDim.x + threadIdx.x < Nelem){
                out[blockIdx.x*blockDim.x + threadIdx.x] = cuCdiv(make_cuDoubleComplex(val,0),ptr[blockIdx.x*blockDim.x + threadIdx.x]);
            }
            __syncthreads();
        }
        __global__ void cuDiv_tn_kernel(cuDoubleComplex *out, const tor10_float *ptr, const tor10_uint64 Nelem, const cuDoubleComplex *val){
            if(blockIdx.x*blockDim.x + threadIdx.x < Nelem){
                out[blockIdx.x*blockDim.x + threadIdx.x] = cuCdiv(make_cuDoubleComplex(ptr[blockIdx.x*blockDim.x + threadIdx.x],0),val[blockIdx.x*blockDim.x + threadIdx.x]);
            }
            __syncthreads();
        ...
        __global__ void cuDiv_rconst_kernel(cuFloatComplex *out, const tor10_float *ptr, const tor10_uint64 Nelem, const cuFloatComplex val){
            if(blockIdx.x*blockDim.x + threadIdx.x < Nelem){
                out[blockIdx.x*blockDim.x + threadIdx.x] = cuCdivf(make_cuFloatComplex(ptr[blockIdx.x*blockDim.x + threadIdx.x],0),val);
            }
            __syncthreads();
        }
        __global__ void cuDiv_lconst_kernel(cuFloatComplex *out, const tor10_float val, const tor10_uint64 Nelem, const cuFloatComplex *ptr){
            if(blockIdx.x*blockDim.x + threadIdx.x < Nelem){
                out[blockIdx.x*blockDim.x + threadIdx.x] = cuCdivf(make_cuFloatComplex(val,0),ptr[blockIdx.x*blockDim.x + threadIdx.x]);
            }
            __syncthreads();
        }
        __global__ void cuDiv_tn_kernel(cuFloatComplex *out, const tor10_float *ptr, const tor10_uint64 Nelem, const cuFloatComplex *val){
            if(blockIdx.x*blockDim.x + threadIdx.x < Nelem){
                out[blockIdx.x*blockDim.x + threadIdx.x] = cuCdivf(make_cuFloatComplex(ptr[blockIdx.x*blockDim.x + threadIdx.x],0),val[blockIdx.x*blockDim.x + threadIdx.x]);
            }
            __syncthreads();
        ...
        __global__ void cuDiv_rconst_kernel(cuDoubleComplex *out, const tor10_int64 *ptr, const tor10_uint64 Nelem, const cuDoubleComplex val){
            if(blockIdx.x*blockDim.x + threadIdx.x < Nelem){
                out[blockIdx.x*blockDim.x + threadIdx.x] = cuCdiv(make_cuDoubleComplex(ptr[blockIdx.x*blockDim.x + threadIdx.x],0),val);
            }
            __syncthreads();
        }
        __global__ void cuDiv_lconst_kernel(cuDoubleComplex *out, const tor10_int64 val, const tor10_uint64 Nelem, const cuDoubleComplex *ptr){
            if(blockIdx.x*blockDim.x + threadIdx.x < Nelem){
                out[blockIdx.x*blockDim.x + threadIdx.x] = cuCdiv(make_cuDoubleComplex(val,0),ptr[blockIdx.x*blockDim.x + threadIdx.x]);
            }
            __syncthreads();
        }
        __global__ void cuDiv_tn_kernel(cuDoubleComplex *out, const tor10_int64 *ptr, const tor10_uint64 Nelem, const cuDoubleComplex *val){
            if(blockIdx.x*blockDim.x + threadIdx.x < Nelem){
                out[blockIdx.x*blockDim.x + threadIdx.x] = cuCdiv(make_cuDoubleComplex(ptr[blockIdx.x*blockDim.x + threadIdx.x],0),val[blockIdx.x*blockDim.x + threadIdx.x]);
            }
            __syncthreads();
        ...
        __global__ void cuDiv_rconst_kernel(cuFloatComplex *out, const tor10_int64 *ptr, const tor10_uint64 Nelem, const cuFloatComplex val){
            if(blockIdx.x*blockDim.x + threadIdx.x < Nelem){
                out[blockIdx.x*blockDim.x + threadIdx.x] = cuCdivf(make_cuFloatComplex(ptr[blockIdx.x*blockDim.x + threadIdx.x],0),val);
            }
            __syncthreads();
        }
        __global__ void cuDiv_lconst_kernel(cuFloatComplex *out, const tor10_int64 val, const tor10_uint64 Nelem, const cuFloatComplex *ptr){
            if(blockIdx.x*blockDim.x + threadIdx.x < Nelem){
                out[blockIdx.x*blockDim.x + threadIdx.x] = cuCdivf(make_cuFloatComplex(val,0),ptr[blockIdx.x*blockDim.x + threadIdx.x]);
            }
            __syncthreads();
        }
        __global__ void cuDiv_tn_kernel(cuFloatComplex *out, const tor10_int64 *ptr, const tor10_uint64 Nelem, const cuFloatComplex *val){
            if(blockIdx.x*blockDim.x + threadIdx.x < Nelem){
                out[blockIdx.x*blockDim.x + threadIdx.x] = cuCdivf(make_cuFloatComplex(ptr[blockIdx.x*blockDim.x + threadIdx.x],0),val[blockIdx.x*blockDim.x + threadIdx.x]);
            }
            __syncthreads();
        ...
        __global__ void cuDiv_rconst_kernel(cuDoubleComplex *out, const tor10_uint64 *ptr, const tor10_uint64 Nelem, const cuDoubleComplex val){
            if(blockIdx.x*blockDim.x + threadIdx.x < Nelem){
                out[blockIdx.x*blockDim.x + threadIdx.x] = cuCdiv(make_cuDoubleComplex(ptr[blockIdx.x*blockDim.x + threadIdx.x],0),val);
            }
            __syncthreads();
        }
        __global__ void cuDiv_lconst_kernel(cuDoubleComplex *out, const tor10_uint64 val, const tor10_uint64 Nelem, const cuDoubleComplex *ptr){
            if(blockIdx.x*blockDim.x + threadIdx.x < Nelem){
                out[blockIdx.x*blockDim.x + threadIdx.x] = cuCdiv(make_cuDoubleComplex(val,0),ptr[blockIdx.x*blockDim.x + threadIdx.x]);
            }
            __syncthreads();
        }
        __global__ void cuDiv_tn_kernel(cuDoubleComplex *out, const tor10_uint64 *ptr, const tor10_uint64 Nelem, const cuDoubleComplex *val){
            if(blockIdx.x*blockDim.x + threadIdx.x < Nelem){
                out[blockIdx.x*blockDim.x + threadIdx.x] = cuCdiv(make_cuDoubleComplex(ptr[blockIdx.x*blockDim.x + threadIdx.x],0),val[blockIdx.x*blockDim.x + threadIdx.x]);
            }
            __syncthreads();
        ...
        __global__ void cuDiv_rconst_kernel(cuFloatComplex *out, const tor10_uint64 *ptr, const tor10_uint64 Nelem, const cuFloatComplex val){
            if(blockIdx.x*blockDim.x + threadIdx.x < Nelem){
                out[blockIdx.x*blockDim.x + threadIdx.x] = cuCdivf(make_cuFloatComplex(ptr[blockIdx.x*blockDim.x + threadIdx.x],0),val);
            }
            __syncthreads();
        }
        __global__ void cuDiv_lconst_kernel(cuFloatComplex *out, const tor10_uint64 val, const tor10_uint64 Nelem, const cuFloatComplex *ptr){
            if(blockIdx.x*blockDim.x + threadIdx.x < Nelem){
                out[blockIdx.x*blockDim.x + threadIdx.x] = cuCdivf(make_cuFloatComplex(val,0),ptr[blockIdx.x*blockDim.x + threadIdx.x]);
            }
            __syncthreads();
        }
        __global__ void cuDiv_tn_kernel(cuFloatComplex *out, const tor10_uint64 *ptr, const tor10_uint64 Nelem, const cuFloatComplex *val){
            if(blockIdx.x*blockDim.x + threadIdx.x < Nelem){
                out[blockIdx.x*blockDim.x + threadIdx.x] = cuCdivf(make_cuFloatComplex(ptr[blockIdx.x*blockDim.x + threadIdx.x],0),val[blockIdx.x*blockDim.x + threadIdx.x]);
            }
            __syncthreads();
        ...
        __global__ void cuDiv_rconst_kernel(cuDoubleComplex *out, const tor10_int32 *ptr, const tor10_uint64 Nelem, const cuDoubleComplex val){
            if(blockIdx.x*blockDim.x + threadIdx.x < Nelem){
                out[blockIdx.x*blockDim.x + threadIdx.x] = cuCdiv(make_cuDoubleComplex(ptr[blockIdx.x*blockDim.x + threadIdx.x],0),val);
            }
            __syncthreads();
        }
        __global__ void cuDiv_lconst_kernel(cuDoubleComplex *out, const tor10_int32 val, const tor10_uint64 Nelem, const cuDoubleComplex *ptr){
            if(blockIdx.x*blockDim.x + threadIdx.x < Nelem){
                out[blockIdx.x*blockDim.x + threadIdx.x] = cuCdiv(make_cuDoubleComplex(val,0),ptr[blockIdx.x*blockDim.x + threadIdx.x]);
            }
            __syncthreads();
        }
        __global__ void cuDiv_tn_kernel(cuDoubleComplex *out, const tor10_int32 *ptr, const tor10_uint64 Nelem, const cuDoubleComplex *val){
            if(blockIdx.x*blockDim.x + threadIdx.x < Nelem){
                out[blockIdx.x*blockDim.x + threadIdx.x] = cuCdiv(make_cuDoubleComplex(ptr[blockIdx.x*blockDim.x + threadIdx.x],0),val[blockIdx.x*blockDim.x + threadIdx.x]);
            }
            __syncthreads();
        ...
        __global__ void cuDiv_rconst_kernel(cuFloatComplex *out, const tor10_int32 *ptr, const tor10_uint64 Nelem, const cuFloatComplex val){
            if(blockIdx.x*blockDim.x + threadIdx.x < Nelem){
                out[blockIdx.x*blockDim.x + threadIdx.x] = cuCdivf(make_cuFloatComplex(ptr[blockIdx.x*blockDim.x + threadIdx.x],0),val);
            }
            __syncthreads();
        }
        __global__ void cuDiv_lconst_kernel(cuFloatComplex *out, const tor10_int32 val, const tor10_uint64 Nelem, const cuFloatComplex *ptr){
            if(blockIdx.x*blockDim.x + threadIdx.x < Nelem){
                out[blockIdx.x*blockDim.x + threadIdx.x] = cuCdivf(make_cuFloatComplex(val,0),ptr[blockIdx.x*blockDim.x + threadIdx.x]);
            }
            __syncthreads();
        }
        __global__ void cuDiv_tn_kernel(cuFloatComplex *out, const tor10_int32 *ptr, const tor10_uint64 Nelem, const cuFloatComplex *val){
            if(blockIdx.x*blockDim.x + threadIdx.x < Nelem){
                out[blockIdx.x*blockDim.x + threadIdx.x] = cuCdivf(make_cuFloatComplex(ptr[blockIdx.x*blockDim.x + threadIdx.x],0),val[blockIdx.x*blockDim.x + threadIdx.x]);
            }
            __syncthreads();
        ...
        __global__ void cuDiv_rconst_kernel(cuDoubleComplex *out, const tor10_uint32 *ptr, const tor10_uint64 Nelem, const cuDoubleComplex val){
            if(blockIdx.x*blockDim.x + threadIdx.x < Nelem){
                out[blockIdx.x*blockDim.x + threadIdx.x] = cuCdiv(make_cuDoubleComplex(ptr[blockIdx.x*blockDim.x + threadIdx.x],0),val);
            }
            __syncthreads();
        }
        __global__ void cuDiv_lconst_kernel(cuDoubleComplex *out, const tor10_uint32 val, const tor10_uint64 Nelem, const cuDoubleComplex *ptr){
            if(blockIdx.x*blockDim.x + threadIdx.x < Nelem){
                out[blockIdx.x*blockDim.x + threadIdx.x] = cuCdiv(make_cuDoubleComplex(val,0),ptr[blockIdx.x*blockDim.x + threadIdx.x]);
            }
            __syncthreads();
        }
        __global__ void cuDiv_tn_kernel(cuDoubleComplex *out, const tor10_uint32 *ptr, const tor10_uint64 Nelem, const cuDoubleComplex *val){
            if(blockIdx.x*blockDim.x + threadIdx.x < Nelem){
                out[blockIdx.x*blockDim.x + threadIdx.x] = cuCdiv(make_cuDoubleComplex(ptr[blockIdx.x*blockDim.x + threadIdx.x],0),val[blockIdx.x*blockDim.x + threadIdx.x]);
            }
            __syncthreads();
        ...
        __global__ void cuDiv_rconst_kernel(cuFloatComplex *out, const tor10_uint32 *ptr, const tor10_uint64 Nelem, const cuFloatComplex val){
            if(blockIdx.x*blockDim.x + threadIdx.x < Nelem){
                out[blockIdx.x*blockDim.x + threadIdx.x] = cuCdivf(make_cuFloatComplex(ptr[blockIdx.x*blockDim.x + threadIdx.x],0),val);
            }
            __syncthreads();
        }
        __global__ void cuDiv_lconst_kernel(cuFloatComplex *out, const tor10_uint32 val, const tor10_uint64 Nelem, const cuFloatComplex *ptr){
            if(blockIdx.x*blockDim.x + threadIdx.x < Nelem){
                out[blockIdx.x*blockDim.x + threadIdx.x] = cuCdivf(make_cuFloatComplex(val,0),ptr[blockIdx.x*blockDim.x + threadIdx.x]);
            }
            __syncthreads();
        }
        __global__ void cuDiv_tn_kernel(cuFloatComplex *out, const tor10_uint32 *ptr, const tor10_uint64 Nelem, const cuFloatComplex *val){
            if(blockIdx.x*blockDim.x + threadIdx.x < Nelem){
                out[blockIdx.x*blockDim.x + threadIdx.x] = cuCdivf(make_cuFloatComplex(ptr[blockIdx.x*blockDim.x + threadIdx.x],0),val[blockIdx.x*blockDim.x + threadIdx.x]);
            }
            __syncthreads();
