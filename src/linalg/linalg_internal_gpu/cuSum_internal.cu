...
namespace cytnx{
    ...
    namespace linalg_internal{
        ...
        void cuSum_internal_cd(boost::intrusive_ptr<Storage_base> &out,
                          ...
                          const char &type){
            utils_internal::cuReduce_gpu_cd((cytnx_complex128*)out->Mem,(cytnx_complex128*)ten->Mem,Nelem);
        }
        void cuSum_internal_cf(boost::intrusive_ptr<Storage_base> &out,
                          const boost::intrusive_ptr<Storage_base> &ten, const cytnx_uint64 &Nelem,
                          const char &type){
            utils_internal::cuReduce_gpu_cf((cytnx_complex64*)out->Mem,(cytnx_complex64*)ten->Mem,Nelem);
        }
