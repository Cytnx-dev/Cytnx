...
namespace cytnx{
    ...
    namespace linalg_internal{
        ...
        void cuDet_internal_cd(void* out, const boost::intrusive_ptr<Storage_base>& in,
            ...
            cuDoubleComplex* _in = (cuDoubleComplex*)utils_internal::cuMalloc_gpu(in->len*sizeof(cuDoubleComplex)); // unify mem.
            checkCudaErrors(cudaMemcpy(_in,in->Mem,sizeof(cytnx_complex128)*in->len,cudaMemcpyDeviceToDevice));
            
            magma_int_t* ipiv; magma_imalloc_cpu(&ipiv,L+1);
            magma_int_t N = L;
            magma_int_t info;
            ...
            #else
                cytnx_error_msg(true,"[ERROR][internal Det] Det for Tensor on GPU require magma. please re-compiling cytnx with magma.%s","\n");
            #endif
        ...
        void cuDet_internal_cf(void* out, const boost::intrusive_ptr<Storage_base>& in,
            ...
            cuFloatComplex* _in = (cuFloatComplex*)utils_internal::cuMalloc_gpu(in->len*sizeof(cuFloatComplex)); // unify mem.
            checkCudaErrors(cudaMemcpy(_in,in->Mem,sizeof(cytnx_complex64)*in->len,cudaMemcpyDeviceToDevice));
        ...
        void cuDet_internal_d(void* out, const boost::intrusive_ptr<Storage_base>& in,
            ...
            cytnx_double* _in = (cytnx_double*)utils_internal::cuMalloc_gpu(in->len*sizeof(cytnx_double)); // unify mem.
            checkCudaErrors(cudaMemcpy(_in,in->Mem,sizeof(cytnx_double)*in->len,cudaMemcpyDeviceToDevice));
        ...
        void cuDet_internal_f(void* out, const boost::intrusive_ptr<Storage_base>& in,
            ...
            cytnx_float* _in = (cytnx_float*)utils_internal::cuMalloc_gpu(in->len*sizeof(cytnx_float)); // unify mem.
            checkCudaErrors(cudaMemcpy(_in,in->Mem,sizeof(cytnx_float)*in->len,cudaMemcpyDeviceToDevice));
