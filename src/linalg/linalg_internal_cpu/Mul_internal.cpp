  ...
  #pragma omp parallel for schedule(dynamic)
    ...
    void Mul_internal_cdtb(boost::intrusive_ptr<Storage_base> &out,
      ...
      if (Lin->size() == 1) {
        memset(
          _out, 0,
          sizeof(cytnx_complex128) * len);  // since Rin cannot be the same as out, set to zero.
#ifdef UNI_OMP
  #pragma omp parallel for schedule(dynamic)
#endif
        for (unsigned long long i = 0; i < len; i++) {
          if (_Rin[i]) _out[i] = _Lin[0];
        }
      } else if (Rin->size() == 1) {
        if (_Rin[0])
          memcpy(_out, _Lin,
                 sizeof(cytnx_complex128) * len);  // since Lin can be the same as out, copy Lin.
        else
...
// handle non-contiguous
  ...
  #pragma omp parallel for schedule(dynamic)
    ...
    void Mul_internal_cftb(boost::intrusive_ptr<Storage_base> &out,
      ...
      if (Lin->size() == 1) {
        memset(_out, 0,
               sizeof(cytnx_complex64) * len);  // since Rin cannot be the same as out, set to zero.
#ifdef UNI_OMP
  #pragma omp parallel for schedule(dynamic)
#endif
        for (unsigned long long i = 0; i < len; i++) {
        ...
        if (_Rin[0])
          memcpy(_out, _Lin,
                 sizeof(cytnx_complex64) * len);  // since Lin can be the same as out, copy Lin.
        else
...
// handle non-contiguous
  ...
  #pragma omp parallel for schedule(dynamic)
    ...
    void Mul_internal_dtb(boost::intrusive_ptr<Storage_base> &out,
      ...
      if (Lin->size() == 1) {
        memset(_out, 0,
               sizeof(cytnx_double) * len);  // since Rin cannot be the same as out, set to zero.
#ifdef UNI_OMP
  #pragma omp parallel for schedule(dynamic)
#endif
        for (unsigned long long i = 0; i < len; i++) {
        ...
        if (_Rin[0])
          memcpy(_out, _Lin,
                 sizeof(cytnx_double) * len);  // since Lin can be the same as out, copy Lin.
        else
...
// handle non-contiguous
  ...
  #pragma omp parallel for schedule(dynamic)
    ...
    void Mul_internal_ftb(boost::intrusive_ptr<Storage_base> &out,
      ...
      if (Lin->size() == 1) {
        memset(_out, 0,
               sizeof(cytnx_float) * len);  // since Rin cannot be the same as out, set to zero.
#ifdef UNI_OMP
  #pragma omp parallel for schedule(dynamic)
#endif
        for (unsigned long long i = 0; i < len; i++) {
        ...
        if (_Rin[0])
          memcpy(_out, _Lin,
                 sizeof(cytnx_float) * len);  // since Lin can be the same as out, copy Lin.
        else
...
// handle non-contiguous
  ...
  #pragma omp parallel for schedule(dynamic)
    ...
    void Mul_internal_i64tb(boost::intrusive_ptr<Storage_base> &out,
      ...
      if (Lin->size() == 1) {
        memset(_out, 0,
               sizeof(cytnx_int64) * len);  // since Rin cannot be the same as out, set to zero.
#ifdef UNI_OMP
  #pragma omp parallel for schedule(dynamic)
#endif
        for (unsigned long long i = 0; i < len; i++) {
        ...
        if (_Rin[0])
          memcpy(_out, _Lin,
                 sizeof(cytnx_int64) * len);  // since Lin can be the same as out, copy Lin.
        else
...
// handle non-contiguous
  ...
  #pragma omp parallel for schedule(dynamic)
    ...
    void Mul_internal_u64tb(boost::intrusive_ptr<Storage_base> &out,
      ...
      if (Lin->size() == 1) {
        memset(_out, 0,
               sizeof(cytnx_uint64) * len);  // since Rin cannot be the same as out, set to zero.
#ifdef UNI_OMP
  #pragma omp parallel for schedule(dynamic)
#endif
        for (unsigned long long i = 0; i < len; i++) {
        ...
        if (_Rin[0])
          memcpy(_out, _Lin,
                 sizeof(cytnx_uint64) * len);  // since Lin can be the same as out, copy Lin.
        else
...
// handle non-contiguous
  ...
  #pragma omp parallel for schedule(dynamic)
    ...
    void Mul_internal_i32tb(boost::intrusive_ptr<Storage_base> &out,
      ...
      if (Lin->size() == 1) {
        memset(_out, 0,
               sizeof(cytnx_int32) * len);  // since Rin cannot be the same as out, set to zero.
#ifdef UNI_OMP
  #pragma omp parallel for schedule(dynamic)
#endif
        for (unsigned long long i = 0; i < len; i++) {
        ...
        if (_Rin[0])
          memcpy(_out, _Lin,
                 sizeof(cytnx_int32) * len);  // since Lin can be the same as out, copy Lin.
        else
...
// handle non-contiguous
  ...
  #pragma omp parallel for schedule(dynamic)
    ...
    void Mul_internal_u32tb(boost::intrusive_ptr<Storage_base> &out,
      ...
      if (Lin->size() == 1) {
        memset(_out, 0,
               sizeof(cytnx_uint32) * len);  // since Rin cannot be the same as out, set to zero.
#ifdef UNI_OMP
  #pragma omp parallel for schedule(dynamic)
#endif
        for (unsigned long long i = 0; i < len; i++) {
        ...
        if (_Rin[0])
          memcpy(_out, _Lin,
                 sizeof(cytnx_uint32) * len);  // since Lin can be the same as out, copy Lin.
        else
...
// handle non-contiguous
  ...
  #pragma omp parallel for schedule(dynamic)
    ...
    void Mul_internal_i16tb(boost::intrusive_ptr<Storage_base> &out,
      ...
      if (Lin->size() == 1) {
        memset(_out, 0,
               sizeof(cytnx_int16) * len);  // since Rin cannot be the same as out, set to zero.
#ifdef UNI_OMP
  #pragma omp parallel for schedule(dynamic)
#endif
        for (unsigned long long i = 0; i < len; i++) {
        ...
        if (_Rin[0])
          memcpy(_out, _Lin,
                 sizeof(cytnx_int16) * len);  // since Lin can be the same as out, copy Lin.
        else
...
// handle non-contiguous
  ...
  #pragma omp parallel for schedule(dynamic)
    ...
    void Mul_internal_u16tb(boost::intrusive_ptr<Storage_base> &out,
      ...
      if (Lin->size() == 1) {
        memset(_out, 0,
               sizeof(cytnx_uint16) * len);  // since Rin cannot be the same as out, set to zero.
#ifdef UNI_OMP
  #pragma omp parallel for schedule(dynamic)
#endif
        for (unsigned long long i = 0; i < len; i++) {
        ...
        if (_Rin[0])
          memcpy(_out, _Lin,
                 sizeof(cytnx_uint16) * len);  // since Lin can be the same as out, copy Lin.
        else
