  ...
  namespace linalg_internal {
    ...
    void Sdd_internal_cd(const boost::intrusive_ptr<Storage_base> &in,
      ...
      info = LAPACKE_zgesdd(LAPACK_COL_MAJOR, jobz, N, M, (lapack_complex_double *)Mij, ldA,
                            ...
                            (lapack_complex_double *)U->Mem, ldvT);

      cytnx_error_msg(info != 0, "%s %d",
                      "Error in Lapack function 'zgesdd': Lapack INFO = ", info);
    ...
    void Sdd_internal_cf(const boost::intrusive_ptr<Storage_base> &in,
      ...
      info = LAPACKE_cgesdd(LAPACK_COL_MAJOR, jobz, N, M, (lapack_complex_float *)Mij, ldA,
                            ...
                            (lapack_complex_float *)U->Mem, ldvT);

      cytnx_error_msg(info != 0, "%s %d",
                      "Error in Lapack function 'cgesdd': Lapack INFO = ", info);
    ...
    void Sdd_internal_d(const boost::intrusive_ptr<Storage_base> &in,
      ...
      //double *superb = (double *)malloc(sizeof(double) * (min - 1));
      info = LAPACKE_dgesdd(LAPACK_COL_MAJOR, jobz, N, M, Mij, ldA, (cytnx_double *)S->Mem,
                            (cytnx_double *)vT->Mem, ldu, (cytnx_double *)U->Mem, ldvT);

      cytnx_error_msg(info != 0, "%s %d",
                      "Error in Lapack function 'dgesdd': Lapack INFO = ", info);
    ...
    void Sdd_internal_f(const boost::intrusive_ptr<Storage_base> &in,
      ...
      //float *superb = (float *)malloc(sizeof(float) * (min - 1));
      info = LAPACKE_sgesdd(LAPACK_COL_MAJOR, jobz, N, M, Mij, ldA, (cytnx_float *)S->Mem,
                            (cytnx_float *)vT->Mem, ldu, (cytnx_float *)U->Mem, ldvT);

      cytnx_error_msg(info != 0, "%s %d",
                      "Error in Lapack function 'sgesdd': Lapack INFO = ", info);
