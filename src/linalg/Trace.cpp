...
namespace cytnx{
    ...
    template<class T>
    void _trace_nd(Tensor &out, const Tensor &Tn, const cytnx_uint64 &Ndiag, const cytnx_uint64 &Nelem, const vector<cytnx_uint64> &accu, const vector<cytnx_uint64> &remain_rank_id, const vector<cytnx_int64> &shape, const cytnx_uint64 &ax1, const cytnx_uint64 &ax2){
    ...
    void _trace_2d_para(Tensor &out, const Tensor &Tn, const cytnx_uint64 &Ndiag,const int &Nomp){
        ...
        for(cytnx_uint64 i=0;i<Ndiag;i++)
            buffer[omp_get_thread_num()] += Tn.at<T>({i,i});
    ...
    template<class T>
    void _trace_nd_para(Tensor &out, const Tensor &Tn, const cytnx_uint64 &Ndiag, const cytnx_uint64 &Nelem, const vector<cytnx_uint64> &accu, const vector<cytnx_uint64> &remain_rank_id, const vector<cytnx_int64> &shape, const cytnx_uint64 &ax1, const cytnx_uint64 &ax2, const int &Nomp){
        ...
        if(Nelem < Ndiag){
            ...
            for(cytnx_uint64 i=0;i<Nelem;i++){
                ...
                //calculate indexer        
                for(int x=0;x<shape.size();x++){
                    //cout << "accux"<< accu[x] << endl;                   
                    //cout << "rri" << remain_rank_id[x] << endl; 
                    #pragma omp parallel
                    {
                        //printf("%d] rri: %d] tmp:%d] rhs %d]\n",omp_get_thread_num(),remain_rank_id[x],tmp,cytnx_uint64(tmp/accu[x]));
                        indexers[omp_get_thread_num()][remain_rank_id[x]] = cytnx_uint64(tmp/accu[x]);
                    }
                    tmp %= accu[x];
                }
                ...
                memset(&buffer[0],0,sizeof(T)*buffer.size());
                
                #pragma omp parallel for schedule(dynamic)
                for(cytnx_uint64 d=0;d<Ndiag;d++){
                    indexers[omp_get_thread_num()][ax1] = indexers[omp_get_thread_num()][ax2] = d;
                    buffer[omp_get_thread_num()] += Tn.at<T>(indexers[omp_get_thread_num()]);
                }

                //for(int z=0;z<Nomp;z++) cout << buffer[z] << " ";
                //cout << endl;
                
                for(int j=1;j<Nomp;j++)
                    buffer[0] += buffer[j];
                
                out.storage().at<T>(i) = buffer[0];
    ...
    namespace linalg{
        ...
        Tensor Trace(const Tensor &Tn, const cytnx_uint64 &axisA, const cytnx_uint64 &axisB){
            ...
            if(axisA < axisB){ax1 = axisA; ax2 = axisB;}
            else{ax1 = axisB; ax2 = axisA;}
            ...
            shape.erase(shape.begin()+ax2); shape.erase(shape.begin()+ax1);
                ...
                if(shape.size()==0){
                    switch(Tn.dtype()){
                        ...
                        case Type.Bool:
                             ...
                             cytnx_error_msg(true,"[ERROR][Trace] Bool type cannot perform Trace, use .astype() to promote first.%s","\n");
                             break;
                        default:
                    ...
                    switch(Tn.dtype()){
                        case Type.ComplexDouble:
                            _trace_nd_para<cytnx_complex128>(out,Tn,Ndiag,Nelem,accu,remain_rank_id,shape,ax1,ax2,Nomp);
                            break;
                        case Type.ComplexFloat:
                            _trace_nd_para<cytnx_complex64>(out,Tn,Ndiag,Nelem,accu,remain_rank_id,shape,ax1,ax2,Nomp);
                            break;
                        case Type.Double:
                            _trace_nd_para<cytnx_double>(out,Tn,Ndiag,Nelem,accu,remain_rank_id,shape,ax1,ax2,Nomp);
                            break;
                        case Type.Float:
                            _trace_nd_para<cytnx_float>(out,Tn,Ndiag,Nelem,accu,remain_rank_id,shape,ax1,ax2,Nomp);
                            break;
                        case Type.Uint64:
                            _trace_nd_para<cytnx_uint64>(out,Tn,Ndiag,Nelem,accu,remain_rank_id,shape,ax1,ax2,Nomp);
                            break;
                        case Type.Int64:
                            _trace_nd_para<cytnx_int64>(out,Tn,Ndiag,Nelem,accu,remain_rank_id,shape,ax1,ax2,Nomp);
                            break;
                        case Type.Int32:
                            _trace_nd_para<cytnx_int32>(out,Tn,Ndiag,Nelem,accu,remain_rank_id,shape,ax1,ax2,Nomp);
                            break;
                        case Type.Uint32:
                            _trace_nd_para<cytnx_uint32>(out,Tn,Ndiag,Nelem,accu,remain_rank_id,shape,ax1,ax2,Nomp);
                            break;
                        case Type.Uint16:
                            _trace_nd_para<cytnx_uint16>(out,Tn,Ndiag,Nelem,accu,remain_rank_id,shape,ax1,ax2,Nomp);
                            break;
                        case Type.Int16:
                            _trace_nd_para<cytnx_int16>(out,Tn,Ndiag,Nelem,accu,remain_rank_id,shape,ax1,ax2,Nomp);
                            break;
                        case Type.Bool:
                            cytnx_error_msg(true,"[ERROR][Trace] Bool type cannot perform Trace, use .astype() to promote first.%s","\n");
                            break;
