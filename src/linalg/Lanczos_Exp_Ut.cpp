...
namespace cytnx {
  namespace linalg {
    ...
    UniTensor Invert_BiCGSTAB(LinOp *Hop, const UniTensor &b, const UniTensor& Tin, const int& k,
							 const double &CvgCrit = 1.0e-12, const unsigned int &Maxiter = 10000) {
    ...
    void _Lanczos_Exp_Ut(UniTensor &out, LinOp *Hop, const UniTensor &Tin,
	  ...
	  auto Op_apprx_norm = static_cast<double>(Hop->matvec(Tin).get_block_().flatten().Norm().item().real());
	  ...
	  // For i = 0 to k
	  for (int i = 0; i <= k; ++i) {
		// Call the procedure Invert_A (v[i], k, eps1). The procedure returns a vector w[i], such that, 
	  ...
	  //Let T_k be the (k + 1) × (k + 1) matrix a[i,j] i,j is {0,...,k} and Tk_hat = 1 / 2 (Tk^Dagger  + Tk).
	   ...
	   *
	   */
	  auto B = UniTensor(B_mat, false, 1, 
	          {"cytnx_internal_label_kl", "cytnx_internal_label_kr"});
    ...
    UniTensor Lanczos_Exp_Ut(LinOp *Hop, const UniTensor &Tin, const double &CvgCrit,
      ...
      cytnx_error_msg(
        ...
        "[ERROR][Lanczos_Exp_Ut] Lanczos_Exp_Ut can only accept operator with floating types (complex/real)%s",
        "\n");
