...
namespace cytnx {
  namespace linalg {
    ...
    namespace {  // actual implementations:
      void Svd_truncate_Dense_UT_internal(std::vector<UniTensor> &outCyT,
      ...
      }  // Svd_truncate_Dense_UT_internal no minblockdim

      void Svd_truncate_Block_UT_internal(std::vector<UniTensor> &outCyT,
                                          const cytnx::UniTensor &Tin, const cytnx_uint64 &keepdim,
                                          const double &err, const bool &is_UvT,
      ...
      }  // Svd_truncate_Block_UT_internal no minblockdim
      ...
      }  // Svd_truncate_Block_UT_internal with minblockdim
    }  // unnamed namespace
    ...
    std::vector<cytnx::UniTensor> Svd_truncate(const cytnx::UniTensor &Tin,
      ...
      if (Tin.uten_type() == UTenType.Dense) {
        ...
        Svd_truncate_Block_UT_internal(outCyT, Tin, keepdim, err, is_UvT, return_err, mindim);
      } else {
        cytnx_error_msg(
    ...
    std::vector<cytnx::UniTensor> Svd_truncate(const cytnx::UniTensor &Tin,
      ...
      if (Tin.uten_type() == UTenType.Dense) {
        ...
        Svd_truncate_Dense_UT_internal(outCyT, Tin, keepdim, err, is_UvT, return_err,
                 ...
                 (Tin.uten_type() == UTenType.BlockFermionic)) {
        Svd_truncate_Block_UT_internal(outCyT, Tin, keepdim, min_blockdim, err, is_UvT, return_err,
                                       mindim);
      } else {
        cytnx_error_msg(
          true, "[ERROR] Svd_truncate only supports Dense/Block/BlockFermionic UniTensors.%s",
