...
namespace cytnx{
    ...
    namespace linalg{
        ...
        Tensor Vectordot(const Tensor &Tl, const Tensor &Tr, const bool &is_conj){
            ...
            cytnx_error_msg(Tl.shape().size()!=1,"[ERROR][Tl] tensor for Vectordot should be rank-1.%s","\n");
            cytnx_error_msg(Tr.shape().size()!=1,"[ERROR][Tr] tensor for Vectordot should be rank-1.%s","\n");
            cytnx_error_msg(Tr.shape()[0]!=Tl.shape()[0],"[ERROR] two tensor for Vectordot should have same length.%s","\n");
            ...
            if(out.device()==Device.cpu){
                cytnx::linalg_internal::lii.Vd_ii[out.dtype()](out._impl->storage()._impl,L._impl->storage()._impl,R._impl->storage()._impl,L._impl->storage()._impl->size(),is_conj);
                return out;
            }else{
                #ifdef UNI_GPU
                    ...
                    return out;
                #else
                    cytnx_error_msg(true,"[Vectordot] fatal error,%s","try to call the gpu section without CUDA support.\n");
                    return Tensor();   
                #endif                                      
            } 
