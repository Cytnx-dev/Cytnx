    ...
    namespace linalg{
        Tensor Dot(const Tensor &Tl, const Tensor &Tr){
            ...
            if(Tl.shape().size()==1 && Tr.shape().size()==1){
                ...
                //check dimension:
                cytnx_error_msg(Tl.shape().back() != Tr.shape()[0],"[Dot] error. The last dimension of Tl must be the same as dimension of Tr.%s","\n");
                ...
                if(Tl.device()==Device.cpu){
                    ...
                    cytnx::linalg_internal::lii.Matvec_ii[_tl.dtype()](out._impl->storage()._impl, 
                    ...
                    cytnx::linalg_internal::lii.cuMatvec_ii[_tl.dtype()](out._impl->storage()._impl, 
                                                            _tl._impl->storage()._impl,
                                                            _tr._impl->storage()._impl,  
                                                             lin_dim,_tr.shape()[0]);
                        ...
                        return out;
                    #else
                        cytnx_error_msg(true,"[Dot] fatal error,%s","try to call the gpu section without CUDA support.\n");
                        return Tensor();                                         
                    #endif
                }    
                ...
                cytnx_error_msg(true,"[Dot] error, invalid ranks for Tensors to perform Tensordot. rank-Tl [%d], rank-Tr [%d]",Tl.shape().size(),Tr.shape().size());
            }
