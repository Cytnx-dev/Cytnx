...
namespace cytnx{
    namespace linalg{
        Tensor Expf(const Tensor &Tin){
            ...
            Tensor out;
            if( (Tin.dtype() == Type.ComplexFloat) || (Tin.dtype() == Type.Float) ) out = Tin.clone();
            else if(Tin.dtype() == Type.ComplexDouble) out = Tin.astype(Type.ComplexFloat);
            else if(Tin.dtype() > 4) out = Tin.astype(Type.Float);
            else if(Tin.dtype() == Type.Double) out = Tin.astype(Type.Float);
