...
namespace cytnx{
    namespace linalg{
        Tensor Matmul_dg(const Tensor &Tl, const Tensor &Tr){
            ...
            cytnx_error_msg(Tl.shape().size() > 2,"[Matmul_dg] error, tensor Tl ,Matmul_dg can only operate on rank-2 x rank-1 (rank-1 x rank-2) Tensor.%s","\n");
            cytnx_error_msg(Tr.shape().size() > 2,"[Matmul_dg] error, tensor Tr ,Matmul_dg can only operate on rank-2 x rank-1 (rank-1 x rank-2) Tensor.%s","\n");
            cytnx_error_msg(Tl.shape().size()==Tr.shape().size(),"[Matmul_dg] error, tensor Tr:rank[%d] Tl:rank[%d] ,Matmul_dg can only operate on rank-2 x rank-1 (rank-1 x rank-2) Tensor.\n",Tl.shape().size(),Tr.shape().size());

            //check device:
            cytnx_error_msg(Tl.device()!=Tr.device(),"[Matmul_dg] error two tensor should be on same device.%s","\n");

            //check dimension match
            cytnx_error_msg(Tl.shape().back() != Tr.shape()[0],"[Matmul_dg] error, dimension not match.%s","\n");

            int diag_L;
            if(Tl.shape().size()==1) diag_L = 1;
            else diag_L = 0;
            ...
            if(Tl.device()==Device.cpu){
                ...
                return out;
                    ...
                    cytnx::linalg_internal::lii.cuMatmul_dg_ii[_tl.dtype()](out._impl->storage()._impl,
                                                              _tl._impl->storage()._impl,
                                                              _tr._impl->storage()._impl,
                                                              _tl.shape()[0],_tl.shape().back(),_tr.shape().back(),diag_L);
