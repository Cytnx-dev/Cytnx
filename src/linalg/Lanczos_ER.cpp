...
namespace cytnx{
    namespace linalg{
        ...
        void _Lanczos_ER_general(std::vector<Tensor> &out, LinOp *Hop, std::vector<Tensor> &buffer, const cytnx_uint64 &k, const bool &is_V, const cytnx_uint32 &max_krydim, const cytnx_uint64 &maxiter, const double &CvgCrit, const bool &verbose){
        ...
        */
        ...
        std::vector<Tensor> Lanczos_ER(LinOp *Hop, const cytnx_uint64 &k, const bool &is_V, const cytnx_uint64 &maxiter, const double &CvgCrit, const bool &is_row, const Tensor &Tin, const cytnx_uint32 &max_krydim, const bool &verbose){
                ...
                cytnx_error_msg(k>=Hop->nx(),"[ERROR][Lanczos] k can only be up to total dimension of input vector D-1%s","\n");

                // check Tin should be rank-1:
                std::vector<Tensor> buffer(max_krydim+1);
                if(Tin.dtype()==Type.Void){
                    buffer[0] = cytnx::random::normal({Hop->nx()},Hop->dtype(),Hop->device()); // randomly initialize.
                }else{
