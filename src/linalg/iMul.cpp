    ...
    void iMul(Tensor &Lt, const Tensor &Rt) {
      ...
      if ((Lt.is_contiguous() && Rt.is_contiguous())) {
        ...
        if (Lt.device() == Device.cpu) {
          ...
          // #1003: route in-place arithmetic through the typed cuMul_dispatch instead of the
        ...
        if (Lt.device() == Device.cpu) {
          ...
          // tensor RHS, cuMul_dispatch now has a proper non-contiguous kernel that reads both
