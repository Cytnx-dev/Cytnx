...
namespace cytnx{
    namespace tn_algo{
        ...
        void MPS::_Save(std::fstream &f) const{
            ...
            cytnx_error_msg(this->_impl->mps_type_id==MPSType.Void,"[ERROR][MPS] cannot save an uninitialize MPS.%s","\n");
            
            unsigned int IDDs = 109;
            f.write((char*)&IDDs,sizeof(unsigned int));
            
            f.write((char*)&this->_impl->mps_type_id,sizeof(int)); // mps type, this is used to determine Regular/iMPS upon load
        ...
        void MPS::_Load(std::fstream &f){
            ...
            int mpstype;
            f.read((char*)&mpstype,sizeof(int)); // mps type, this is used to determine Sparse/Dense upon load
