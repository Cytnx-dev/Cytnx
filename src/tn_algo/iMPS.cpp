...
namespace cytnx{
    namespace tn_algo{
        ...
        void iMPS::Init(const cytnx_uint64 &N, const std::vector<cytnx_uint64> &vphys_dim, const cytnx_uint64 &virt_dim, const cytnx_int64 &dtype){
            //checking:
            ...
            cytnx_error_msg(dtype!=Type.Double,"[ERROR][iMPS] MPS currently only support Double type.%s","\n");
            ...
            for(cytnx_int64 k=0; k<N; k++){
                this->_TNs[k] = UniTensor(cytnx::random::normal({chi, vphys_dim[k], chi}, 0., 1.),false,2);
