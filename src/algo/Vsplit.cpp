...
namespace cytnx {
  namespace algo {
    ...
    void Vsplit_(std::vector<Tensor> &out, const Tensor &Tin, const std::vector<cytnx_uint64> &dims){
        ...
        cytnx_error_msg(Tin.dtype()==Type.Bool,"[ERROR][Vsplit_] currently does not support Bool type!%s","\n");
        ...
        cytnx_error_msg(sum != Tin.shape()[0],"[ERROR][Vsplit_] the sum of all elements in [dims] must match the dim of axes [0] for Tin.%s","\n");
        ...
        if(Tin.device()==Device.cpu){
           algo_internal::vSplit_internal(targ_ptrs, (char*)_Tn.storage().data(), dims, _Tn.shape()[1], Type.typeSize(Tin.dtype())); 
        }else{
