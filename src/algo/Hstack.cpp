...
namespace cytnx {
  namespace algo {
    ...
    Tensor Hstack(const std::vector<Tensor> &In_tensors){
      ...
      cytnx_error_msg(In_tensors[0].shape().size()!=2,"[ERROR][Hstack] elem: [0], Vstack can only work for rank-2 tensors.%s","\n");
      ...
      if(device_id==Device.cpu){
        algo_internal::hConcate_internal((char*)out.storage().data(),rawPtr,Ds,Dshare,Dcomb,Type.typeSize(dtype_id));
      }else{
