...
namespace cytnx {
  namespace algo {
    ...
    void Hsplit_(std::vector<Tensor> &out, const Tensor &Tin, const std::vector<cytnx_uint64> &dims){
        cytnx_error_msg(Tin.shape().size()!=2,"[ERROR][Hsplit_] Can only work for rank-2 Tensor.%s","\n");
        cytnx_error_msg(Tin.dtype()==Type.Bool,"[ERROR][Hsplit_] currently does not support Bool type!%s","\n");
        ...
        cytnx_error_msg(sum != Tin.shape()[1],"[ERROR][Hsplit_] the sum of all elements in [dims] must match the dim of axes [1] for Tin.%s","\n");
        ...
        if(Tin.device()==Device.cpu){
           algo_internal::hSplit_internal(targ_ptrs, (char*)_Tn.storage().data(), dims, _Tn.shape()[0], Type.typeSize(Tin.dtype())); 
        }else{
