...
namespace cytnx {
    namespace algo{
        ...
        Tensor Concatenate(Tensor T1, Tensor T2){
            ...
            //check:
            cytnx_error_msg(T1.shape().size()!=1,"[ERROR] concatenate currently can only accept 1d Tensor @T1.%s","\n");
            cytnx_error_msg(T2.shape().size()!=1,"[ERROR] concatenate currently can only accept 1d Tensor @T2.%s","\n");
            ...
            Tensor t1,t2;
            cytnx_int64 dtype;
            if(T1.dtype() < T2.dtype()){ t1=T1; t2 = T2.astype(T1.dtype()); dtype=t1.dtype();}
            else{ t1 = T1.astype(T2.dtype()); t2 = T2; dtype=t2.dtype();}
