...
namespace cytnx {
  ...
  namespace algo_internal {
    ...
    void vSplit_internal(std::vector<char*> &targ_ptrs, const char* In_ptr, const std::vector<cytnx_uint64> &dims, const cytnx_uint64 &Col_dim, const cytnx_uint64 &ElemSize){
    ...
    void hSplit_internal(std::vector<char*> &targ_ptrs, const char* In_ptr, const std::vector<cytnx_uint64> &dims, const cytnx_uint64 &Row_dim, const cytnx_uint64 &ElemSize){
        ...
        for(cytnx_uint64 t=0;t<dims.size();t++){
            ...
            for(cytnx_uint64 r=0;r<Row_dim;r++){
                memcpy(targ_ptrs[t] + r*dims[t]*ElemSize, In_ptr + (r*Dtot + offs[t])*ElemSize, ElemSize*dims[t]);          
            }
        }
