...
namespace cytnx{
    namespace utils_internal{
        ...
        //                 const vector<vector<cytnx_uint64>> &dest_shapes, const vector<vector<cytnx_uint64>> &src_shapes,
        ...
        //                 map<cytnx_uint64, pair<cytnx_uint64,cytnx_uint64>>  &dest_outer2inner_row, map<cytnx_uint64,pair<cytnx_uint64, cytnx_uint64>>  &dest_outer2inner_col,
        ...
        void _moving_elem(vector<Tensor> &dest_blocks, const vector<Tensor> &src_blocks,
            ...
            vector<cytnx_uint64> acc_in_old(src_inner_rowrank),acc_out_old(oldshape.size()-src_inner_rowrank);
            ...
            std::vector<cytnx_uint64> tfidx_tmp(acc_in_old.size()+acc_out_old.size()); // for concatenate            
            ...
            for(unsigned int b=0;b<src_blocks.size();b++){
                    ...
                    //     unsigned int j=elem%src_shapes[b][1];
                        //decompress 
                        // vector<cytnx_uint64> tfidx = vec_concatenate(c2cartesian(src_inner2outer_row[b][i],acc_in_old), c2cartesian(src_inner2outer_col[b][j],acc_out_old));
                        // vec_concatenate_(tfidx_tmp, c2cartesian(src_inner2outer_row[b][i],acc_in_old), c2cartesian(src_inner2outer_col[b][j],acc_out_old));                        
                        ...
                        auto dest_mem = dest_blocks[dest_outer2inner_row[new_row].first]._impl->storage()._impl->Mem;
                        auto src_mem = src_blocks[b]._impl->storage()._impl->Mem;
                        cytnx_int64 dest_idx = (dest_outer2inner_row[new_row].second)*(dest_blocks[dest_outer2inner_row[new_row].first].shape()[1]) + dest_outer2inner_col[new_col].second;
                        ...
                        // cytnx_int64 dest_idx = (dest_outer2inner_row[new_row].second)*(dest_shapes[dest_outer2inner_row[new_row].first][1]) + dest_outer2inner_col[new_col].second;
                        // cytnx_int64 src_idx = i*src_shapes[b][1]+j;
        ...
        void blocks_mvelems_d(vector<Tensor> &dest_blocks, const vector<Tensor> &src_blocks,
                            ...
                            const vector<cytnx_uint64> &mapper, const vector<cytnx_uint64> &inv_mapper,
                            const cytnx_uint64 &src_inner_rowrank,  const cytnx_uint64 &dest_rowrank){
            _moving_elem<cytnx_double>(dest_blocks, src_blocks, src_shape, src_inner2outer_row, src_inner2outer_col,
                                    dest_outer2inner_row, dest_outer2inner_col, mapper, inv_mapper, src_inner_rowrank, dest_rowrank);
        } // end blocks_mvelems
