...
namespace cytnx{
    ...
    void FloatStorage::_Init_byptr(void *rawptr, const unsigned long long &len_in, const int &device, const bool &iscap, const unsigned long long &cap_in){
        this->Mem = rawptr;
        this->len = len_in;
        if(iscap){
            this->cap = cap_in;
    ...
    boost::intrusive_ptr<Storage_base> FloatStorage::clone(){
        ...
        if(this->device==Device.cpu){
            memcpy(out->Mem,this->Mem,sizeof(float)*this->len);
                ...
                checkCudaErrors(cudaMemcpy(out->Mem,this->Mem,sizeof(float)*this->len,cudaMemcpyDeviceToDevice));
            #else
            ...
            if(this->device==Device.cpu){
                //here, cpu->gpu with gid=device
                    ...
                    if(device==Device.cpu){
                        ...
                        checkCudaErrors(cudaMemcpy(htmp,this->Mem,sizeof(float)*this->len,cudaMemcpyDeviceToHost));
                        cudaFree(this->Mem);
                        this->Mem = htmp;
                        this->device = device;
                    }else{
    ...
    void FloatStorage::PrintElem_byShape(std::ostream &os, const std::vector<cytnx_uint64> &shape, const std::vector<cytnx_uint64> &mapper){
            ...
            if(mapper.size()==0){
                ...
                while(1){
                    ...
                    for(cytnx_uint64 i=0;i<shape.back();i++){
                        stk2.back() = i;
                        sprintf(buffer,"%.5e ",elem_ptr_[cnt]); os << std::string(buffer);
                        cnt++;
                    }
                ...
                while(1){
                    ...
                    for(cytnx_uint64 i=0;i<shape.back();i++){
                        ...
                        sprintf(buffer,"%.5e ",elem_ptr_[RealMemPos]); os << std::string(buffer);
                        //cnt++;
                    }
    ...
    void FloatStorage::print_elems(){
        ...
        for(unsigned long long cnt=0;cnt<this->len;cnt++){
            sprintf(buffer,"%.5e ",elem_ptr_[cnt]); cout << std::string(buffer);
        }
        cout << "]" << endl;
        free(buffer);
    }
    ...
    void FloatStorage::resize(const cytnx_uint64 &newsize){
        ...
        if(newsize > this->cap){
            ...
            if(this->device==Device.cpu){
                void *htmp = malloc(sizeof(cytnx_float)*this->cap);
