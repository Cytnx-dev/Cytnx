...
namespace cytnx{
    namespace stat{
        ...
        //-----------[2d]
        Histogram2d::Histogram2d(const unsigned long long &Nbinx, const unsigned long long &Nbiny, const double &min_x, const double &max_x, const double &min_y, const double &max_y){
            if(min_x >= max_x){
                std::cout << "[ERROR] cannot have min >= max [x axis]" << std::endl;exit(1);
            }
            if(min_y >= max_y){
                std::cout << "[ERROR] cannot have min >= max [y axis]" << std::endl;exit(1);
            }
            this->minx = min_x;
            this->maxx = max_x;
            this->miny = min_y;
