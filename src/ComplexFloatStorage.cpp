...
namespace cytnx{
    ...
    void ComplexFloatStorage::_Init_byptr(void *rawptr, const unsigned long long &len_in, const int &device, const bool &iscap, const unsigned long long &cap_in){
    ...
    void ComplexFloatStorage::print_elems(){
        ...
        for(unsigned long long cnt=0;cnt<this->len;cnt++){
            sprintf(buffer,"%.5e%+.5ej ",elem_ptr_[cnt].real(),elem_ptr_[cnt].imag()); cout << std::string(buffer);
        }
        cout << " ]" << endl;
        free(buffer);
    }
