...
namespace cytnx {
  //+++++++++++++++++++
  void ComplexDoubleStorage::Init(const unsigned long long &len_in, const int &device, const bool &init_zero) {
    ...
    if (device == Device.cpu) {
      if(init_zero) this->Mem = utils_internal::Calloc_cpu(this->cap, sizeof(complex<double>));
      else this->Mem = utils_internal::Malloc_cpu(this->cap*sizeof(complex<double>));
    } else {
#ifdef UNI_GPU
  ...
  void ComplexDoubleStorage::_Init_byptr(void *rawptr, const unsigned long long &len_in,
  ...
  boost::intrusive_ptr<Storage_base> ComplexDoubleStorage::clone() {
    ...
    if (this->device == Device.cpu) {
      ...
      checkCudaErrors(cudaMemcpy(out->Mem, this->Mem, sizeof(cytnx_complex128) * this->len,
  ...
  void ComplexDoubleStorage::Move_memory_(const std::vector<cytnx_uint64> &old_shape,
  ...
  boost::intrusive_ptr<Storage_base> ComplexDoubleStorage::Move_memory(
...
// here, cpu->gpu with gid=device
        ...
        checkCudaErrors(cudaMemcpy(dtmp, this->Mem, sizeof(cytnx_complex128) * this->len,
        ...
        if (device == Device.cpu) {
          ...
          checkCudaErrors(cudaMemcpy(htmp, this->Mem, sizeof(cytnx_complex128) * this->len,
          ...
          checkCudaErrors(cudaMemcpyPeer(dtmp, device, this->Mem, this->device,
                                         sizeof(cytnx_complex128) * this->len));
          cudaFree(this->Mem);
          this->Mem = dtmp;
          this->device = device;
        }
...
// here, cpu->gpu with gid=device
  ...
  void ComplexDoubleStorage::PrintElem_byShape(std::ostream &os,
  ...
  void ComplexDoubleStorage::resize(const cytnx_uint64 &newsize) {
    ...
    if (newsize > this->cap) {
      ...
      if (this->device == Device.cpu) {
        void *htmp = calloc(this->cap, sizeof(cytnx_complex128));
        ...
        cytnx_error_msg(device >= Device.Ngpus, "%s", "[ERROR] invalid device.");
        cudaSetDevice(device);
        void *dtmp = utils_internal::cuCalloc_gpu(this->cap, sizeof(cytnx_complex128));
        checkCudaErrors(cudaMemcpyPeer(dtmp, device, this->Mem, this->device,
                                       sizeof(cytnx_complex128) * this->len));
        cudaFree(this->Mem);
        this->Mem = dtmp;
  ...
  boost::intrusive_ptr<Storage_base> ComplexDoubleStorage::real() {
    if (this->device == Device.cpu) {
      boost::intrusive_ptr<Storage_base> out(new DoubleStorage());
      void *dtmp = malloc(sizeof(cytnx_double) * this->cap);
      ...
      boost::intrusive_ptr<Storage_base> out(new DoubleStorage());
      cudaSetDevice(device);
      void *dtmp = utils_internal::cuMalloc_gpu(sizeof(cytnx_double) * this->cap);
  ...
  boost::intrusive_ptr<Storage_base> ComplexDoubleStorage::imag() {
    if (this->device == Device.cpu) {
      boost::intrusive_ptr<Storage_base> out(new DoubleStorage());
      void *dtmp = malloc(sizeof(cytnx_double) * this->cap);
  ...
  void ComplexDoubleStorage::set_item(const cytnx_uint64 &idx, const Scalar &val) {
    this->at<cytnx_complex128>(idx) = complex128(val);
  }
  void ComplexDoubleStorage::set_item(const cytnx_uint64 &idx, const cytnx_complex128 &val) {
    this->at<cytnx_complex128>(idx) = val;
  }
  void ComplexDoubleStorage::set_item(const cytnx_uint64 &idx, const cytnx_complex64 &val) {
    this->at<cytnx_complex128>(idx) = val;
  }
  void ComplexDoubleStorage::set_item(const cytnx_uint64 &idx, const cytnx_double &val) {
    this->at<cytnx_complex128>(idx) = val;
  }
  void ComplexDoubleStorage::set_item(const cytnx_uint64 &idx, const cytnx_float &val) {
    this->at<cytnx_complex128>(idx) = val;
  }
  void ComplexDoubleStorage::set_item(const cytnx_uint64 &idx, const cytnx_int64 &val) {
    this->at<cytnx_complex128>(idx) = val;
  }
  void ComplexDoubleStorage::set_item(const cytnx_uint64 &idx, const cytnx_uint64 &val) {
    this->at<cytnx_complex128>(idx) = val;
  }
  void ComplexDoubleStorage::set_item(const cytnx_uint64 &idx, const cytnx_int32 &val) {
    this->at<cytnx_complex128>(idx) = val;
  }
  void ComplexDoubleStorage::set_item(const cytnx_uint64 &idx, const cytnx_uint32 &val) {
    this->at<cytnx_complex128>(idx) = val;
  }
  void ComplexDoubleStorage::set_item(const cytnx_uint64 &idx, const cytnx_int16 &val) {
    this->at<cytnx_complex128>(idx) = val;
  }
  void ComplexDoubleStorage::set_item(const cytnx_uint64 &idx, const cytnx_uint16 &val) {
    this->at<cytnx_complex128>(idx) = val;
  }
  void ComplexDoubleStorage::set_item(const cytnx_uint64 &idx, const cytnx_bool &val) {
    this->at<cytnx_complex128>(idx) = val;
  }
