...
namespace tor10{
    ...
    boost::intrusive_ptr<Storage_base> ComplexDoubleStorage::copy(){
        boost::intrusive_ptr<Storage_base> out(new ComplexDoubleStorage());
        out->Init(this->len,this->device);
        if(this->device==tor10device.cpu){
            memcpy(out->Mem,this->Mem,sizeof(tor10_complex128)*this->len);
        }else{
            #ifdef UNI_GPU
                checkCudaErrors(cudaMemcpy(out->Mem,this->Mem,sizeof(tor10_complex128)*this->len,cudaMemcpyDeviceToDevice));
            #else
    ...
    void ComplexDoubleStorage::Move_memory_(const std::vector<tor10_uint64> &old_shape, const std::vector<tor10_uint64> &mapper, const std::vector<tor10_uint64> &invmapper){
    ...
    boost::intrusive_ptr<Storage_base> ComplexDoubleStorage::Move_memory(const std::vector<tor10_uint64> &old_shape, const std::vector<tor10_uint64> &mapper, const std::vector<tor10_uint64> &invmapper){
        boost::intrusive_ptr<Storage_base> tmp(this);
        if(this->device==tor10device.cpu){
        ...
        if(this->device != device){
            if(this->device==tor10device.cpu){
                //here, cpu->gpu with gid=device
                    ...
                    tor10_error_msg(device>=tor10device.Ngpus,"%s","[ERROR] invalid device.");
                    cudaSetDevice(device);          
                    void *dtmp = utils_internal::cuMalloc_gpu(sizeof(tor10_complex128)*this->len);
                    checkCudaErrors(cudaMemcpy(dtmp,this->Mem,sizeof(tor10_complex128)*this->len,cudaMemcpyHostToDevice));
                    free(this->Mem);
                    this->Mem = dtmp;
                    this->device=device;
                #else
                    ...
                    if(device==tor10device.cpu){
                        ...
                        void *htmp = malloc(sizeof(tor10_complex128)*this->len);
                        checkCudaErrors(cudaMemcpy(htmp,this->Mem,sizeof(tor10_complex128)*this->len,cudaMemcpyDeviceToHost));
                        cudaFree(this->Mem);
                        this->Mem = htmp;
                        this->device = device;
                    }else{
                        ...
                        cudaSetDevice(device);
                        void *dtmp = utils_internal::cuMalloc_gpu(sizeof(tor10_complex128)*this->len);
                        checkCudaErrors(cudaMemcpyPeer(dtmp,device,this->Mem,this->device,sizeof(tor10_complex128)*this->len));
                        cudaFree(this->Mem);
                        this->Mem = dtmp;
                        this->device = device;
                    }
    ...
    void ComplexDoubleStorage::PrintElem_byShape(std::ostream &os, const std::vector<tor10_uint64> &shape, const std::vector<tor10_uint64> &mapper){
