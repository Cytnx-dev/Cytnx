...
namespace cytnx{
    void Uint32Storage::Init(const unsigned long long &len_in, const int &device){
        ...
        this->dtype = cytnxtype.Uint32;
        if(device== cytnxdevice.cpu){
            this->Mem = utils_internal::Malloc_cpu(this->len*sizeof(cytnx_uint32));
        }else{
            #ifdef UNI_GPU
                cytnx_error_msg(device>=cytnxdevice.Ngpus,"%s","[ERROR] invalid device.");
                cudaSetDevice(device);
                this->Mem = utils_internal::cuMalloc_gpu(this->len*sizeof(cytnx_uint32));
            #else
        ...
        if(this->device != device){
            if(this->device==cytnxdevice.cpu){
                //here, cpu->gpu with gid=device
                    ...
                    cytnx_error_msg(device>=cytnxdevice.Ngpus,"%s","[ERROR] invalid device.");
                    cudaSetDevice(device);          
                    void *dtmp = utils_internal::cuMalloc_gpu(sizeof(cytnx_uint32)*this->len);
                    ...
                    if(device==cytnxdevice.cpu){
                        ...
                        void *htmp = malloc(sizeof(cytnx_uint32)*this->len);
    ...
    void Uint32Storage::GetElem_byShape(boost::intrusive_ptr<Storage_base> &out, const std::vector<cytnx_uint64> &shape, const std::vector<cytnx_uint64> &mapper, const std::vector<cytnx_uint64> &len, const std::vector< std::vector<cytnx_uint64> > &locators){
        #ifdef UNI_DEBUG
                ...
                cytnx_uint32* elem_ptr_     = static_cast<cytnx_uint32*>(this->Mem);
                cytnx_uint32* new_elem_ptr_ = static_cast<cytnx_uint32*>(out->Mem ); 
