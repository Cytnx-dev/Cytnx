...
namespace cytnx{
    void Uint16Storage::Init(const unsigned long long &len_in, const int &device){
        ...
        if(device== Device.cpu){
            this->Mem = utils_internal::Malloc_cpu(this->len*sizeof(cytnx_uint16));
        }else{
            #ifdef UNI_GPU
                cytnx_error_msg(device>=Device.Ngpus,"%s","[ERROR] invalid device.");
                cudaSetDevice(device);
                this->Mem = utils_internal::cuMalloc_gpu(this->len*sizeof(cytnx_uint16));
            #else
        ...
        if(this->device != device){
            if(this->device==Device.cpu){
                //here, cpu->gpu with gid=device
                    ...
                    cytnx_error_msg(device>=Device.Ngpus,"%s","[ERROR] invalid device.");
                    cudaSetDevice(device);          
                    void *dtmp = utils_internal::cuMalloc_gpu(sizeof(cytnx_uint16)*this->len);
                    ...
                    if(device==Device.cpu){
                        ...
                        void *htmp = malloc(sizeof(cytnx_uint16)*this->len);
