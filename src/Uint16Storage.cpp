...
namespace cytnx{
    void Uint16Storage::Init(const unsigned long long &len_in, const int &device){
        //cout << "Uint16.init" << endl;
        this->len = len_in;

        //check:
        //cytnx_error_msg(len_in < 1, "%s", "[ERROR] cannot init a Storage with zero element");
        this->dtype = Type.Uint16;
        ...
        if(device== Device.cpu){
            //this->Mem = utils_internal::Malloc_cpu(this->cap*sizeof(cytnx_uint16));
            this->Mem = utils_internal::Calloc_cpu(this->cap,sizeof(cytnx_uint16));
        }else{
            #ifdef UNI_GPU
                cytnx_error_msg(device>=Device.Ngpus,"%s","[ERROR] invalid device.");
                cudaSetDevice(device);
                //this->Mem = utils_internal::cuMalloc_gpu(this->cap*sizeof(cytnx_uint16));
                this->Mem = utils_internal::cuCalloc_gpu(this->cap,sizeof(cytnx_uint16));
            #else
    ...
    void Uint16Storage::_Init_byptr(void *rawptr, const unsigned long long &len_in, const int &device, const bool &iscap, const unsigned long long &cap_in){
    ...
    boost::intrusive_ptr<Storage_base> Uint16Storage::_create_new_sametype(){
        boost::intrusive_ptr<Storage_base> out(new Uint16Storage());
        return out;
    }

    boost::intrusive_ptr<Storage_base> Uint16Storage::clone(){
        boost::intrusive_ptr<Storage_base> out(new Uint16Storage());
        out->Init(this->len,this->device);
        if(this->device==Device.cpu){
            memcpy(out->Mem,this->Mem,sizeof(cytnx_uint16)*this->len);
        }else{
            #ifdef UNI_GPU
                checkCudaErrors(cudaSetDevice(this->device));
                checkCudaErrors(cudaMemcpy(out->Mem,this->Mem,sizeof(cytnx_uint16)*this->len,cudaMemcpyDeviceToDevice));
            #else
    ...
    void Uint16Storage::Move_memory_(const std::vector<cytnx_uint64> &old_shape, const std::vector<cytnx_uint64> &mapper, const std::vector<cytnx_uint64> &invmapper){
    ...
    boost::intrusive_ptr<Storage_base> Uint16Storage::Move_memory(const std::vector<cytnx_uint64> &old_shape, const std::vector<cytnx_uint64> &mapper, const std::vector<cytnx_uint64> &invmapper){
        boost::intrusive_ptr<Storage_base> tmp(this);
        if(this->device==Device.cpu){
    ...
    void Uint16Storage::to_(const int &device){
        if(this->device != device){
            if(this->device==Device.cpu){
                //here, cpu->gpu with gid=device
                    ...
                    cytnx_error_msg(device>=Device.Ngpus,"%s","[ERROR] invalid device.");
                    cudaSetDevice(device);          
                    void *dtmp = utils_internal::cuMalloc_gpu(sizeof(cytnx_uint16)*this->cap);
                    checkCudaErrors(cudaMemcpy(dtmp,this->Mem,sizeof(cytnx_uint16)*this->len,cudaMemcpyHostToDevice));
                    free(this->Mem);
                    this->Mem = dtmp;
                    this->device=device;
                #else
                    ...
                    if(device==Device.cpu){
                        ...
                        void *htmp = malloc(sizeof(cytnx_uint16)*this->cap);
                        checkCudaErrors(cudaMemcpy(htmp,this->Mem,sizeof(cytnx_uint16)*this->len,cudaMemcpyDeviceToHost));
                        cudaFree(this->Mem);
                        this->Mem = htmp;
                        this->device = device;
                    }else{
                        ...
                        cudaSetDevice(device);
                        void *dtmp = utils_internal::cuMalloc_gpu(sizeof(cytnx_uint16)*this->cap);
                        checkCudaErrors(cudaMemcpyPeer(dtmp,device,this->Mem,this->device,sizeof(cytnx_uint16)*this->len));
                        cudaFree(this->Mem);
                        this->Mem = dtmp;
                        this->device = device;
                    }
    ...
    boost::intrusive_ptr<Storage_base> Uint16Storage::to(const int &device){
    ...
    void Uint16Storage::PrintElem_byShape(std::ostream &os, const std::vector<cytnx_uint64> &shape, const std::vector<cytnx_uint64> &mapper){
        ...
        if(len==0){
            ...
            sprintf(buffer," (%d",shape[0]); os << std::string(buffer);
            ...
            cytnx_uint64 s;
            cytnx_uint16* elem_ptr_ = static_cast<cytnx_uint16*>(this->Mem);
            ...
            if(mapper.size()==0){
                ...
                while(1){
                    ...
                    for(cytnx_uint64 i=0;i<shape.back();i++){
                        stk2.back() = i; 
                        sprintf(buffer,"%5d ",elem_ptr_[cnt]); os << std::string(buffer);
                        cnt++;
                    }
                ...
                while(1){
                    ...
                    for(cytnx_uint64 i=0;i<shape.back();i++){
                        ...
                        sprintf(buffer,"%5d ",elem_ptr_[RealMemPos]); os << std::string(buffer);
                        //cnt++;
                    }
    ...
    void Uint16Storage::print_elems(){
        char* buffer = (char*)malloc(sizeof(char)*256);
        cytnx_uint16* elem_ptr_ = static_cast<cytnx_uint16*>(this->Mem);
        cout << "[ ";
        for(unsigned long long cnt=0;cnt<this->len;cnt++){
            sprintf(buffer,"%5d ",elem_ptr_[cnt]); cout  << std::string(buffer);
        }
        cout << " ]" << endl;
        free(buffer);
    }

    void Uint16Storage::fill(const cytnx_complex128 &val){
        cytnx_error_msg(true,"[ERROR]%s"," cannot fill complex value into real container");
    }
    void Uint16Storage::fill(const cytnx_complex64  &val){
        cytnx_error_msg(true,"[ERROR]%s"," cannot fill complex value into real container");
    }
    ...
    void Uint16Storage::set_zeros(){
        if(this->device == Device.cpu){
            utils_internal::SetZeros(this->Mem,sizeof(cytnx_uint16)*this->len);
        }else{
            #ifdef UNI_GPU
                checkCudaErrors(cudaSetDevice(this->device));
                utils_internal::cuSetZeros(this->Mem,sizeof(cytnx_uint16)*this->len);
            #else
                cytnx_error_msg(1,"[ERROR][set_zeros] fatal, the storage is on gpu without CUDA support.%s","\n");
            #endif
        }
    }
    void Uint16Storage::resize(const cytnx_uint64 &newsize){
        ...
        if(newsize > this->cap){
            ...
            if(this->device==Device.cpu){
                void *htmp = calloc(this->cap,sizeof(cytnx_uint16));
                memcpy(htmp,this->Mem,sizeof(cytnx_uint16)*this->len);
                free(this->Mem);
                this->Mem = htmp;
            }else{
                #ifdef UNI_GPU
                    cytnx_error_msg(device>=Device.Ngpus,"%s","[ERROR] invalid device.");
                    cudaSetDevice(device);
                    void *dtmp = utils_internal::cuCalloc_gpu(this->cap,sizeof(cytnx_uint16));
                    checkCudaErrors(cudaMemcpyPeer(dtmp,device,this->Mem,this->device,sizeof(cytnx_uint16)*this->len));
                    cudaFree(this->Mem);
                    this->Mem = dtmp;
                #else
    ...
    void Uint16Storage::append(const Scalar &val){
        if(this->len+1 > this->cap){
            this->resize(this->len+1);
        }else{
            this->len+=1;
        }
        this->at<cytnx_uint16>(this->len-1) = cytnx_uint16(val);
    }
    void Uint16Storage::append(const cytnx_complex128 &val){
        cytnx_error_msg(true,"[ERROR]%s"," cannot append complex value into real container");
    }
    void Uint16Storage::append(const cytnx_complex64  &val){
        cytnx_error_msg(true,"[ERROR]%s"," cannot append complex value into real container");
    }
    void Uint16Storage::append(const cytnx_double  &val){
        if(this->len+1 > this->cap){
            this->resize(this->len+1);
        }else{
            this->len+=1;
        }
        this->at<cytnx_uint16>(this->len-1) = val;
    }
    void Uint16Storage::append(const cytnx_float  &val){
        if(this->len+1 > this->cap){
            this->resize(this->len+1);
        }else{
            this->len+=1;
    ...
    void Uint16Storage::append(const cytnx_int64  &val){
        if(this->len+1 > this->cap){
            this->resize(this->len+1);
        }else{
            this->len+=1;
    ...
    void Uint16Storage::append(const cytnx_int32  &val){
        if(this->len+1 > this->cap){
            this->resize(this->len+1);
        }else{
            this->len+=1;
    ...
    void Uint16Storage::append(const cytnx_int16  &val){
        if(this->len+1 > this->cap){
            this->resize(this->len+1);
        }else{
            this->len+=1;
    ...
    void Uint16Storage::append(const cytnx_uint64  &val){
        if(this->len+1 > this->cap){
            this->resize(this->len+1);
        }else{
            this->len+=1;
    ...
    void Uint16Storage::append(const cytnx_uint32  &val){
        if(this->len+1 > this->cap){
            this->resize(this->len+1);
        }else{
            this->len+=1;
    ...
    void Uint16Storage::append(const cytnx_uint16  &val){
        if(this->len+1 > this->cap){
            this->resize(this->len+1);
        }else{
            this->len+=1;
    ...
    void Uint16Storage::append(const cytnx_bool  &val){
        if(this->len+1 > this->cap){
            this->resize(this->len+1);
        }else{
            this->len+=1;
    ...
    boost::intrusive_ptr<Storage_base> Uint16Storage::real(){
        cytnx_error_msg(true,"[ERROR] Storage.real() can only be called from complex type.%s","\n");
    }   
    boost::intrusive_ptr<Storage_base> Uint16Storage::imag(){
        cytnx_error_msg(true,"[ERROR] Storage.imag() can only be called from complex type.%s","\n");
    }   
    Scalar Uint16Storage::get_item(const cytnx_uint64 &idx)const{
        return Scalar(this->at<cytnx_uint16>(idx));
    }

    void Uint16Storage::set_item(const cytnx_uint64 &idx, const Scalar &val){
        this->at<cytnx_uint16>(idx) = cytnx_uint16(val);
    }
    void Uint16Storage::set_item(const cytnx_uint64 &idx,const cytnx_complex128 &val){
        cytnx_error_msg(true,"[ERROR] cannot set complex to real.%s","\n");
    }
    void Uint16Storage::set_item(const cytnx_uint64 &idx,const cytnx_complex64  &val){
        cytnx_error_msg(true,"[ERROR] cannot set complex to real.%s","\n");
    }
    void Uint16Storage::set_item(const cytnx_uint64 &idx,const cytnx_double     &val){
        this->at<cytnx_uint16>(idx) = val;
    }
    void Uint16Storage::set_item(const cytnx_uint64 &idx,const cytnx_float      &val){
        this->at<cytnx_uint16>(idx) = val;
    }
    void Uint16Storage::set_item(const cytnx_uint64 &idx,const cytnx_int64      &val){
        this->at<cytnx_uint16>(idx) = val;
    }
    void Uint16Storage::set_item(const cytnx_uint64 &idx,const cytnx_uint64     &val){
        this->at<cytnx_uint16>(idx) = val;
    }
    void Uint16Storage::set_item(const cytnx_uint64 &idx,const cytnx_int32      &val){
        this->at<cytnx_uint16>(idx) = val;
    }
    void Uint16Storage::set_item(const cytnx_uint64 &idx,const cytnx_uint32     &val){
        this->at<cytnx_uint16>(idx) = val;
    }
    void Uint16Storage::set_item(const cytnx_uint64 &idx,const cytnx_int16      &val){
        this->at<cytnx_uint16>(idx) = val;
    }
    void Uint16Storage::set_item(const cytnx_uint64 &idx,const cytnx_uint16     &val){
        this->at<cytnx_uint16>(idx) = val;
    }
    void Uint16Storage::set_item(const cytnx_uint64 &idx,const cytnx_bool       &val){
        this->at<cytnx_uint16>(idx) = val;
    }
