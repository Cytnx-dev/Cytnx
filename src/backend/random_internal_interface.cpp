...
namespace cytnx {
  namespace random_internal {
    ...
    random_internal_interface::random_internal_interface() {
      /// function signature.
      Normal = std::vector<Rnd_io>(N_fType, NULL);
      ...
      /// function signature.
      Uniform = std::vector<Rnd_io>(N_fType, NULL);
...
#ifdef UNI_GPU
      cuNormal = std::vector<Rnd_io>(N_fType, NULL);
      ...
      cuUniform = std::vector<Rnd_io>(N_fType, NULL);
