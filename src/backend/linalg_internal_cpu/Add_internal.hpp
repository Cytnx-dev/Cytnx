...
namespace cytnx {
  namespace linalg_internal {
    // Typed Add kernel (#941): the output type TO is part of the signature,
    // not recovered from a dtype() check at runtime. It is impossible to
    // instantiate this with an out whose actual StorageImplementation<T> does
    // not match TO -- the caller recovers out/lhs/rhs as typed storage
    // (storage_cast<T> / storage_as_type_or_replace<T>) before calling in.
    template <typename TO, typename TL, typename TR>
    inline void AddInternalImpl(const boost::intrusive_ptr<StorageImplementation<TO>> &out,
