  ...
  namespace linalg_internal {
    ...
    void Eigh_internal_cd(const boost::intrusive_ptr<Storage_base> &in,
      ...
      if (v->dtype() != Type.Void) {
        ...
        memcpy(v->data(), in->data(), sizeof(cytnx_complex128) * cytnx_uint64(L) * L);
        jobs = 'V';
      } else {
        tA = (cytnx_complex128 *)malloc(cytnx_uint64(L) * L * sizeof(cytnx_complex128));
        memcpy(tA, in->data(), sizeof(cytnx_complex128) * cytnx_uint64(L) * L);
      }
    ...
    void Eigh_internal_cf(const boost::intrusive_ptr<Storage_base> &in,
      ...
      if (v->dtype() != Type.Void) {
        ...
        memcpy(v->data(), in->data(), sizeof(cytnx_complex64) * cytnx_uint64(L) * L);
        jobs = 'V';
      } else {
        tA = (cytnx_complex64 *)malloc(cytnx_uint64(L) * L * sizeof(cytnx_complex64));
        memcpy(tA, in->data(), sizeof(cytnx_complex64) * cytnx_uint64(L) * L);
      }
    ...
    void Eigh_internal_d(const boost::intrusive_ptr<Storage_base> &in,
      ...
      if (v->dtype() != Type.Void) {
        ...
        memcpy(v->data(), in->data(), sizeof(cytnx_double) * cytnx_uint64(L) * L);
        jobs = 'V';
      } else {
        tA = (cytnx_double *)malloc(cytnx_uint64(L) * L * sizeof(cytnx_double));
        memcpy(tA, in->data(), sizeof(cytnx_double) * cytnx_uint64(L) * L);
      }

      lapack_int ldA = L;
      lapack_int info;
      info = LAPACKE_dsyev(LAPACK_COL_MAJOR, jobs, 'U', L, tA, ldA, (cytnx_double *)e->data());
    ...
    void Eigh_internal_f(const boost::intrusive_ptr<Storage_base> &in,
      ...
      if (v->dtype() != Type.Void) {
        ...
        memcpy(v->data(), in->data(), sizeof(cytnx_float) * cytnx_uint64(L) * L);
        jobs = 'V';
      } else {
        tA = (cytnx_float *)malloc(cytnx_uint64(L) * L * sizeof(cytnx_float));
        memcpy(tA, in->data(), sizeof(cytnx_float) * cytnx_uint64(L) * L);
      }

      lapack_int ldA = L;
      lapack_int info;
      info = LAPACKE_ssyev(LAPACK_COL_MAJOR, jobs, 'U', L, tA, ldA, (cytnx_float *)e->data());
