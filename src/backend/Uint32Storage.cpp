...
namespace cytnx {
  void Uint32Storage::Init(const unsigned long long &len_in, const int &device,
    ...
    if (device == Device.cpu) {
      ...
      // this->Mem = utils_internal::cuMalloc_gpu(this->cap*sizeof(cytnx_uint32));
      this->Mem = utils_internal::cuCalloc_gpu(this->cap, sizeof(cytnx_uint32));
#else
  ...
  void Uint32Storage::_Init_byptr(void *rawptr, const unsigned long long &len_in, const int &device,
  ...
  boost::intrusive_ptr<Storage_base> Uint32Storage::_create_new_sametype() {
    boost::intrusive_ptr<Storage_base> out(new Uint32Storage());
    return out;
  }

  boost::intrusive_ptr<Storage_base> Uint32Storage::clone() {
    boost::intrusive_ptr<Storage_base> out(new Uint32Storage());
    out->Init(this->len, this->device);
    if (this->device == Device.cpu) {
      memcpy(out->Mem, this->Mem, sizeof(cytnx_uint32) * this->len);
    } else {
#ifdef UNI_GPU
      checkCudaErrors(cudaSetDevice(this->device));
      checkCudaErrors(cudaMemcpy(out->Mem, this->Mem, sizeof(cytnx_uint32) * this->len,
  ...
  void Uint32Storage::Move_memory_(const std::vector<cytnx_uint64> &old_shape,
  ...
  boost::intrusive_ptr<Storage_base> Uint32Storage::Move_memory(
  ...
  void Uint32Storage::to_(const int &device) {
...
// here, cpu->gpu with gid=device
        ...
        checkCudaErrors(
          cudaMemcpy(dtmp, this->Mem, sizeof(cytnx_uint32) * this->len, cudaMemcpyHostToDevice));
        free(this->Mem);
        this->Mem = dtmp;
        this->device = device;
#else
        ...
        if (device == Device.cpu) {
          ...
          checkCudaErrors(
            cudaMemcpy(htmp, this->Mem, sizeof(cytnx_uint32) * this->len, cudaMemcpyDeviceToHost));
          cudaFree(this->Mem);
          this->Mem = htmp;
          this->device = device;
        } else {
  ...
  boost::intrusive_ptr<Storage_base> Uint32Storage::to(const int &device) {
...
// here, cpu->gpu with gid=device
  ...
  void Uint32Storage::PrintElem_byShape(std::ostream &os, const std::vector<cytnx_uint64> &shape,
                                        const std::vector<cytnx_uint64> &mapper) {
    ...
    if (len == 0) {
      ...
      sprintf(buffer, buffer, "%s", "Shape :");
      os << std::string(buffer);
      sprintf(buffer, buffer, " (%d", shape[0]);
      os << std::string(buffer);
  ...
  void Uint32Storage::fill(const cytnx_complex64 &val) {
    cytnx_error_msg(true, "[ERROR]%s", " cannot fill complex value into real container");
  }
  ...
  void Uint32Storage::resize(const cytnx_uint64 &newsize) {
    ...
    if (newsize > this->cap) {
      ...
      if (this->device == Device.cpu) {
        ...
        checkCudaErrors(
          cudaMemcpyPeer(dtmp, device, this->Mem, this->device, sizeof(cytnx_uint32) * this->len));
        cudaFree(this->Mem);
        this->Mem = dtmp;
#else
        cytnx_error_msg(
  ...
  void Uint32Storage::append(const Scalar &val) {
    if (this->len + 1 > this->cap) {
      this->resize(this->len + 1);
    } else {
      this->len += 1;
    }
    this->at<cytnx_uint32>(this->len - 1) = cytnx_uint32(val);
  }
  ...
  boost::intrusive_ptr<Storage_base> Uint32Storage::real() {
    cytnx_error_msg(true, "[ERROR] Storage.real() can only be called from complex type.%s", "\n");
  }
  boost::intrusive_ptr<Storage_base> Uint32Storage::imag() {
    cytnx_error_msg(true, "[ERROR] Storage.imag() can only be called from complex type.%s", "\n");
  }
  Scalar Uint32Storage::get_item(const cytnx_uint64 &idx) const {
    return Scalar(this->at<cytnx_uint32>(idx));
  }

  void Uint32Storage::set_item(const cytnx_uint64 &idx, const Scalar &val) {
    this->at<cytnx_uint32>(idx) = cytnx_uint32(val);
  }
  void Uint32Storage::set_item(const cytnx_uint64 &idx, const cytnx_complex128 &val) {
    cytnx_error_msg(true, "[ERROR] cannot set complex to real.%s", "\n");
  }
  void Uint32Storage::set_item(const cytnx_uint64 &idx, const cytnx_complex64 &val) {
    cytnx_error_msg(true, "[ERROR] cannot set complex to real.%s", "\n");
  }
  void Uint32Storage::set_item(const cytnx_uint64 &idx, const cytnx_double &val) {
    this->at<cytnx_uint32>(idx) = val;
  }
  void Uint32Storage::set_item(const cytnx_uint64 &idx, const cytnx_float &val) {
    this->at<cytnx_uint32>(idx) = val;
  }
  void Uint32Storage::set_item(const cytnx_uint64 &idx, const cytnx_int64 &val) {
    this->at<cytnx_uint32>(idx) = val;
  }
  void Uint32Storage::set_item(const cytnx_uint64 &idx, const cytnx_uint64 &val) {
    this->at<cytnx_uint32>(idx) = val;
  }
  void Uint32Storage::set_item(const cytnx_uint64 &idx, const cytnx_int32 &val) {
    this->at<cytnx_uint32>(idx) = val;
  }
  void Uint32Storage::set_item(const cytnx_uint64 &idx, const cytnx_uint32 &val) {
    this->at<cytnx_uint32>(idx) = val;
  }
  void Uint32Storage::set_item(const cytnx_uint64 &idx, const cytnx_int16 &val) {
    this->at<cytnx_uint32>(idx) = val;
  }
  void Uint32Storage::set_item(const cytnx_uint64 &idx, const cytnx_uint16 &val) {
    this->at<cytnx_uint32>(idx) = val;
  }
  void Uint32Storage::set_item(const cytnx_uint64 &idx, const cytnx_bool &val) {
    this->at<cytnx_uint32>(idx) = val;
  }

  // bool Uint32Storage::approx_eq(const boost::intrusive_ptr<Storage_base> &rhs,
