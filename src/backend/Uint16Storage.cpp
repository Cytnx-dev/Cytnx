...
namespace cytnx {
  ...
  void Uint16Storage::_Init_byptr(void *rawptr, const unsigned long long &len_in, const int &device,
  ...
  boost::intrusive_ptr<Storage_base> Uint16Storage::clone() {
    ...
    if (this->device == Device.cpu) {
      ...
      checkCudaErrors(cudaMemcpy(out->Mem, this->Mem, sizeof(cytnx_uint16) * this->len,
  ...
  void Uint16Storage::Move_memory_(const std::vector<cytnx_uint64> &old_shape,
  ...
  boost::intrusive_ptr<Storage_base> Uint16Storage::Move_memory(
...
// here, cpu->gpu with gid=device
        ...
        checkCudaErrors(
          cudaMemcpy(dtmp, this->Mem, sizeof(cytnx_uint16) * this->len, cudaMemcpyHostToDevice));
        free(this->Mem);
        this->Mem = dtmp;
        this->device = device;
#else
        ...
        if (device == Device.cpu) {
          ...
          checkCudaErrors(
            cudaMemcpy(htmp, this->Mem, sizeof(cytnx_uint16) * this->len, cudaMemcpyDeviceToHost));
          cudaFree(this->Mem);
          this->Mem = htmp;
          this->device = device;
        } else {
...
// here, cpu->gpu with gid=device
  ...
  void Uint16Storage::PrintElem_byShape(std::ostream &os, const std::vector<cytnx_uint64> &shape,
                                        const std::vector<cytnx_uint64> &mapper) {
  ...
  void Uint16Storage::resize(const cytnx_uint64 &newsize) {
    ...
    if (newsize > this->cap) {
      ...
      if (this->device == Device.cpu) {
        ...
        checkCudaErrors(
          cudaMemcpyPeer(dtmp, device, this->Mem, this->device, sizeof(cytnx_uint16) * this->len));
        cudaFree(this->Mem);
        this->Mem = dtmp;
#else
        cytnx_error_msg(
  ...
  // bool Uint16Storage::approx_eq(const boost::intrusive_ptr<Storage_base> &rhs,
