#include "cuGetElems_contiguous_gpu.hpp"
...
namespace cytnx {
  namespace utils_internal {
    ...
    template <class T>
    __global__ void cuGetElems_contiguous_kernel(T* d_out, T* d_in, cytnx_uint64* offj,
                                                 cytnx_uint64* new_offj, cytnx_uint64* locators,
                                                 cytnx_uint64* picksize, cytnx_uint64 rank,
                                                 cytnx_uint64 TotalElem, cytnx_uint64 Nelem_grp) {
      if (blockIdx.x * blockDim.x + threadIdx.x < TotalElem) {
        ...
        cudaMemcpyAsync(d_out + n * Nelem_grp, d_in + Loc * Nelem_grp, sizeof(T) * Nelem_grp,
                        cudaMemcpyDeviceToDevice);
        // memcpy(new_elem_ptr_ + n * Nelem_grp, elem_ptr_ + Loc * Nelem_grp,
        //        sizeof(cytnx_complex128) * Nelem_grp); from cpu code
      }
    }
    ...
    void cuGetElems_contiguous_gpu_cd(void* out, void* in, const std::vector<cytnx_uint64>& offj,
      ...
      checkCudaErrors(cudaMemcpy(d_offj.get(), &offj[0], sizeof(cytnx_uint64) * offj.size(),
                                 cudaMemcpyHostToDevice));

      utils_internal::DeviceBuffer<cytnx_uint64> d_new_offj(new_offj.size());
      checkCudaErrors(cudaMemcpy(d_new_offj.get(), &new_offj[0],
                                 sizeof(cytnx_uint64) * new_offj.size(), cudaMemcpyHostToDevice));
      ...
      checkCudaErrors(cudaMemcpy(d_picksize.get(), &picksize[0],
                                 sizeof(cytnx_uint64) * picksize.size(), cudaMemcpyHostToDevice));

      cytnx_uint64 NBlocks = TotalElem / 256;
      if (TotalElem % 256) NBlocks += 1;
      cuGetElems_contiguous_kernel<<<NBlocks, 256>>>(
        new_elem_ptr_, elem_ptr_, d_offj.get(), d_new_offj.get(), d_locators.get(),
        d_picksize.get(), offj.size(), TotalElem, Nelem_grp);
    }
