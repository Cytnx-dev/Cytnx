...
namespace cytnx {
  ...
  namespace linalg_internal {
    ...
    void cuSum_internal_cd(boost::intrusive_ptr<Storage_base> &out,
                           ...
                           const char &type) {
      utils_internal::cuReduce_gpu_cd((cytnx_complex128 *)out->data(),
                                      (cytnx_complex128 *)ten->data(), Nelem);
    }
    void cuSum_internal_cf(boost::intrusive_ptr<Storage_base> &out,
                           const boost::intrusive_ptr<Storage_base> &ten, const cytnx_uint64 &Nelem,
                           const char &type) {
      utils_internal::cuReduce_gpu_cf((cytnx_complex64 *)out->data(),
                                      (cytnx_complex64 *)ten->data(), Nelem);
    }
    void cuSum_internal_d(boost::intrusive_ptr<Storage_base> &out,
                          const boost::intrusive_ptr<Storage_base> &ten, const cytnx_uint64 &Nelem,
                          const char &type) {
      utils_internal::cuReduce_gpu_d((cytnx_double *)out->data(), (cytnx_double *)ten->data(),
                                     Nelem);
    }
    void cuSum_internal_f(boost::intrusive_ptr<Storage_base> &out,
                          const boost::intrusive_ptr<Storage_base> &ten, const cytnx_uint64 &Nelem,
                          const char &type) {
      utils_internal::cuReduce_gpu_f((cytnx_float *)out->data(), (cytnx_float *)ten->data(), Nelem);
    }
    void cuSum_internal_i64(boost::intrusive_ptr<Storage_base> &out,
      ...
      utils_internal::cuReduce_gpu_i64((cytnx_int64 *)out->data(), (cytnx_int64 *)ten->data(),
                                       Nelem);
    }
    void cuSum_internal_u64(boost::intrusive_ptr<Storage_base> &out,
                            const boost::intrusive_ptr<Storage_base> &ten,
                            const cytnx_uint64 &Nelem, const char &type) {
      utils_internal::cuReduce_gpu_u64((cytnx_uint64 *)out->data(), (cytnx_uint64 *)ten->data(),
                                       Nelem);
    }
    void cuSum_internal_i32(boost::intrusive_ptr<Storage_base> &out,
                            const boost::intrusive_ptr<Storage_base> &ten,
                            const cytnx_uint64 &Nelem, const char &type) {
      utils_internal::cuReduce_gpu_i32((cytnx_int32 *)out->data(), (cytnx_int32 *)ten->data(),
                                       Nelem);
    }
    void cuSum_internal_u32(boost::intrusive_ptr<Storage_base> &out,
                            const boost::intrusive_ptr<Storage_base> &ten,
                            const cytnx_uint64 &Nelem, const char &type) {
      utils_internal::cuReduce_gpu_u32((cytnx_uint32 *)out->data(), (cytnx_uint32 *)ten->data(),
                                       Nelem);
    }
    void cuSum_internal_i16(boost::intrusive_ptr<Storage_base> &out,
                            const boost::intrusive_ptr<Storage_base> &ten,
                            const cytnx_uint64 &Nelem, const char &type) {
      utils_internal::cuReduce_gpu_i16((cytnx_int16 *)out->data(), (cytnx_int16 *)ten->data(),
                                       Nelem);
    }
    void cuSum_internal_u16(boost::intrusive_ptr<Storage_base> &out,
                            const boost::intrusive_ptr<Storage_base> &ten,
                            const cytnx_uint64 &Nelem, const char &type) {
      utils_internal::cuReduce_gpu_u16((cytnx_uint16 *)out->data(), (cytnx_uint16 *)ten->data(),
                                       Nelem);
    }
