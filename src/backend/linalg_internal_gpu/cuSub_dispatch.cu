...
namespace cytnx {
  ...
  namespace linalg_internal {
    ...
    // Typed GPU dispatch for out-of-place subtraction. Delegates to the shared
    // std::visit machinery in cuArithmeticDispatch.cuh (op_code 2 = Sub), which
    ...
    void cuSub_dispatch(boost::intrusive_ptr<Storage_base> &out,
      ...
      cuArithmeticDispatchGPU<2>(out, Lin, Rin, len, shape, invmapper_L, invmapper_R);
    }
