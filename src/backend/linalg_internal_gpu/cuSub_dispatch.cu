...
namespace cytnx {
  ...
  namespace linalg_internal {
    ...
    namespace {
      ...
      template <typename TO, typename TL, typename TR>
      __global__ void cuSub_dispatch_constconst_kernel(TO *out, const TL lhs, const cytnx_uint64 n,
                                                       const TR rhs) {
        const cytnx_uint64 idx = blockIdx.x * blockDim.x + threadIdx.x;
        if (idx < n) out[idx] = CuSubDispatchOp<TO>(lhs, rhs);
      }

      template <typename TO, typename TL, typename TR>
      __global__ void cuSub_dispatch_lconst_kernel(TO *out, const TL lhs, const cytnx_uint64 n,
                                                   const TR *rhs) {
        const cytnx_uint64 idx = blockIdx.x * blockDim.x + threadIdx.x;
        if (idx < n) out[idx] = CuSubDispatchOp<TO>(lhs, rhs[idx]);
      }

      template <typename TO, typename TL, typename TR>
      __global__ void cuSub_dispatch_rconst_kernel(TO *out, const TL *lhs, const cytnx_uint64 n,
                                                   const TR rhs) {
        const cytnx_uint64 idx = blockIdx.x * blockDim.x + threadIdx.x;
        if (idx < n) out[idx] = CuSubDispatchOp<TO>(lhs[idx], rhs);
      }

      template <typename TO, typename TL, typename TR>
      __global__ void cuSub_dispatch_tn_kernel(TO *out, const TL *lhs, const cytnx_uint64 n,
                                               const TR *rhs) {
        const cytnx_uint64 idx = blockIdx.x * blockDim.x + threadIdx.x;
        if (idx < n) out[idx] = CuSubDispatchOp<TO>(lhs[idx], rhs[idx]);
      }

      template <typename TO, typename TL, typename TR>
      __global__ void cuSub_dispatch_tn_kernel_nonconti(
        ...
        if (idx < n) {
          ...
          out[idx] = CuSubDispatchOp<TO>(lhs[Lidx], rhs[Ridx]);
        }
      }

      template <typename TL, typename TR>
      void cuSub_dispatch_typed(boost::intrusive_ptr<Storage_base> &out,
        ...
        cytnx_error_msg(out->dtype() != Type_class::cy_typeid_gpu_v<TO>,
                        "[cuSub_dispatch] output dtype mismatch. got=%d expected=%d%s",
        ...
        if (Lin->size() == 1 and Rin->size() == 1) {
          cuSub_dispatch_constconst_kernel<<<NBlocks, 512>>>(_out, _Lin[0], len, _Rin[0]);
        } else if (Lin->size() == 1) {
          cuSub_dispatch_lconst_kernel<<<NBlocks, 512>>>(_out, _Lin[0], len, _Rin);
        } else if (Rin->size() == 1) {
          cuSub_dispatch_rconst_kernel<<<NBlocks, 512>>>(_out, _Lin, len, _Rin[0]);
        } else {
          if (shape.size() == 0) {
            cuSub_dispatch_tn_kernel<<<NBlocks, 512>>>(_out, _Lin, len, _Rin);
          } else {
            ...
            cuSub_dispatch_tn_kernel_nonconti<<<NBlocks, 512,
      ...
      void cuSub_dispatch_rhs(boost::intrusive_ptr<Storage_base> &out,
                              boost::intrusive_ptr<Storage_base> &Lin,
                              boost::intrusive_ptr<Storage_base> &Rin,
        ...
        switch (Rin->dtype()) {
          ...
          case Type.Double:
            ...
            break;
          case Type.Float:
            cuSub_dispatch_typed<TL, cytnx_float>(out, Lin, Rin, len, shape, invmapper_L,
                                                  invmapper_R);
            break;
          case Type.Int64:
            cuSub_dispatch_typed<TL, cytnx_int64>(out, Lin, Rin, len, shape, invmapper_L,
                                                  invmapper_R);
            break;
          case Type.Uint64:
            cuSub_dispatch_typed<TL, cytnx_uint64>(out, Lin, Rin, len, shape, invmapper_L,
                                                   invmapper_R);
            break;
          case Type.Int32:
            cuSub_dispatch_typed<TL, cytnx_int32>(out, Lin, Rin, len, shape, invmapper_L,
                                                  invmapper_R);
            break;
          case Type.Uint32:
            cuSub_dispatch_typed<TL, cytnx_uint32>(out, Lin, Rin, len, shape, invmapper_L,
                                                   invmapper_R);
            break;
          case Type.Int16:
            cuSub_dispatch_typed<TL, cytnx_int16>(out, Lin, Rin, len, shape, invmapper_L,
                                                  invmapper_R);
            break;
          case Type.Uint16:
            cuSub_dispatch_typed<TL, cytnx_uint16>(out, Lin, Rin, len, shape, invmapper_L,
                                                   invmapper_R);
            break;
          case Type.Bool:
            cuSub_dispatch_typed<TL, cytnx_bool>(out, Lin, Rin, len, shape, invmapper_L,
                                                 invmapper_R);
            break;
          default:
            cytnx_error_msg(true, "[cuSub_dispatch] unsupported rhs dtype: %d%s", Rin->dtype(),
                            "\n");
        }
      }
    ...
    void cuSub_dispatch(boost::intrusive_ptr<Storage_base> &out,
                        boost::intrusive_ptr<Storage_base> &Lin,
      ...
      cytnx_error_msg(out->dtype() != expected_dtype,
                      "[cuSub_dispatch] output dtype mismatch. got=%d expected=%d%s", out->dtype(),
                      expected_dtype, "\n");
      ...
      switch (Lin->dtype()) {
        ...
        case Type.Double:
          ...
          break;
        case Type.Float:
          cuSub_dispatch_rhs<cytnx_float>(out, Lin, Rin, len, shape, invmapper_L, invmapper_R);
          break;
        case Type.Int64:
          cuSub_dispatch_rhs<cytnx_int64>(out, Lin, Rin, len, shape, invmapper_L, invmapper_R);
          break;
        case Type.Uint64:
          cuSub_dispatch_rhs<cytnx_uint64>(out, Lin, Rin, len, shape, invmapper_L, invmapper_R);
          break;
        case Type.Int32:
          cuSub_dispatch_rhs<cytnx_int32>(out, Lin, Rin, len, shape, invmapper_L, invmapper_R);
          break;
        case Type.Uint32:
          cuSub_dispatch_rhs<cytnx_uint32>(out, Lin, Rin, len, shape, invmapper_L, invmapper_R);
          break;
        case Type.Int16:
          cuSub_dispatch_rhs<cytnx_int16>(out, Lin, Rin, len, shape, invmapper_L, invmapper_R);
          break;
        case Type.Uint16:
          cuSub_dispatch_rhs<cytnx_uint16>(out, Lin, Rin, len, shape, invmapper_L, invmapper_R);
          break;
        case Type.Bool:
          cuSub_dispatch_rhs<cytnx_bool>(out, Lin, Rin, len, shape, invmapper_L, invmapper_R);
          break;
        default:
          cytnx_error_msg(true, "[cuSub_dispatch] unsupported lhs dtype: %d%s", Lin->dtype(), "\n");
      }
    }
