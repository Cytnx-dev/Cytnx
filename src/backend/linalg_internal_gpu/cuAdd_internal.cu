...
namespace cytnx {
  ...
  namespace linalg_internal {
    ...
    // Typed GPU dispatch for out-of-place addition. Delegates to the shared std::visit
    // machinery in cuArithmeticDispatch.cuh (op_code 0 = Add), which mirrors the CPU
    // design: dispatch on the ordinary Cytnx value types and map to the CUDA-native
    // representation only at the kernel boundary (#1013).
    void cuAdd_dispatch(boost::intrusive_ptr<Storage_base> &out,
      ...
      cuArithmeticDispatchGPU<0>(out, Lin, Rin, len, shape, invmapper_L, invmapper_R);
    }
