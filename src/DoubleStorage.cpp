...
namespace cytnx{
    ...
    void DoubleStorage::print_elems(){
        ...
        for(unsigned long long cnt=0;cnt<this->len;cnt++){
            sprintf(buffer,"%.5e ",elem_ptr_[cnt]); std::cout << std::string(buffer);
        }
        cout << "]" << endl;
        free(buffer);
    }
    ...
    void DoubleStorage::resize(const cytnx_uint64 &newsize){
        ...
        if(newsize > this->cap){
            ...
            if(this->device==Device.cpu){
                void *htmp = malloc(sizeof(cytnx_double)*this->cap);
                ...
                this->Mem = htmp;
                    ...
                    checkCudaErrors(cudaMemcpyPeer(dtmp,device,this->Mem,this->device,sizeof(cytnx_double)*this->len));
                    cudaFree(this->Mem);
                    this->Mem = dtmp;
                #else
