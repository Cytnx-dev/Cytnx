...
namespace cytnx {
  ...
  void BlockFermionicUniTensor::beauty_print_block(const cytnx_uint64 &Nin,
                                                   const cytnx_uint64 &Nout,
                                                   const std::vector<cytnx_uint64> &qn_indices,
                                                   const std::vector<Bond> &bonds,
                                                   const Tensor &block, std::ostream &file) const {
    //[21 Aug 2024] This is a copy from BlockUniTensor
    ...
    file << (std::string(" ") * (3 + Lmax + 5)) << std::string("-") * (4 + mL + mR + 5) << endl;
    file << empty_line << endl;
  ...
  void BlockFermionicUniTensor::print_block(const cytnx_int64 &idx, const bool &full_info,
    ...
    file << "========================\n";
    if (this->_is_diag) file << " *is_diag: True\n";
    file << "BLOCK [#" << idx << "]\n";
    file << "fermionic sign = " << (this->_signflip[idx] ? -1 : +1) << "\n";
    /*
    file << "  |-Qn indices for each axis:\n   {\t";
    for(int s=0;s<this->_inner_to_outer_idx[idx].size();s++){
        file << this->_inner_to_outer_idx[idx][s] << "\t";
    }
    file << "}" << endl;
    file << "\t";
    for(int s=0;s<this->_bonds.size();s++){
        file << ((this->_bonds[s].type()>0)?"OUT":"IN") << "\t";
    }
    file << endl;
    file << "  |-Qn for each axis:\n";
    for(int s=0;s<this->_bonds[0].Nsym();s++){
        file << " " <<this->_bonds[0]._impl->_syms[s].stype_str() << ":\t";
        for(int l=0;l<this->_blocks[idx].shape().size();l++){
            file << std::showpos <<
    this->_bonds[l]._impl->_qnums[this->_inner_to_outer_idx[idx][l]][s] << "\t";
        }
        file << std::noshowpos << endl;
    }
    */
    file << " |- []   : Qn index \n";
    file << " |- Sym(): Qnum of correspond symmetry\n";
    this->beauty_print_block(this->_rowrank, this->_labels.size() - this->_rowrank,
                             this->_inner_to_outer_idx[idx], this->_bonds, this->_blocks[idx],
                             file);

    if (full_info)
      file << this->_blocks[idx];
    else {
      file << "  |-dtype:\t" << Type.getname(this->_blocks[idx].dtype()) << endl;
      file << "  |-device:\t" << Device.getname(this->_blocks[idx].device()) << endl;
      file << "  |-contiguous:\t" << (this->_blocks[idx].is_contiguous() ? "True" : "False")
           << endl;
      file << "  |-shape:\t";
      vec_print_simple(file, this->_blocks[idx].shape());
    }
  }

  void BlockFermionicUniTensor::print_blocks(const bool &full_info, std::ostream &file) const {
    //[21 Aug 2024] This is a copy from BlockUniTensor; sign structure is printed additionally
    file << "-------- start of print ---------\n";
    ...
    for (int i = 0; i < this->_signflip.size(); i++) {
      sprintf(buffer, this->_signflip[i] ? " -1" : " +1");
      file << buffer;
    }
    sprintf(buffer, "\n");
    file << buffer;
    ...
    */
    ...
    for (int b = 0; b < this->_blocks.size(); b++) {
      this->print_block(b, full_info, file);
    }
    ...
    /*
      ...
      for (cytnx_uint64 i = 0; i < tmp.size(); i++) {
        file << "Qnum:" << tmp_qnums[i] << std::endl;
        file << tmp[i] << std::endl;
        file << "=============\n";
      }
      file << "-------- end of print ---------\n";
  ...
  void BlockFermionicUniTensor::print_diagram(const bool &bond_info, std::ostream &file) const {
  ...
  boost::intrusive_ptr<UniTensor_base> BlockFermionicUniTensor::contiguous() {
    //[21 Aug 2024] This is a copy from BlockUniTensor; changed to create a BlockFermionicUniTensor
  ...
  boost::intrusive_ptr<UniTensor_base> BlockFermionicUniTensor::permute(
    ...
    for (cytnx_int64 i = 0; i < mapper.size(); i++) {
      ...
      cytnx_error_msg(
        ...
        "[ERROR][BlockFermionicUniTensor][permute] Label '%s' does not exist in UniTensor '%s'.\n",
  ...
  void BlockFermionicUniTensor::permute_(const std::vector<std::string> &mapper,
    ...
    for (cytnx_uint64 i = 0; i < mapper.size(); i++) {
      ...
      cytnx_error_msg(
        it == this->_labels.end(),
        "[ERROR][BlockFermionicUniTensor][permute_] Label '%s' does not exist in UniTensor '%s'.\n",
        mapper[i].c_str(), this->_name.c_str());
  ...
  boost::intrusive_ptr<UniTensor_base> BlockFermionicUniTensor::permute_nosignflip(
    ...
    for (cytnx_int64 i = 0; i < mapper.size(); i++) {
      ...
      cytnx_error_msg(it == out_raw->_labels.end(),
                      "[ERROR][BlockFermionicUniTensor][permute_nosignflip] Label '%s' does not "
                      "exist in UniTensor '%s'.\n",
                      mapper[i].c_str(), this->_name.c_str());
      mapper_i64.push_back(std::distance(out_raw->_labels.begin(), it));
    }
  ...
  void BlockFermionicUniTensor::permute_nosignflip_(const std::vector<std::string> &mapper,
    ...
    for (cytnx_uint64 i = 0; i < mapper.size(); i++) {
      ...
      cytnx_error_msg(it == this->_labels.end(),
                      "[ERROR][BlockFermionicUniTensor][permute_nosignflip_] Label '%s' does not "
                      "exist in UniTensor '%s'.\n",
                      mapper[i].c_str(), this->_name.c_str());
      mapper_i64.push_back(std::distance(this->_labels.begin(), it));
    }
  ...
  void BlockFermionicUniTensor::twist_(const std::string &label) {
    ...
    cytnx_error_msg(
      it == this->_labels.end(),
      "[ERROR][BlockFermionicUniTensor][twist_] Label '%s' does not exist in UniTensor '%s'.\n",
      label.c_str(), this->_name.c_str());
  ...
  boost::intrusive_ptr<UniTensor_base> BlockFermionicUniTensor::contract(
    ...
    cytnx_error_msg(
      ...
      "[ERROR][BlockFermionicUniTensors][contract] Cannot contract BlockFermionicUniTensor "
    ...
    cytnx_error_msg(
      this->syms() != rhs->syms(),
      "[ERROR][BlockFermionicUniTensors][contract] Two BlockFermionicUniTensors have different "
    ...
    if (comm_idx1.size() == 0) {
      ...
      for (int i = 0; i < comm_labels.size(); i++) {
        if (User_debug) {
          cytnx_error_msg(this->_bonds[comm_idx1[i]].qnums() != rhs->_bonds[comm_idx2[i]].qnums(),
                          "[ERROR][BlockFermionicUniTensors][contract] Contracted bond with label "
                          "'%s' has a qnum mismatch.\n",
                          comm_labels[i].c_str());
          cytnx_error_msg(this->_bonds[comm_idx1[i]].getDegeneracies() !=
                            rhs->_bonds[comm_idx2[i]].getDegeneracies(),
                          "[ERROR][BlockFermionicUniTensors][contract] Contracted bond with label "
                          "'%s' has a degeneracies mismatch.\n",
                          comm_labels[i].c_str());
        }
        cytnx_error_msg(this->_bonds[comm_idx1[i]].type() + rhs->_bonds[comm_idx2[i]].type(),
                        "[ERROR][BlockFermionicUniTensors][contract] A bond with type BRA can only "
                        "be contracted with type KET. "
                        "Invalid contraction for label '%s'.\n",
                        comm_labels[i].c_str());
      }
      ...
      if ((non_comm_idx1.size() == 0) && (non_comm_idx2.size() == 0)) {
        ...
        std::vector<bool> signfliplhs =
          this->_lhssigns_(_shadow_comm_idx1_reversed, comm_idx1.size());
          ...
          for (cytnx_int64 a = 0; a < this->_blocks.size(); a++) {
            ...
            if (false and (tmp->dtype() <= 4 and this->dtype() <= 4 and tmp_Rtn->dtype() <= 4) and
              ...
              cytnx_error_msg(true,
                              "[ERROR][BlockFermionicUniTensor][contract] Fermionic sign flips not "
                              "implemented yet in Gemm_Batch contracition. One needs to change the "
                              "signs of the alphas.%s",
                              "\n")
  ...
  void BlockFermionicUniTensor::Trace_(const cytnx_int64 &a, const cytnx_int64 &b) {
    ...
    cytnx_error_msg(
      ida == idb,
      "[ERROR][BlockFermionicUniTensor::Trace_] index a and index b should not be the same.%s",
      "\n");
    ...
    cytnx_error_msg(this->_bonds[ida]._impl->_degs != this->_bonds[idb]._impl->_degs,
                    "[ERROR][BlockFermionicUniTensor::Trace_] The dimension of two bond for trace "
                    "does not match!%s",
                    "\n");

    // check if two bonds qnum matches:
    cytnx_error_msg(this->_bonds[ida]._impl->_qnums != this->_bonds[idb]._impl->_qnums,
                    "[ERROR][BlockFermionicUniTensor::Trace_] The quantum numbers of two bond for "
                    "trace does not match!%s",
                    "\n");
  ...
  const Scalar::Sproxy BlockFermionicUniTensor::at_for_sparse(
    const std::vector<cytnx_uint64> &locator) const {
    //[21 Aug 2024] This is a copy from BlockUniTensor;
    // TODOfermions: one might want to store the sign in Sproxy as well in the future (here and in
    // other instances)
    cytnx_int64 bidx;
    std::vector<cytnx_uint64> loc_in_T;
    this->_fx_locate_elem(bidx, loc_in_T, locator);
  ...
  void BlockFermionicUniTensor::truncate_(const cytnx_int64 &bond_idx,
    ...
    cytnx_error_msg(
      this->is_diag(),
      "[ERROR][BlockFermionicUniTensor][truncate_] Cannot use truncate_ when is_diag() = true.%s",
      "\n");
    ...
    cytnx_error_msg(
      ...
      "[ERROR][BlockFermionicUniTensor][truncate_] Cannot remove the only qnums on a given Bond!%s",
      "\n");
  ...
  void BlockFermionicUniTensor::truncate_(const std::string &bond_idx,
    ...
    cytnx_error_msg(
      it == this->_labels.end(),
      "[ERROR][BlockFermionicUniTensor][truncate_] label '%s' does not exist in UniTensor '%s'.\n",
  ...
  void BlockFermionicUniTensor::_fx_group_duplicates(
    ...
    for (cytnx_int64 bn = 0; bn < dup_bond_idxs.size(); bn++) {
      ...
      std::vector<cytnx_bool> new_signs;
      ...
      for (cytnx_int64 b = 0; b < this->_blocks.size(); b++) {
        ...
        new_signs.push_back(this->_signflip[b]);
  ...
  void BlockFermionicUniTensor::combineBonds(const std::vector<cytnx_int64> &indicators,
    ...
    cytnx_error_msg(
      ...
      "UniTensor.to_dense()/to_dense_() first.%s [NOTE] this is BlockFermionicUniTensor, so "
  ...
  void BlockFermionicUniTensor::combineBond(const std::vector<std::string> &indicators,
    ...
    for (cytnx_uint64 i = 0; i < indicators.size(); i++) {
      ...
      cytnx_error_msg(
        it == this->_labels.end(),
        "[ERROR][BlockFermionicUniTensor][combineBond] Label '%s' not found in UniTensor '%s'.\n",
