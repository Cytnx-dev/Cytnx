    ...
    if (this->is_contiguous()) {
      ...
      for (unsigned int b = 0; b < this->_blocks.size(); b++) {
        if (this->_is_diag) {
          ...
          vector<cytnx_uint64> acc_in_old(this->_inner_rowrank),
            acc_out_old(oldshape.size() - this->_inner_rowrank);
  ...
  #pragma omp parallel for schedule(dynamic)
          ...
          for (unsigned int elem = 0; elem < this->_blocks[b].storage().size(); elem++) {
            ...
            vector<cytnx_uint64> tfidx =
              vec_concatenate(c2cartesian(this->_inner2outer_row[b][i], acc_in_old),
            ...
            // cout << "new shape:" ;
            // for(int t=0;t<tmp->_bonds.size();t++) cout << tmp->_bonds[t].dim() << " "; cout <<
            // endl;//[DEBUG]
            ...
            cout << "checkblock";
            cout << tmp->_outer2inner_row[new_row].first << " " <<
            tmp->_outer2inner_col[new_col].first << endl; cout << "newblock" << endl; cout <<
            tmp->_blocks[tmp->_outer2inner_row[new_row].first]<< endl; cout << "oldblock" << endl;
            cout << this->_blocks[b] << endl;
            */

            tmp->_blocks[tmp->_outer2inner_row[new_row].first].storage().at(
              tmp->_outer2inner_row[new_row].second *
                tmp->_blocks[tmp->_outer2inner_row[new_row].first].shape()[1] +
              tmp->_outer2inner_col[new_col].second) =
              this->_blocks[b].storage().at(i * this->_blocks[b].shape()[1] + j);
