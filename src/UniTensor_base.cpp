...
namespace cytnx {
  ...
  void UniTensor_base::Init(const std::vector<Bond> &bonds,
                            ...
                            const bool &no_alloc) {
    cytnx_error_msg(true, "[ERROR] fatal internal, cannot call on an un-initialized
  UniTensor_base%s",
                    "\n");
  }
  */
  ...
  std::vector<bool> UniTensor_base::signflip() const {
  ...
  boost::intrusive_ptr<UniTensor_base> UniTensor_base::permute_nosignflip(
    const std::vector<cytnx_int64> &mapper, const cytnx_int64 &rowrank) {
    cytnx_error_msg(true,
                    "[ERROR] fatal internal, can only be called on a BlockFermionicUniTensor. Use "
                    "permute_ instead for bosonic tensors!%s",
                    "\n");
    return nullptr;
  }
  boost::intrusive_ptr<UniTensor_base> UniTensor_base::permute_nosignflip(
  ...
  void UniTensor_base::permute_nosignflip_(const std::vector<cytnx_int64> &mapper,
                                           const cytnx_int64 &rowrank) {
    cytnx_error_msg(true,
                    "[ERROR] fatal internal, can only be called on a BlockFermionicUniTensor. Use "
                    "permute_ instead for bosonic tensors!%s",
  ...
  void UniTensor_base::permute_nosignflip_(const std::vector<std::string> &mapper,
                                           const cytnx_int64 &rowrank) {
    cytnx_error_msg(true,
                    "[ERROR] fatal internal, can only be called on a BlockFermionicUniTensor. Use "
                    "permute_ instead for bosonic tensors!%s",
  ...
  void UniTensor_base::to_hdf5_dispatch(H5::Group &container, const bool overwrite) const {
    cytnx_error_msg(
      true, "[ERROR] fatal internal, cannot call on an un-initialized UniTensor_base%s", "\n");
  }

  void UniTensor_base::from_hdf5_dispatch(H5::Group &container, bool restore_device) {
