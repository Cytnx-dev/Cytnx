...
namespace tor10{
    void Uint64Storage::Init(const unsigned long long &len_in,const int &device){
        ...
        this->dtype_id = tor10type.Uint64;
        if(device==tor10device.cpu){
            this->Mem = utils_internal::Calloc_cpu(this->len,sizeof(tor10_uint64));
        }else{
            #ifdef UNI_GPU
                tor10_error_msg(device>=tor10device.Ngpus,"%s","[ERROR] invalid device.");
                cudaSetDevice(device);
                this->Mem = utils_internal::cuMalloc_gpu(this->len*sizeof(tor10_uint64));
            #else
    ...
    boost::intrusive_ptr<Storage_base> Uint64Storage::copy(){
        ...
        if(this->device==tor10device.cpu){
            memcpy(out->Mem,this->Mem,sizeof(tor10_uint64)*this->len);
        }else{
            #ifdef UNI_GPU
                checkCudaErrors(cudaMemcpy(out->Mem,this->Mem,sizeof(tor10_uint64)*this->len,cudaMemcpyDeviceToDevice));
            #else
    ...
    void Uint64Storage::Move_memory_(const std::vector<tor10_uint64> &old_shape, const std::vector<tor10_uint64> &mapper, const std::vector<tor10_uint64> &invmapper){
    ...
    boost::intrusive_ptr<Storage_base> Uint64Storage::Move_memory(const std::vector<tor10_uint64> &old_shape, const std::vector<tor10_uint64> &mapper, const std::vector<tor10_uint64> &invmapper){
        boost::intrusive_ptr<Storage_base> tmp(this);
        if(this->device==tor10device.cpu){
        ...
        if(this->device != device){
            if(this->device==tor10device.cpu){
                //here, cpu->gpu with gid=device
                    ...
                    tor10_error_msg(device>=tor10device.Ngpus,"%s","[ERROR] invalid device.");
                    cudaSetDevice(device);          
                    void *dtmp = utils_internal::cuMalloc_gpu(sizeof(tor10_uint64)*this->len);
                    checkCudaErrors(cudaMemcpy(dtmp,this->Mem,sizeof(tor10_uint64)*this->len,cudaMemcpyHostToDevice));
                    free(this->Mem);
                    this->Mem = dtmp;
                    this->device=device;
                #else
                    ...
                    if(device==tor10device.cpu){
                        ...
                        void *htmp = malloc(sizeof(tor10_uint64)*this->len);
                        checkCudaErrors(cudaMemcpy(htmp,this->Mem,sizeof(tor10_uint64)*this->len,cudaMemcpyDeviceToHost));
                        cudaFree(this->Mem);
                        this->Mem = htmp;
                        this->device = device;
                    }else{
                        ...
                        cudaSetDevice(device);
                        void *dtmp = utils_internal::cuMalloc_gpu(sizeof(tor10_uint64)*this->len);
                        checkCudaErrors(cudaMemcpyPeer(dtmp,device,this->Mem,this->device,sizeof(tor10_uint64)*this->len));
                        cudaFree(this->Mem);
                        this->Mem = dtmp;
                        this->device = device;
                    }
    ...
    void Uint64Storage::PrintElem_byShape(std::ostream &os, const std::vector<tor10_uint64> &shape, const std::vector<tor10_uint64> &mapper){
        ...
        if(len==0){
            ...
            tor10_uint64 s;
            tor10_uint64* elem_ptr_ = static_cast<tor10_uint64*>(this->Mem);
