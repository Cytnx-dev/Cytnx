...
namespace cytnx{
    ...
    void BoolStorage::_Init_byptr(void *rawptr, const unsigned long long &len_in, const int &device, const bool &iscap, const unsigned long long &cap_in){
        this->Mem = rawptr;
        this->len = len_in;
        if(iscap){
            this->cap = cap_in;
    ...
    boost::intrusive_ptr<Storage_base> BoolStorage::clone(){
        ...
        if(this->device==Device.cpu){
            ...
            #ifdef UNI_GPU
                checkCudaErrors(cudaMemcpy(out->Mem,this->Mem,sizeof(bool)*this->len,cudaMemcpyDeviceToDevice));
            #else
    ...
    void BoolStorage::Move_memory_(const std::vector<cytnx_uint64> &old_shape, const std::vector<cytnx_uint64> &mapper, const std::vector<cytnx_uint64> &invmapper){
    ...
    boost::intrusive_ptr<Storage_base> BoolStorage::Move_memory(const std::vector<cytnx_uint64> &old_shape, const std::vector<cytnx_uint64> &mapper, const std::vector<cytnx_uint64> &invmapper){
        boost::intrusive_ptr<Storage_base> tmp(this);
        if(this->device==Device.cpu){
        ...
        if(this->device != device){
            if(this->device==Device.cpu){
                //here, cpu->gpu with gid=device
                    ...
                    checkCudaErrors(cudaMemcpy(dtmp,this->Mem,sizeof(bool)*this->cap,cudaMemcpyHostToDevice));
                    free(this->Mem);
                    this->Mem = dtmp;
                    this->device=device;
                #else
                    ...
                    if(device==Device.cpu){
                        ...
                        checkCudaErrors(cudaMemcpy(htmp,this->Mem,sizeof(bool)*this->cap,cudaMemcpyDeviceToHost));
                        cudaFree(this->Mem);
                        this->Mem = htmp;
                        this->device = device;
                    }else{
                        ...
                        cudaSetDevice(device);
                        void *dtmp = utils_internal::cuMalloc_gpu(sizeof(bool)*this->cap);
                        checkCudaErrors(cudaMemcpyPeer(dtmp,device,this->Mem,this->device,sizeof(bool)*this->cap));
                        cudaFree(this->Mem);
                        this->Mem = dtmp;
                        this->device = device;
                    }
        ...
        if(this->device == device){
            ...
            if(this->device==Device.cpu){
                //here, cpu->gpu with gid=device
                    ...
                    cudaSetDevice(device);          
                    void *dtmp = utils_internal::cuMalloc_gpu(sizeof(bool)*this->cap);
                    checkCudaErrors(cudaMemcpy(dtmp,this->Mem,sizeof(float)*this->cap,cudaMemcpyHostToDevice));
                    boost::intrusive_ptr<Storage_base> out(new BoolStorage());
    ...
    void BoolStorage::resize(const cytnx_uint64 &newsize){
        ...
        if(newsize > this->cap){
            ...
            if(this->device==Device.cpu){
                void *htmp = malloc(sizeof(cytnx_bool)*this->cap);
                ...
                this->Mem = htmp;
                    ...
                    cytnx_error_msg(device>=Device.Ngpus,"%s","[ERROR] invalid device.");
                    cudaSetDevice(device);
                    void *dtmp = utils_internal::cuMalloc_gpu(sizeof(cytnx_bool)*this->cap);
                    checkCudaErrors(cudaMemcpyPeer(dtmp,device,this->Mem,this->device,sizeof(cytnx_bool)*this->len));
                    cudaFree(this->Mem);
                    this->Mem = dtmp;
                #else
