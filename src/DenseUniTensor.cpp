...
namespace cytnx{
    ...
    boost::intrusive_ptr<UniTensor_base> DenseUniTensor::relabels(const std::vector<cytnx_int64> &new_labels){
    ...
    void DenseUniTensor::reshape_(const std::vector<cytnx_int64> &new_shape, const cytnx_uint64 &rowrank){
        ...
        if(comm_idx1.size() == 0){
            ...
            if(this->is_tag()){
                for(int i=0;i<comm_idx1.size();i++)
                    cytnx_error_msg(this->_bonds[comm_idx1[i]].type() == rhs->_bonds[comm_idx2[i]].type(),"[ERROR][DenseUniTensor][contract] cannot contract common label: <%d> @ self bond#%d & rhs bond#%d, BRA-KET mismatch!%s",this->labels()[comm_idx1[i]],comm_idx1[i],comm_idx2[i],"\n");
            }
    ...
    void DenseUniTensor::Trace_(const cytnx_int64 &a, const cytnx_int64 &b, const bool &by_label){
        ...
        cytnx_error_msg(ida == idb, "[ERROR][DenseUniTensor::Trace_] index a and index b should not be the same.%s","\n");

        // check dimension:
        cytnx_error_msg(this->_bonds[ida].dim()!= this->_bonds[idb].dim(),"[ERROR][DenseUniTensor::Trace_] The dimension of two bond for trace does not match!%s","\n");
        ...
        if(this->is_braket_form()){
            ...
            if(this->_bonds[ida].type() == this->_bonds[idb].type()){
                cytnx_error_msg(true,"[ERROR][DenseUniTensor::Trace_] BD_BRA can only contract with BD_KET.%s","\n");
            }
    ...
    void DenseUniTensor::_save_dispatch(std::fstream &f) const{
        this->_block._Save(f); 
    }
    ...
    void DenseUniTensor::truncate_(const cytnx_int64 &bond_idx, const cytnx_uint64 &dim, const bool &by_label){
        ...
        if(by_label){
            ...
            cytnx_error_msg(it==this->_labels.end(),"[ERROR][DenseUniTensor][truncate] Error, bond label does not exist in the current label list.%s","\n");
            idx = std::distance(this->_labels.begin(),it);
        }else{
            idx = bond_idx;
        }
        cytnx_error_msg(idx>=this->_labels.size(),"[ERROR][DenseUniTensor][truncate] Error, index [%d] is out of range. Total rank: %d\n",idx,this->_labels.size());    
        cytnx_error_msg(dim>this->_bonds[idx].dim(),"[ERROR][DenseUniTensor][truncate] dimension can only be <= the dimension of the instance bond.%s","\n");
